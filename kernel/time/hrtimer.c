...
#ifdef CONFIG_QGKI
static bool nanosleep_validate_interval(struct timespec64 *ts)
{
	ktime_t kt;
	s64 interval = 0;
	kt = timespec64_to_ktime(*ts);
	interval = ktime_to_us(kt);
	if (interval < 200) {
		if (current->group_leader == NULL)
			return true;

		printk_ratelimited(KERN_ERR "nanosleep %d: [%d|%s]:[%d|%s]\n",
							interval, current->pid, current->comm,
							current->tgid, current->group_leader->comm);

		//filter those app tasks which may fire the nanosleep events storm
		if ((task_uid(current->group_leader).val) > 10000) {
			printk_ratelimited(KERN_ERR "nanosleep %d: treat task %s\n", interval, current->group_leader->comm);
			yield();
			return false;
		}
	}
...
#ifdef CONFIG_QGKI
	if (!nanosleep_validate_interval(&tu))
		return 0;
#endif	/*CONFIG_QGKI*/
