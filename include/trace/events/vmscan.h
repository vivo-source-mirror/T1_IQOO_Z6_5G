...
DECLARE_EVENT_CLASS(mm_vmscan_vital_page_template,
	TP_PROTO(s8 priority,
		struct inode *inode,
		struct page *page,
		int lru,
		int reserved,
	...
	TP_ARGS(priority, inode, page, lru, reserved, total_scan),

	TP_STRUCT__entry(
		__field(s8, priority)
		__field(unsigned long, i_ino)
		__field(unsigned long, i_child_of)
	...
	TP_fast_assign(
		...
		__entry->i_child_of = inode->i_child_of;
		__entry->pfn = page_to_pfn(page);
		__entry->lru = lru;
		__entry->active = PageActive(page);
		__entry->vital = inode->i_vital;
		__entry->free = nr_free_pages();
		__entry->reserved = reserved;
		__entry->total_scan = total_scan;
	),

	TP_printk("lru=%s, priority=%d, ino=%lu, child_of=%lu, active=%d, "
			"vital=%d, pfn=%lu, free=%lu, reserved=%d, total_scan=%lu",
...
DEFINE_EVENT(mm_vmscan_vital_page_template, mm_vmscan_skip_page,
	TP_PROTO(s8 priority,
		struct inode *inode,
		struct page *page,
		int lru,
...
TRACE_EVENT(mm_vmscan_lru_shrink_active,
	...
	TP_fast_assign(
		...
		__entry->reclaim_flags = trace_reclaim_flags(file);
#ifdef CONFIG_FCACHE_CTRL
		__entry->special = special;
#endif
	),

	TP_printk("nid=%d nr_taken=%ld nr_active=%ld nr_deactivated=%ld nr_referenced=%ld priority=%d "
#ifdef CONFIG_FCACHE_CTRL
			"special=%d "
#endif
			"flags=%s",
