...
#define SEC_SERIAL_NUM_ADDR			0x01B401B8
#define SEC_ENABLE_STAGE_ADDR       0x01B401CC
#define	SEC_ENABLE_REG_ADDR			0x01B405D8
#define SEC_KEY_VER_ADDR			0x01B401C4
#define SEC_ROOT_CERT_HASH_ADDR		0x01B40238
#define	SEC_RPMB_KEY_PROVISION_ADDR	0x01B40228
...
static int sec_stage_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "0x%08x\n",
		sec_stage);
	return 0;
...
static int sec_stage_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, sec_stage_proc_show, NULL);
}
...
static const struct file_operations sec_stage_proc_fops = {
	.open		= sec_stage_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
// secboot stage:
    proc_create("sec_stage", 0444, NULL, &sec_stage_proc_fops);
	request_mem_region(SEC_ENABLE_STAGE_ADDR, 0x8, "sec_stage");
	sec_stage_ptr = ioremap(SEC_ENABLE_STAGE_ADDR, 0x8);
	sec_stage = ioread32(sec_stage_ptr);
	printk(KERN_ERR "sec_enable=0x%lx, sec_stage=0x%lx\n", sec_enable, sec_stage);
	release_mem_region(SEC_ENABLE_STAGE_ADDR, 0x8); 
	iounmap(sec_stage_ptr);
