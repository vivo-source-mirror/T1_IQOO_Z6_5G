...
#define QPNP_PON_WD_INT_EN_SET(pon)			((pon)->base + 0x15)
#define QPNP_PON_WD_INT_EN_CLR(pon)			((pon)->base + 0x16)
#define QPNP_PON_WD_INT_EN_ACTIVE			(0x08)
#define QPNP_PON_WD_INT_EN_SET_MASK			(BIT(3))
...
static const char * const qpnp_poff_reason[][2] = {
	...
	[5] = {"Triggered from KPDPWR_AND_RESIN (Power Key and Reset Line)", PREASON_KPDPWR_AND_RESIN},
	...
	[14] = {"Triggered from OTST3 (Over Temperature)", PREASON_OTST3},
	...
	[31] = {"Triggered from OTST3 (Over Temperature)", PREASON_OTST3},
	...
	[39] = {"Triggered from S3_RESET_KPDPWR_ANDOR_RESIN", PREASON_S3_RESET_KPDPWR_ANDOR_RESIN},
};

static char qpnp_pon_reason_str[64] = "none";
static char qpnp_poff_reason_str[64] = "none";
static int combo = 1;
module_param(combo, int, S_IRUSR);
...
char *qpnp_pon_get_pon_reason(void)
{
	return qpnp_pon_reason_str;
}
EXPORT_SYMBOL(qpnp_pon_get_pon_reason);

char *qpnp_pon_get_poff_reason(void)
{
	return qpnp_poff_reason_str;
}
EXPORT_SYMBOL(qpnp_pon_get_poff_reason);

static int qpnp_pon_kpdpwr_resin_cfg(struct qpnp_pon *pon,
		struct qpnp_pon_config *cfg, bool debug_mode)
{
	int rc = 0;
	...
	pr_err("debug_mode:%d, s2_type:%d\n", debug_mode, cfg->s2_type);
...
void qpnp_pon_kpdpwr_resin_ex(int enable)
{
	...
	pr_err("KPDPWR_RESIN s2_type enable:%d\n", !!enable);
...
static int qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	pr_err("[HW_KEY] PMIC input: keycode=%d, status=0x%02X\n", cfg->key_code,
		pon_rt_sts);
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	/*vsen team add to enable disable volume dowm to wakeup*/
...
static int qpnp_pon_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_kpdpwr_volume_down_wake);/*vsen team add */
