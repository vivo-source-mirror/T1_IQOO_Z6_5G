...
static int fts_caltimediff(struct timespec *diff, struct timespec *start, struct timespec *end)   
{
...
int fts8756_fts_fw_resume(void)
{
    int ret = 0;
	struct firmware fw_download;
	int fw_size = 0;
...
int fts8756_focal_fw_recovery(void)
{
    int ret = 0;
    u8 boot_state = 0;
    u8 chip_id = 0;
