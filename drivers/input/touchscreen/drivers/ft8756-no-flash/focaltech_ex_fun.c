...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_WRITE_REGISTER:
        writelen = 2;
        ret = fts8756_fts_write(writebuf + 1, writelen);
        if (ret < 0) {
	...
	case PROC_ENTER_TEST_ENVIRONMENT:  
        if (FTS_NORMAL_MODE == writebuf[1]) {
            ret = fts8756_fts_fw_enter_test_environment(FTS_NORMAL_MODE);
        } else {
            ret = fts8756_fts_fw_enter_test_environment(FTS_FACTORY_MODE);
        }
        break;
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_READ_REGISTER:
        readlen = 1;
        ret = fts8756_fts_read(ts_data->proc->cmd, 1, buf, 1);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        readlen = count;
        ret = fts8756_fts_read(ts_data->proc->cmd, ts_data->proc->cmd_len, buf, readlen);
        if (ret < 0) {
    ...
    case PROC_READ_STATUS:
        /*
        ret = fts8756_fts_read_status(buf);
        if (ret < 0) {
...
int fts8756_fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
...
void fts8756_fts_release_apk_debug_channel(struct fts_ts_data *ts_data)
{
    if (ts_data->proc) {
...
static ssize_t fts_bootmode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    FTS_FUNC_ENTER();
    mutex_lock(&fts8756_fts_data->mutex);
    if (true == fts8756_fts_data->fw_is_running) {
...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_esdcheck_proc_busy(1);
#endif
    if (fts8756_fts8756_fts_read_reg_byte(FTS_REG_FW_VER, &fwver) < 0) {
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = fts8756_fts8756_fts_read_reg_byte(reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = fts8756_fts_read(&reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = fts8756_fts8756_fts_write_reg_byte(reg, val);
            } else {
                rw_op.res = fts8756_fts_write(rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
...
/* fts8756_fts_upgrade_bin interface */
static ssize_t fts_fwupgradebin_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    fts8756_fts_upgrade_bin(fwname, 0);
    mutex_unlock(&fts8756_fts_data->mutex);
...
static ssize_t fts_dumpreg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_esdcheck_proc_busy(1);
#endif
    fts8756_fts8756_fts_read_reg_byte(FTS_REG_POWER_MODE, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Power Mode:0x%02x\n", val);

    fts8756_fts8756_fts_read_reg_byte( FTS_REG_FW_VER, &val);
    count += snprintf(buf + count, PAGE_SIZE, "FW Ver:0x%02x\n", val);

    fts8756_fts8756_fts_read_reg_byte( FTS_REG_LIC_VER, &val);
    count += snprintf(buf + count, PAGE_SIZE, "LCD Initcode Ver:0x%02x\n", val);

    fts8756_fts8756_fts_read_reg_byte( FTS_REG_IDE_PARA_VER_ID, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Param Ver:0x%02x\n", val);

    fts8756_fts8756_fts_read_reg_byte( FTS_REG_IDE_PARA_STATUS, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Param status:0x%02x\n", val);

    fts8756_fts8756_fts_read_reg_byte( FTS_REG_VENDOR_ID, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Vendor ID:0x%02x\n", val);

    fts8756_fts8756_fts_read_reg_byte( FTS_REG_LCD_BUSY_NUM, &val);
    ...
    fts8756_fts8756_fts_read_reg_byte( FTS_REG_CHARGER_MODE_EN, &val);
    count += snprintf(buf + count, PAGE_SIZE, "charge stat:0x%02x\n", val);

    fts8756_fts8756_fts_read_reg_byte( FTS_REG_INT_CNT, &val);
    count += snprintf(buf + count, PAGE_SIZE, "INT count:0x%02x\n", val);

    fts8756_fts8756_fts_read_reg_byte( FTS_REG_FLOW_WORK_CNT, &val);
...
static ssize_t fts_tpbuf_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < fts8756_fts_data->pnt_buf_size; i++) {
        count += snprintf(buf + count, PAGE_SIZE, "%02x ", fts8756_fts_data->point_buf[i]);
    }
    count += snprintf(buf + count, PAGE_SIZE, "\n");
    mutex_unlock(&fts8756_fts_data->mutex);
...
/*  upgrade from fw bin file   example:echo "*.bin" > fts8756_fts_upgrade_bin */
static DEVICE_ATTR(fts8756_fts_upgrade_bin, S_IRUGO | S_IWUSR, fts_fwupgradebin_show, fts_fwupgradebin_store);
...
static struct attribute *fts_attributes[] = {
    ...
    &dev_attr_fts8756_fts_upgrade_bin.attr,
