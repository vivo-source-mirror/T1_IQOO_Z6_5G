...
int fts_create_gesture_sysfs_V2(struct device *dev)
{
    int ret = 0;
...
static void fts_gesture_report(int gesture_id)
{
    ...
    /* report event key */
    if (gesture != -1) { 
		vts_report_event_down(fts_data_V2->vtsdev, gesture);
		vts_report_event_up(fts_data_V2->vtsdev, gesture);
    }

    FTS_FUNC_EXIT();
}

int fts_gesture_readdata_V2(struct fts_ts_data *ts_data, u8 *data)
{
    int i = 0;
    int gestrue_id = 0;
    u8 pointnum = 0;
    ...
    memset(fts_gesture_data.coordinate_x, 0, sizeof(fts_gesture_data.coordinate_x));
    memset(fts_gesture_data.coordinate_y, 0, sizeof(fts_gesture_data.coordinate_y));

    gestrue_id = data[2];
    pointnum = data[3];
    VTD("[GESTURE]PointNum=%d", pointnum);
    if (pointnum > ARRAY_SIZE(fts_gesture_data.coordinate_x)) {
        VTE("gesture pointnum(%d) fail", pointnum);
        return -EIO;
    }

	for (i = 0; i < ARRAY_SIZE(fts_gesture_data.coordinate_x); i++)
	...
	pointnum = ARRAY_SIZE(fts_gesture_data.coordinate_y);
	if (GESTURE_O_CW == gestrue_id) {
		fts_gesture_data.coordinate_y[ARRAY_SIZE(fts_gesture_data.coordinate_y)-1] = 16;
	} else if (GESTURE_O == gestrue_id) {
		fts_gesture_data.coordinate_y[ARRAY_SIZE(fts_gesture_data.coordinate_y)-1] = 32;
	}
...
void fts_gesture_recovery_V2(void)
{
    if ((ENABLE == fts_gesture_data.mode) && (ENABLE == fts_gesture_data.active)) {
        VTD("enter fts_gesture_recovery_V2");
        fts_write_reg_byte_V2(0xD1, 0xff);
        fts_write_reg_byte_V2(0xD2, 0xff);
        fts_write_reg_byte_V2(0xD5, 0xff);
        fts_write_reg_byte_V2(0xD6, 0xff);
        fts_write_reg_byte_V2(0xD7, 0xff);
        fts_write_reg_byte_V2(0xD8, 0xff);
        fts_write_reg_byte_V2(FTS_REG_GESTURE_EN, ENABLE);
    }
}
...
int fts_gesture_suspend_V2(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        */
        fts_write_reg_byte_V2(FTS_REG_GESTURE_EN, ENABLE);
        msleep(1);
        fts_read_reg_byte_V2(FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
    }
...
int fts_gesture_resume_V2(struct fts_ts_data *ts_data)
{
    ...
    if (fts_gesture_data.active == DISABLE) {
        VTD("gesture in suspend is failed, no running fts_gesture_resume_V2");
        return -EINVAL;
    }

    fts_gesture_data.active = DISABLE;
    for (i = 0; i < 5; i++) {
        fts_write_reg_byte_V2(FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        fts_read_reg_byte_V2(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
...
{ 
    fts_create_gesture_sysfs_V2(&ts_data->spi->dev);
    fts_gesture_data.mode = ENABLE; 
    return 0;
}
