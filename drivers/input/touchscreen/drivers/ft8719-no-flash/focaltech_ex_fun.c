...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_WRITE_REGISTER:
        writelen = 2;
        ret = fts_write_V2(writebuf + 1, writelen);
        if (ret < 0) {
	...
	case PROC_ENTER_TEST_ENVIRONMENT:  
        if (FTS_NORMAL_MODE == writebuf[1]) {
            ret = fts_fw_enter_test_environment_V2(FTS_NORMAL_MODE);
        } else {
            ret = fts_fw_enter_test_environment_V2(FTS_FACTORY_MODE);
        }
        break;
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_READ_REGISTER:
        readlen = 1;
        ret = fts_read_V2(ts_data->proc->cmd, 1, buf, 1);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        readlen = count;
        ret = fts_read_V2(ts_data->proc->cmd, ts_data->proc->cmd_len, buf, readlen);
        if (ret < 0) {
...
int fts_create_apk_debug_channel_V2(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
...
void fts_release_apk_debug_channel_V2(struct fts_ts_data *ts_data)
{
    if (ts_data->proc) {
...
static ssize_t fts_hw_reset_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    FTS_FUNC_ENTER();
    mutex_lock(&fts_data_V2->mutex);
    fts_reset_proc_V2(0);
    count = snprintf(buf, PAGE_SIZE, "hw reset executed\n");
    mutex_unlock(&fts_data_V2->mutex);
    FTS_FUNC_EXIT();
...
static ssize_t fts_bootmode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    FTS_FUNC_ENTER();
    mutex_lock(&fts_data_V2->mutex);
    if (true == fts_data_V2->fw_is_running) {
...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_esdcheck_proc_busy(1);
#endif
    if (fts_read_reg_byte_V2(FTS_REG_FW_VER, &fwver) < 0) {
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = fts_read_reg_byte_V2(reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = fts_read_V2(&reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = fts_write_reg_byte_V2(reg, val);
            } else {
                rw_op.res = fts_write_V2(rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
...
static ssize_t fts_tpbuf_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < fts_data_V2->pnt_buf_size; i++) {
        count += snprintf(buf + count, PAGE_SIZE, "%02x ", fts_data_V2->point_buf[i]);
    }
    count += snprintf(buf + count, PAGE_SIZE, "\n");
    mutex_unlock(&fts_data_V2->mutex);
...
/*  upgrade from fw bin file   example:echo "*.bin" > fts_upgrade_bin_V2 */
static DEVICE_ATTR(fts_upgrade_bin_V2, S_IRUGO | S_IWUSR, fts_fwupgradebin_show, fts_fwupgradebin_store);
...
static struct attribute *fts_attributes[] = {
    ...
    &dev_attr_fts_upgrade_bin_V2.attr,
