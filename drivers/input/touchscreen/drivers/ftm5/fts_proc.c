...
#define CMD_CHANGE_OUTPUT_MODE			0xFF	/* /< Select the output
							 ...
							 * scriptless protocol,
							 * need to pass:
							 * info->bin_output = 1 data
							 * returned as binary,
							 * info->bin_output =0 data
							 * returned as hex
							 * string */
...
static void *fts_seq_start(struct seq_file *s, loff_t *pos)
{
	...
	if (info->driver_test_buff == NULL && *pos == 0) {
		logError_ftm5(1, "%s %s: No data to print!\n", tag_ftm5, __func__);
		...
		/* logError_ftm5(0, "%s %s: len = %d info->driver_test_buff = %s\n", tag_ftm5,
		...
		if (*pos >= info->limit)
			/* logError_ftm5(0, "%s %s: Apparently, we're done.\n", tag_ftm5,
			 * __func__); */
			return NULL;
	}
	...
	/* logError_ftm5(0, "%s %s: In start(), updated pos = %ld info->limit = %d info->printed = %d info->chunk = %d\n",
	 *	tag_ftm5, __func__, *pos, info->limit, info->printed, info->chunk); */
...
static void *fts_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	struct fts_ts_info *info = (struct fts_ts_info *)s->private;
	/* int* val_ptr; */
	/* logError_ftm5(0, "%s %s: In next(), v = %X, pos = %ld.\n", tag_ftm5, __func__,
	 * v, *pos); */
	(*pos) += info->chunk;/* increase my position counter */
	info->chunk = CHUNK_PROC;

	/* logError_ftm5(0, "%s %s: In next(), updated pos = %ld info->limit = %d info->printed = %d\n",
	 *	tag_ftm5, __func__, *pos, info->limit,info->printed); */
...
{
	/* logError_ftm5(0, "%s %s: Entering stop().\n", tag_ftm5, __func__); */
	struct fts_ts_info *info = (struct fts_ts_info *)s->private;
	if (v) {
		/* logError_ftm5(0, "%s %s: v is %X.\n", tag_ftm5, __func__, v); */
	} else {
		/* logError_ftm5(0, "%s %s: v is null.\n", tag_ftm5, __func__); */
		info->limit = 0;
		info->chunk = 0;
		info->printed = 0;
		if (info->driver_test_buff != NULL) {
			/* logError_ftm5(0, "%s %s: Freeing and clearing info->driver_test_buff.\n",
			 *	tag_ftm5, __func__); */
			kfree(info->driver_test_buff);
			info->driver_test_buff = NULL;
		} else {
			/* logError_ftm5(0, "%s %s: info->driver_test_buff is already null.\n",
			 *	tag_ftm5, __func__); */
		}
	}
}
...
pbuf = (char *)kmalloc(count * sizeof(u8),
						 GFP_KERNEL);
if(pbuf == NULL){
	res = ERROR_ALLOC;
	return res;
}
cmd = (u8 *)kmalloc(count * sizeof(u8),
						 GFP_KERNEL);
if(cmd == NULL){
	kfree(pbuf);
	res = ERROR_ALLOC;
...
funcToTest = (u32 *)kmalloc((count+1)/3 * sizeof(u32),
							 GFP_KERNEL);
if(funcToTest == NULL){
	kfree(cmd);
	kfree(pbuf);
	...
	/*for(temp = 0; temp<count; temp++){
	  *      logError_ftm5(0,"%s p[%d] = %02X\n",tag_ftm5, temp, p[temp]);
	  * }*/
	...
	if (count > MESSAGE_MIN_HEADER_SIZE - 1 && p[0] == MESSAGE_START_BYTE) {
		logError_ftm5(0, "%s Enter in Byte Mode!\n", tag_ftm5);
		byte_call = 1;
		...
		logError_ftm5(0,
			 ...
			 tag_ftm5, info->mess.msg_size, info->mess.counter, info->mess.action);
		...
		if (count < info->mess.msg_size || p[count - 2] != MESSAGE_END_BYTE) {
			logError_ftm5(1,
				 ...
				 tag_ftm5, info->mess.msg_size, (int)count, p[count - 1],
		...
		logError_ftm5(1, "%s functionToTest[0] = %02X cmd[0]= %02X\n", tag_ftm5,
			 funcToTest[0], cmd[0]);
		switch (funcToTest[0]) {
		case CMD_GETFWFILE:
	...
	if (numberParam >= 1) {
		...
		case CMD_VERSION_BYTE:
			logError_ftm5(0, "%s %s: Get Version Byte\n", tag_ftm5,
				 __func__);
			byteToRead = 2;
			info->mess.dummy = 0;
			...
			if (readData != NULL) {
				...
				res = OK;
				logError_ftm5(0, "%s %s: Version = %02X%02X\n", tag_ftm5,
		...
		case CMD_WRITEREAD_BYTE:
			if (numberParam >= 5) {	/* need to pass: cmd[0]  cmd[1]
				...
				logError_ftm5(0, "%s bytesToRead = %d\n", tag_ftm5,
				...
				readData = (u8 *)kmalloc((byteToRead +
							 ...
							 sizeof(u8),
							 GFP_KERNEL);
				res = ftm5_writeRead(info, &cmd[1], temp, readData,
						    byteToRead + info->mess.dummy);
				size += (byteToRead * sizeof(u8));
			} else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
					 tag_ftm5);
				res = ERROR_OP_NOT_ALLOW;
			}
			break;
		...
		case CMD_WRITE_BYTE:
			if (numberParam >= 2) {	/* need to pass: cmd[0]  cmd[1]
				...
				res = ftm5_write(info, &cmd[1], temp);
			} else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
					 tag_ftm5);
				res = ERROR_OP_NOT_ALLOW;
		...
		case CMD_READ_BYTE:
			if (numberParam >= 3) {	/* need to pass: byteToRead1
				...
				readData = (u8 *)kmalloc((byteToRead +
							 ...
							 sizeof(u8),
							 GFP_KERNEL);
				res = ftm5_read(info, readData, byteToRead +
					       info->mess.dummy);
				size += (byteToRead * sizeof(u8));
			} else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
		...
		case CMD_WRITETHENWRITEREAD_BYTE:
			...
			if (numberParam >= 6) {
				...
				readData = (u8 *)kmalloc(byteToRead *
							 sizeof(u8),
							 GFP_KERNEL);
				res = ftm5_writeThenWriteRead(info, &cmd[3], cmd[1],
							     &cmd[3 +
								  (int)cmd[1]],
							     cmd[2], readData,
							     byteToRead);
		...
		case CMD_WRITEU8UX_BYTE:
			...
			if (numberParam >= 4) {
				if (cmd[2] <= sizeof(u64)) {
					...
					logError_ftm5(0, "%s addr = %016llX %ld\n",
						 tag_ftm5, addr, (long int)addr);
					res = ftm5_writeU8UX(info, cmd[1], cmd[2],
							    addr, &cmd[3 +
								       cmd[2]],
							    (numberParam -
							     cmd
					...
					logError_ftm5(1, "%s Wrong address size!\n",
						 tag_ftm5);
					res = ERROR_OP_NOT_ALLOW;
				}
			} else {
			...
			if (numberParam >= 6) {
				if (cmd[2] <= sizeof(u64)) {
					...
					logError_ftm5(0,
						 ...
						 tag_ftm5, addr, byteToRead);
					res = ftm5_writeReadU8UX(info, cmd[1], cmd[2],
								addr, readData,
								byteToRead,
								cmd[numberParam
								    - 1]);
		...
		case CMD_WRITEU8UXTHENWRITEU8UX_BYTE:
			...
			if (numberParam >= 6) {
				if ((cmd[2] + cmd[4]) <= sizeof(u64)) {
					...
					logError_ftm5(0, "%s addr = %016llX %lld\n",
						 tag_ftm5, addr, addr);
					res = ftm5_writeU8UXthenWriteU8UX(info, cmd[1],
									 cmd[2],
									 cmd[3],
									 cmd[4],
									 addr,
			...
			if (numberParam >= 8) {
				if ((cmd[2] + cmd[4]) <= sizeof(u64)) {
					...
					logError_ftm5(1,
						 ...
						 tag_ftm5, __func__, cmd[5], addr);
					u8ToU16_be(&cmd[numberParam - 3],
						   &byteToRead);
					readData = (u8 *)kmalloc(byteToRead *
								 sizeof(u8),
								 GFP_KERNEL);
					res = ftm5_writeU8UXthenWriteReadU8UX(info,
		...
		case CMD_FWWRITE:
			if (numberParam >= 3) {	/* need to pass: cmd[0]  cmd[1]
				...
				if (numberParam >= 2) {
					temp = numberParam - 1;
					res = ftm5_writeFwCmd(info, &cmd[1], temp);
				} else {
					logError_ftm5(1, "%s Wrong parameters!\n",
						 tag_ftm5);
					res = ERROR_OP_NOT_ALLOW;
				}
			} else {
		...
		case CMD_INTERRUPT:
			...
			if (numberParam >= 2) {
				if (cmd[1] == 1)
					res = fts_enableInterrupt_ftm5(info);
				else
					res = fts_disableInterrupt_ftm5(info);
			} else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
		...
		case CMD_CLEANUP:/* TOUCH ENABLE/DISABLE */
			if (numberParam == 2)	/* need to pass: enableTouch */
				res = cleanUp_ftm5(info, funcToTest[1]);
			else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
					 tag_ftm5);
				res = ERROR_OP_NOT_ALLOW;
		...
		case CMD_GETMSFRAME:
			if (numberParam == 2) {
				...
				msleep(WAIT_AFTER_SENSEOFF);
				 flushFIFO_ftm5(info); */ /* delete the events related to
				 * some touch (allow to call this function while
				 * touching the screen without having a flooding
				 * of the FIFO) */
				res = getMSFrame3_ftm5(info, (MSFrameType)cmd[1],
						  &frameMS);
				if (res < 0)
					logError_ftm5(0,
					 ...
					 * is
					  ...
					  */
					res = OK;
					print_frame_short_ftm5("MS frame =",
		...
		case CMD_GETSYNCFRAME:
			...
			if (numberParam == 2) {
				logError_ftm5(1, "%s Reading Sync Frame...\n", tag_ftm5);
				res = getSyncFrame(info, cmd[1], &frameMS, &frameSS);
				if (res < OK)
					logError_ftm5(0,
		...
		case CMD_REQCOMPDATA:	/* request comp data */
			if (numberParam == 2) {
				logError_ftm5(0,
					 ...
					 tag_ftm5);
				res = requestCompensationData_ftm5(info, cmd[1]);
			...
			if (numberParam == 2) {
				...
				if (res < OK)
					...
					logError_ftm5(0,
						 "%s Requesting Compensation Data Finished!\n",
						 tag_ftm5);
					res = readCompensationDataHeader_ftm5(info, 
		...
		case CMD_READMSCOMPDATA:/* read mutual comp data */
			if (numberParam == 2) {
				logError_ftm5(0, "%s Get MS Compensation Data\n",
					 tag_ftm5);
				res = readMutualSenseCompensationData_ftm5(info, cmd[1],
								      &compData);
		...
		case CMD_READSSCOMPDATA:
			if (numberParam == 2) {	/* read self comp data */
				logError_ftm5(0, "%s Get SS Compensation Data...\n",
					 tag_ftm5);
				res = readSelfSenseCompensationData_ftm5(info, cmd[1],
								    &comData);
				if (res < OK)
					logError_ftm5(0,
			...
			if (numberParam == 2) {
				...
				if (res < OK)
					...
					size = (totCompData.node_data_size *
						sizeof(short) + 9);
					print_frame_short_ftm5("MS Data (TOT Cx) =",
							  array1dTo2d_short_ftm5(
								  totCompData.
								  node_data,
								  totCompData.
		...
		case CMD_READTOTSSCOMPDATA:
			if (numberParam == 2) {	/* read self comp data */
				logError_ftm5(0,
					 ...
					 tag_ftm5);
				res = readTotSelfSenseCompensationData(info, cmd[1],
								&totComData);
				if (res < OK)
					logError_ftm5(0,
		...
		case CMD_FLASHUNLOCK:
			...
			if (res < OK)
				...
				logError_ftm5(0, "%s Flash Unlock OK!\n", tag_ftm5);
			break;

		case CMD_READFWFILE:
			if (numberParam == 2) {	/* read fw file */
				logError_ftm5(0, "%s Reading FW File...\n", tag_ftm5);
				res = readFwFile_ftm5(info, PATH_FILE_FW, &fw,
						 funcToTest[1]);
				if (res < OK)
					logError_ftm5(0,
		...
		case CMD_FLASHPROCEDURE:
			if (numberParam == 3) {	/* flashing procedure */
				logError_ftm5(0,
					 ...
					 tag_ftm5);
				res = flashProcedure_ftm5(info, PATH_FILE_FW, cmd[1],
						     cmd[2]);
				if (res < OK)
					logError_ftm5(0,
		...
		case CMD_FLASHERASEUNLOCK:
			...
			if (res < OK)
				...
				logError_ftm5(0, "%s Flash Erase Unlock Finished!\n",
					 tag_ftm5);
			break;
		...
		case CMD_FLASHERASEPAGE:
			if (numberParam == 2) {	/* need to pass: keep_cx */
				logError_ftm5(0,
					 ...
					 tag_ftm5);
				res = flash_erase_page_by_page_V2(info, cmd[1]);
				if (res < OK)
					logError_ftm5(0,
		...
		case CMD_INITTEST:
			if (numberParam == 2)
				res = production_test_initialization_ftm5(info, cmd[1]);
		...
		case CMD_MSRAWTEST:	/* MS Raw DATA TEST */
			if (numberParam == 2)	/* need to specify if stopOnFail
						 * */
				res = production_test_ms_raw_ftm5(info, LIMITS_FILE,
							     cmd[1], &info->tests_ftm5);
			else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
					 tag_ftm5);
				res = ERROR_OP_NOT_ALLOW;
		...
		case CMD_MSINITDATATEST:/* MS CX DATA TEST */
			if (numberParam == 2)	/* need stopOnFail */
				res = production_test_ms_cx_ftm5(info, LIMITS_FILE, cmd[1],
							    &info->tests_ftm5);
			else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
					 tag_ftm5);
		...
		case CMD_SSRAWTEST:	/* SS RAW DATA TEST */
			if (numberParam == 2) /* need stopOnFail */
				res = production_test_ss_raw_ftm5(info, LIMITS_FILE,
							     cmd[1], &info->tests_ftm5);
			else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
					 tag_ftm5);
		...
		case CMD_SSINITDATATEST:/* SS IX CX DATA TEST */
			if (numberParam == 2)	/* need stopOnFail */
				res = production_test_ss_ix_cx_ftm5(info, LIMITS_FILE,
							       cmd[1], &info->tests_ftm5);
			else {
				logError_ftm5(1, "%s Wrong number of parameters!\n",
					 tag_ftm5);
		...
		case CMD_MAINTEST:
			if (numberParam >= 3) {	/* need to specify if stopOnFail
						 ...
						 * mpflag(optional)*/
				if (numberParam == 3)
					res = production_test_main_ftm5(info, LIMITS_FILE,
							   cmd[1],
							   cmd[2], &info->tests_ftm5,
							   MP_FLAG_OTHERS,
							   &result);
				else
					res = production_test_main_ftm5(info, LIMITS_FILE,
		...
		case CMD_GETFWFILE:
			...
			if (numberParam >= 1) {
				if (numberParam == 1)
					res = getFWdata_ftm5(info, PATH_FILE_FW, &readData,
							&fileSize);
				else
					res = getFWdata_ftm5(info, path, &readData,
							&fileSize);
		...
		case CMD_DIAGNOSTIC:
			...
			res = ftm5_writeReadU8UX(info, FTS_CMD_HW_REG_R,
			...
			index += j;
			logError_ftm5(1, "%s Sensing On!\n", tag_ftm5);
			readData[0] = FTS_CMD_SCAN_MODE;
			readData[1] = SCAN_MODE_ACTIVE;
			readData[2] = 0x1;
			ftm5_write(info, readData, 3);
			res = checkEcho_ftm5(info, readData, 3);
			if (res < OK) {
				logError_ftm5(1,
			...
			logError_ftm5(1, "%s Reading Frames...!\n", tag_ftm5);
			j = snprintf(&info->driver_test_buff[index], fileSize - index,
				     "3) Read Frames:\n");
			index += j;
			for (temp = 0; temp < DIAGNOSTIC_NUM_FRAME; temp++) {
				logError_ftm5(1, "%s Iteration n. %d...\n", tag_ftm5,
					 temp + 1);
				j = snprintf(&info->driver_test_buff[index],
					     fileSize - index,
					     "Iteration n. %d...\n", temp +
				...
				for (addr = 0; addr < 3; addr++) {
					...
					case 0:
						j = snprintf(
							...
							"MS RAW FRAME =");
						index += j;
						res |= getMSFrame3_ftm5(info, MS_RAW,
								   &frameMS);
						break;
					case 2:
						j = snprintf(
							...
							"MS STRENGTH FRAME =");
						index += j;
						res |= getMSFrame3_ftm5(info, MS_STRENGTH,
								   &frameMS);
						break;
					case 1:
						j = snprintf(
							...
							"MS BASELINE FRAME =");
						index += j;
						res |= getMSFrame3_ftm5(info, MS_BASELINE,
								   &frameMS);
						break;
					}
					if (res < OK) {
				...
				for (addr = 0; addr < 3; addr++) {
					...
					case 2:
						j = snprintf(
							...
							"SS STRENGTH FRAME =\n");
						index += j;
						res |= getSSFrame3(info, SS_STRENGTH,
								   &frameSS);
						break;
					case 1:
						j = snprintf(
							...
							"SS BASELINE FRAME =\n");
						index += j;
						res |= getSSFrame3(info, SS_BASELINE,
								   &frameSS);
						break;
					}
					if (res < OK) {
			...
			logError_ftm5(1, "%s Reading error info...\n", tag_ftm5);
			j = snprintf(&info->driver_test_buff[index], fileSize - index,
				     "4) FW INFO DUMP: ");
			index += j;
			temp = dumpErrorInfo_V2(info, readData, ERROR_DUMP_ROW_SIZE *
					     ERROR_DUMP_COL_SIZE);
...
END_DIAGNOSTIC:
		...
		default:
			logError_ftm5(1, "%s COMMAND ID NOT VALID!!!\n", tag_ftm5);
			res = ERROR_OP_NOT_ALLOW;
			break;
		}

		/*res2 = fts_enableInterrupt_ftm5(info);
		 ...
		 * another the interrupt wan to be kept disabled
		  * if (res2 < 0) {
		  *      logError_ftm5(0, "%s stm_driver_test_show: ERROR %08X\n",
		  *tag_ftm5, (res2 | ERROR_ENABLE_INTER));
		  * }*/
	} else {
		logError_ftm5(1,
...
END:	/* here start the reporting phase, assembling the data to send in the
	...
	logError_ftm5(0, "%s Size = %d\n", tag_ftm5, size);
	info->driver_test_buff = (u8 *)kzalloc(size, GFP_KERNEL);
	logError_ftm5(0, "%s Finish to allocate memory!\n", tag_ftm5);
	if (info->driver_test_buff == NULL) {
		logError_ftm5(0,
	...
	if (byte_call == 0) {
		...
		if (res >= OK) {
			...
			case CMD_READCONFIG:
			case CMD_POLLFOREVENT:
				/* logError_ftm5(0, "%s Data = ", tag_ftm5); */
				if (info->mess.dummy == 1)
					j = 1;
				else
					j = 0;
				for (; j < byteToRead + info->mess.dummy; j++) {
					/* logError_ftm5(0, "%02X ", readData[j]); */
					snprintf(&info->driver_test_buff[index], 3,
						 "%02X", readData[j]);
			...
			case CMD_GETFWFILE:
			case CMD_GETLIMITSFILE:
				logError_ftm5(0, "%s Start To parse!\n", tag_ftm5);
				...
				logError_ftm5(0, "%s Finish to parse!\n", tag_ftm5);
				break;
		...
		info->driver_test_buff[index] = '\n';
		/*for(j=0; j<size; j++){
		  *      logError_ftm5(0,"%c", info->driver_test_buff[j]);
		  * }*/
		info->limit = size;
		info->printed = 0;
	}
ERROR:
	...
	/* logError_ftm5(0,"%s numberParameters = %d\n",tag_ftm5, numberParam); */
	if (readData != NULL)
		kfree(readData);
...
int fts_proc_init(struct fts_ts_info *info)
{
	...
	info->fts_dir = proc_mkdir_data("fts", 0777, NULL, NULL);
	...
	if (entry)
		logError_ftm5(1, "%s %s: proc entry CREATED!\n", tag_ftm5, __func__);
	else {
		logError_ftm5(1, "%s %s: error creating proc entry!\n", tag_ftm5,
			 __func__);
		retval = -ENOMEM;
		goto badfile;
	}
