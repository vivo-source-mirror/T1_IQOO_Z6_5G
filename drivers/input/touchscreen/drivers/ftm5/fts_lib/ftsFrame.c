...
/**
  ...
  */
int getChannelsLength_ftm5(struct fts_ts_info *info)
{
	int ret;
	u8 data[2];

	if (data == NULL) {
		logError_ftm5(1, "%s getChannelsLength_ftm5: ERROR %08X\n", tag_ftm5,
			 ERROR_ALLOC);
		return ERROR_ALLOC;
	}
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int getFrameData_ftm5(struct fts_ts_info *info, u16 address, int size, short *frame)
{
	int i, j, ret;
	u8 *data = (u8 *)kmalloc(size * sizeof(u8), GFP_KERNEL);

	if (data == NULL) {
		logError_ftm5(1, "%s getFrameData_ftm5: ERROR %08X\n", tag_ftm5, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
...
/**
  ...
  */
int getSenseLen_ftm5(struct fts_ts_info *info)
{
	if (info->systemInfo.u8_scrRxLen == 0)
		getChannelsLength_ftm5(info);
...
/**
  ...
  */
int getForceLen_ftm5(struct fts_ts_info *info)
{
...
/**
  ...
  */
int getMSFrame3_ftm5(struct fts_ts_info *info, MSFrameType type, MutualSenseFrame *frame)
{
	u16 offset;
	int ret, force_len, sense_len;

	force_len = getForceLen_ftm5(info);
	sense_len = getSenseLen_ftm5(info);

	frame->node_data = NULL;

	logError_ftm5(0, "%s %s: Starting to get frame %02X\n", tag_ftm5, __func__,
		 type);
	switch (type) {
	case MS_RAW:
...
LOAD_NORM:
		if (force_len == 0 || sense_len == 0) {
			logError_ftm5(1,
				 ...
				 tag_ftm5, __func__, ERROR_CH_LEN);
...
LOAD_FRC:
		...
		break;
	default:
		logError_ftm5(1, "%s %s: Invalid type ERROR %08X\n", tag_ftm5, __func__,
	...
	logError_ftm5(0, "%s %s: Force_len = %d Sense_len = %d Offset = %04X\n",
		 tag_ftm5, __func__, force_len, sense_len, offset);
	...
	ret = getFrameData_ftm5(info, offset, frame->node_data_size * BYTES_PER_NODE,
			   (frame->node_data));
	if (ret < OK) {
		logError_ftm5(1, "%s %s: ERROR %08X\n", tag_ftm5, __func__,
	...
	logError_ftm5(0, "%s Frame acquired!\n", tag_ftm5);
...
int getSSFrame3(struct fts_ts_info *info, SSFrameType type, SelfSenseFrame *frame)
{
	...
	frame->header.force_node = getForceLen_ftm5(info);	/* use getForce/SenseLen
							 * because introduce a
							 * recover mechanism in
							 * case of len =0 */
	frame->header.sense_node = getSenseLen_ftm5(info);
	...
	logError_ftm5(0,
		 ...
		 tag_ftm5, __func__, frame->header.force_node,
	...
	if (frame->force_data == NULL) {
		logError_ftm5(1, "%s %s: can not allocate force_data ERROR %08X\n",
			 tag_ftm5, __func__, ERROR_ALLOC | ERROR_GET_FRAME);
		return ERROR_ALLOC | ERROR_GET_FRAME;
	}
	...
	if (frame->sense_data == NULL) {
		...
		frame->force_data = NULL;
		logError_ftm5(1, "%s %s: can not allocate sense_data ERROR %08X\n",
			 tag_ftm5, __func__, ERROR_ALLOC | ERROR_GET_FRAME);
		return ERROR_ALLOC | ERROR_GET_FRAME;
	}

	ret = getFrameData_ftm5(info, offset_force, frame->header.force_node *
	...
	ret = getFrameData_ftm5(info, offset_sense, frame->header.sense_node *
		       ...
		       u64 *address)
{
	...
	logError_ftm5(0, "%s Type = %02X of SyncFrame data OK!\n", tag_ftm5, type);

	msHeader->force_node = data[5];
	msHeader->sense_node = data[6];
	logError_ftm5(0, "%s MS Frame force_node = %d, sense_node = %d\n", tag_ftm5,
		 msHeader->force_node, msHeader->sense_node);

	ssHeader->force_node = data[7];
	ssHeader->sense_node = data[8];
	logError_ftm5(0, "%s SS Frame force_node = %d, sense_node = %d\n", tag_ftm5,
...
/**
  ...
  */
int getSyncFrame(struct fts_ts_info *info, u8 type, MutualSenseFrame *msFrame, SelfSenseFrame *ssFrame)
{
	int res;
	u64 address;
	...
	logError_ftm5(0, "%s %s: Starting to get Sync Frame %02X...\n", tag_ftm5,
		 __func__, type);
	switch (type) {
	...
	logError_ftm5(0, "%s %s: Requesting Sync Frame %02X...\n", tag_ftm5, __func__,
		 type);
	...
	if (res < OK) {
		logError_ftm5(1,
			 ...
			 tag_ftm5, __func__, res | ERROR_GET_FRAME_DATA);
		return res | ERROR_GET_FRAME_DATA;
	}
	...
	logError_ftm5(0, "%s %s: Getting MS frame at %04llX...\n", tag_ftm5, __func__,
		 address);
	res = getFrameData_ftm5(info, address, (msFrame->node_data_size) * BYTES_PER_NODE,
			   (msFrame->node_data));
	if (res < OK) {
		logError_ftm5(1, "%s %s: error while getting MS data...ERROR %08X\n",
	...
	logError_ftm5(0, "%s %s: Getting SS force frame at %04llX...\n", tag_ftm5, __func__,
		 address);
	res = getFrameData_ftm5(info, address, (ssFrame->header.force_node) *
	...
	logError_ftm5(0, "%s %s: Getting SS sense frame at %04llX...\n", tag_ftm5, __func__,
		 address);
	res = getFrameData_ftm5(info, address, (ssFrame->header.sense_node) *
...
ERROR:
	if (res < OK) {
		...
		logError_ftm5(0, "Getting Sync Frame FAILED! ERROR %08X!\n", res);
	} else {
		logError_ftm5(0, "Getting Sync Frame FINISHED!\n");
