		...
		if (MT_B_TYPE) {
			...
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
				if (ilits->curt_touch[i] == 0 && ilits->prev_touch[i] == 1) {
					vts_report_point_up(vtsdev, i, ilits->finger, touch_info[i].x, touch_info[i].y,
...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	static u16 ili_coordinate_x[6]; 
	static u16 ili_coordinate_y[6]; 
	...
	if (gc->type) {
    	...
    	memset(ili_coordinate_x, 0, 6 * sizeof(u16));
    	memset(ili_coordinate_y, 0, 6 * sizeof(u16)); 
