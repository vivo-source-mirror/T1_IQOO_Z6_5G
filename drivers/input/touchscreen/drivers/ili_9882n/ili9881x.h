...
#define DRIVER_VERSION			"3.0.3.1.200610"
...
#define CSV_LCM_ON_PATH			"/data/vendor/touchscreen/com.touchscreen.tptest/sensor_test"
...
#define UPDATE_REQUEST_FW_PATH		"ILITEK_FW"
...
#define ILI_INFO(fmt, param...)	vts_dev_info(NULL, fmt, ##param)		

#define ILI_ERR(fmt, param...)	vts_dev_err(NULL, fmt, ##param)

extern bool debug_en;
#define ILI_DBG(fmt, param...)	vts_dev_dbg(NULL, fmt, ##param)
...
#define P5_X_PALM_PACKET_ID			0xBB
...
/*  add_for_charger_end  */
	...
	/* members declaired for vivo */
	int vivo_fw_size;
	u8 vivo_fw_data[512*K];
	bool vivo_debug;
	bool vivo_ges_debug;
	u8 vivo_ges_data[P5_X_GESTURE_INFO_LENGTH + 1];

	int vivo_gamemode;
	int vivo_charger;
	int vivo_EdgeRestain;
...
extern void ili_report_ap_mode(u8 *buf, int len, ktime_t kt);
extern void ili_report_debug_mode(u8 *buf, int rlen, ktime_t kt);
extern void ili_report_debug_lite_mode(u8 *buf, int rlen, ktime_t kt);
...
extern int ili_report_handler(ktime_t kt);
...
int ilitek_debug_node_buff_control(bool open);

/* Prototypes for ViVO */
extern int SetFwState(void);
extern int bbk_ili_get_rawordiff_data_v2_9882n(struct vts_device *vtsdev, enum vts_frame_type which, int *data);
extern int bbk_ili_get_rawordiff_data_v2(struct vts_device *vtsdev, enum vts_frame_type which, short *data, int size);
extern int bbk_ili_fw_update_v2_9882n(struct vts_device *vtsdev, const struct firmware *fw);
extern int idleEnableOrDisable_v2_9882n(struct vts_device *vtsdev, int state);
extern int bbk_ili_readUdd_v2_9882n(unsigned char *udd);
extern int bbk_ili_writeUdd_v2_9882n(unsigned char *udd);
extern int bbk_ili_mode_change_v2_9882n(struct vts_device *vtsdev, int which);
extern int bbk_ili_get_fw_version_v2_9882n(int which);
extern int bbk_ili_get_fw_version_v2(struct vts_device *vtsdev, u64 *version);
extern int bbk_ili_gesture_point_get_v2_9882n(u16 *data);
extern int bbk_ili_set_charger_bit_v2_9882n(struct vts_device *vtsdev, int state);
extern int bbk_ili_read_charger_bit_v2_9882n(void);
extern int setEdgeRestainSwitch_v2_9882n(struct vts_device *vtsdev, int on);
extern int bbk_ili_get_header_file_version_v2_9882n(int which, unsigned char *fw);
extern int bbk_ili_get_fw_debug_info_v2_9882n(unsigned char *buf);
extern int bbk_ili_read_ic_mode(struct vts_device *vtsdev);
