...
static ssize_t ilitek_node_mp_lcm_on_test_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	memset(apk_ret, 2 ,sizeof(apk_ret));
	...
	ilits->mpstatus = true;
	ret = ili_mp_test_handler(apk_ret, ON);
	ILI_INFO("MP TEST %s, Error code = %d\n", (ret < 0) ? "FAIL" : "PASS", ret);
	ilits->mpstatus = false;
...
static ssize_t ilitek_proc_vivo_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strcmp(cmd, "rawordiff") == 0) {
		...
		ret = bbk_ili_get_rawordiff_data_v2_9882n(vtsdev, data[1], &debug_v2[0]);
		ILI_INFO("Get RAW/Diff data %s\n", (ret < 0) ? "FAIL" : "DONE");
		ret = debug_en;
		debug_en = DEBUG_ALL;
		ili_dump_data(&debug_v2[0], 32, 32 * 18, 0, "vivo debug");
		debug_en = ret;
	} else if (strcmp(cmd, "fwupdate") == 0) {
		if (request_firmware(&fw, UPDATE_REQUEST_FW_PATH, ilits->dev) < 0) {
			ILI_ERR("Rquest firmware failed\n");
			goto out;
		}
		mutex_lock(&ilits->touch_mutex);
		ret = bbk_ili_fw_update_v2_9882n(vtsdev, fw);
		mutex_unlock(&ilits->touch_mutex);
		ILI_INFO("fw upgrade %s\n", (ret < 0) ? "FAIL" : "DONE");
		release_firmware(fw);
	} else if (strcmp(cmd, "idlectrl") == 0) {
		ret = idleEnableOrDisable_v2_9882n(vtsdev, data[1]);
		ILI_INFO("set idle %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "readudd") == 0) {
		bbk_ili_readUdd_v2_9882n(NULL);
	} else if (strcmp(cmd, "writeudd") == 0) {
		bbk_ili_writeUdd_v2_9882n(NULL);
	} else if (strcmp(cmd, "modechange") == 0) {
		bbk_ili_mode_change_v2_9882n(vtsdev, data[1]);
		ILI_INFO("switch mode %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "getfwver") == 0) {
		ret = bbk_ili_get_fw_version_v2_9882n(data[1]);
		ILI_INFO("get fw version %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "getgesture") == 0) {
		ret = bbk_ili_gesture_point_get_v2_9882n(gesture_v2);
		ILI_INFO("Get gesture len (%d), %s\n", ret, (ret < 0) ? "FAIL" : "DONE");
		ret = debug_en;
		debug_en = DEBUG_ALL;
		ili_dump_data(gesture_v2, 8, sizeof(gesture_v2), 0, "vivo gesture after");
		debug_en = ret;
	} else if (strcmp(cmd, "setcharger") == 0) {
		ret = bbk_ili_set_charger_bit_v2_9882n(vtsdev, data[1]);
		ILI_INFO("set charger %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "readcharger") == 0) {
		bbk_ili_read_charger_bit_v2_9882n();
	} else if (strcmp(cmd, "setedge") == 0) {
		ret = setEdgeRestainSwitch_v2_9882n(vtsdev, data[1]);
		ILI_INFO("set edge palm %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "getfwdebuginfo") == 0) {
		unsigned char *str = NULL;
		str = kzalloc(PAGE_SIZE, GFP_KERNEL);
		ret = bbk_ili_get_fw_debug_info_v2_9882n(str);
		ILI_INFO("return written length = %d\n", ret);
		ILI_INFO("%s", str);
	} else {
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ILITEK_IOCTL_TP_MODE_CTRL:
		...
		if(szBuf[0] == TEST_MODE) {
			ILI_INFO("ioctl: Switch TEST mode\n");
			ilits->mpstatus = true;
		}else {
			ilits->mpstatus = false;	
			ILI_INFO("ioctl: Switch other mode\n");
		}
		break;
...
struct file_operations proc_vivo_mp_test_fops_v2 = {
	.read = ilitek_node_mp_lcm_on_test_read,
};
...
proc_node iliproc[] = {
	...
	{"mp_test", NULL, &proc_mp_lcm_on_test_fops, false},
...
void ili_node_init(void)
{
	...
	if (nRetVal) {
		ILI_ERR("Failed to create ioctl node\n");
	}
