...
int ili_fw_upgrade_handler(void *data)
{
	...
	ILI_INFO("ili_fw_upgrade_handler start to run !");
...
int ili_report_handler(ktime_t kt)
{
	int ret = 0, pid = 0;
	...
	if (pid == P5_X_PALM_PACKET_ID) {
		ILI_INFO("P5_X_PALM_PACKET_ID key event");
		vts_report_event_down(ilits->vtsdev, VTS_EVENT_GESTURE_LARGE_AREA_PRESS);
		vts_report_event_up(ilits->vtsdev, VTS_EVENT_GESTURE_LARGE_AREA_PRESS);
	}
	...
	case P5_X_DEMO_PACKET_ID:
		ili_report_ap_mode(trdata, rlen, kt);
		break;
	case P5_X_DEBUG_PACKET_ID:
		ili_report_debug_mode(trdata, rlen, kt);
		break;
	case P5_X_DEBUG_LITE_PACKET_ID:
		ili_report_debug_lite_mode(trdata, rlen, kt);
		break;
...
//#if (BOOT_FW_UPDATE | HOST_DOWN_LOAD)
	...
	ilits->vivo_EdgeRestain = 1;
	ilits->vivo_charger = 0;
	ilits->vivo_gamemode = 0;
...
#else
	ilits->ws = (struct wakeup_source *)kzalloc(sizeof(struct wakeup_source), GFP_KERNEL);
	if (!ilits->ws) {
		ILI_ERR("wakeup source request failed\n");
	} else {
		wakeup_source_init(ilits->ws, "ili_wakelock");
	}
#endif
