...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	case REQUEST_FIRMWARE:
		VTI("---------------------REQUEST_FIRMWARE");
		fw_get.data = vts_fw_data_get(ilits->vtsdev, VTS_FW_TYPE_FW, &fsize);
		if (!fw_get.data) {
			ILI_ERR("fail to get firmware !");
			return EINVAL;
		}
		fw_get.size = fsize;
		ILI_INFO("success to get vts firmware !");
	...
	case FILP_OPEN:
		...
		fw_get.size = 0;
		fw_get.data = vmalloc(fsize);
		if (ERR_ALLOC_MEM(fw_get.data)) {
		...
		set_fs(KERNEL_DS);
		pos = 0;
		vfs_read(f, (u8 *)fw_get.data, fsize, &pos);
		set_fs(old_fs);
#else
		pos = 0;
#endif
	...
	if (ERR_ALLOC_MEM(fw_get.data) || fw_get.size <= 0) {
	...
	if (ilitek_tddi_fw_hex_convert((u8 *)fw_get.data, fw_get.size, pfw) < 0) {
...
{
	int i, ret = 0, retry = 3;
	VTI("ili_fw_upgrade value is %d  %d  %d", ilits->boot, ilits->force_fw_update, ERR_ALLOC_MEM(pfw));
