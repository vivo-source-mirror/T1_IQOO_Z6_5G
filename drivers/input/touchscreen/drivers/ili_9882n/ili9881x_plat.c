...
void ili_tp_reset(void)
{
	...
	if (gpio_is_valid(ilits->tp_rst)) {
		...
		ILI_ERR("reset gpio is not valid !");
	}
}
...
static int ilitek_plat_gpio_register(void)
{
	...
	struct device_node *dev_node = ilits->node;

	ilits->pinctrl = devm_pinctrl_get(ilits->dev);
	if (IS_ERR_OR_NULL(ilits->pinctrl)) {
		ILI_ERR("fail to get pinctrl !!!");
	} else {
		ilits->pinctrl_sleep = pinctrl_lookup_state(ilits->pinctrl, "cs_sleep");
		if (IS_ERR_OR_NULL(ilits->pinctrl_sleep)) {
			ILI_ERR("fail to get pinctrl_sleep !!!");
		}
	}
	...
	ilits->tp_cs = of_get_named_gpio_flags(dev_node, "touch,cs-gpio", 0, &flag);
	...
	ret = gpio_request(ilits->tp_cs, "TP_CS");
	if (ret < 0) {
		ILI_ERR("Request CS GPIO failed, ret = %d\n", ret);
		gpio_free(ilits->tp_cs);
		ret = gpio_request(ilits->tp_rst, "TP_CS");
		if (ret < 0) {
			ILI_ERR("Retrying request CS GPIO still failed , ret = %d\n", ret);
			goto out;
		}
	}
...
static void ilitek_plat_isr_top_half(void)
{
...
static irqreturn_t ilitek_irq_work_func(int irq, void *handle, ktime_t kt)
{
...
int ili_irq_register(int type)
{
	...
	ret = vts_interrupt_register(ilits->vtsdev, ilits->irq_num, ilitek_irq_work_func, type, ilits);
...
static void ilitek_update_charger(struct work_struct *work)
{
	int ret = 0;
	mutex_lock(&ilits->touch_mutex);
	ret = ili_ic_func_ctrl("plug", !ilits->usb_plug_status);/*lug in*/
	if (ret < 0) {
...
static int ilitek_plat_init(struct vts_device *vtsdev)
{
...
static const struct vts_operations ili9882_vts_ops = {
	.init = ilitek_plat_init,
	.update_firmware = bbk_ili_fw_update_v2_9882n,
	.change_mode = bbk_ili_mode_change_v2_9882n,
	.get_frame = bbk_ili_get_rawordiff_data_v2,
	.get_fw_version = bbk_ili_get_fw_version_v2,
	.set_charging = bbk_ili_set_charger_bit_v2_9882n,
	.set_rotation = setEdgeRestainSwitch_v2_9882n,
	.set_auto_idle = idleEnableOrDisable_v2_9882n,
	.get_ic_mode = bbk_ili_read_ic_mode,
};

int vts_ilitek_setup (void)
{
	struct vts_device *vtsdev;
	int ret = 0;
	vtsdev = vts_device_alloc();
	if(!vtsdev) {
		ILI_INFO("illitek vivoTsAlloc fail");
		return -ENOMEM;
	}
	vtsdev->ops = &ili9882_vts_ops;
	vts_parse_dt_property(vtsdev, ilits->node);
	vts_set_drvdata(vtsdev, ilits);
	ret = vts_register_driver(vtsdev);
	if (ret < 0) {
		ILI_ERR("fail to register vts driver !");
		return ret;
	}
