...
#define LOGD(dev, log, ...) 	do {	\
									VTD(log, ##__VA_ARGS__);		/*LOGx(dev_dbg, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
#define LOGI(dev, log, ...) 	do {	\
									VTI(log, ##__VA_ARGS__);		/*LOGx(dev_info, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
#define LOGN(dev, log, ...) 	do {	\
									VTI(log, ##__VA_ARGS__);		/*LOGx(dev_notice, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
#define LOGW(dev, log, ...) 	do {	\
									VTI(log, ##__VA_ARGS__);		/*LOGy(dev_warn, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
#define LOGE(dev, log, ...) 	do {	\
									VTE(log, ##__VA_ARGS__);		/*LOGy(dev_err, dev, log, ##__VA_ARGS__)*/\
								} while ((dev) == NULL)
...
enum dynamic_config_id {
	...
	DC_ENABLE_GLOVE,
	DC_ROTATE_OR_HORIZONTAL = 0xdc,
	DC_DAKZONE_ENABLE_BIT = 0xdd,
	DC_FINGERPRINT_ENABLE = 0xd4,
	DC_HIGH_REPORT_RATE = 0xd5,
	DC_FD_DISABLE = 0xFD,
	DC_GESTURE_MASK = 0xfe,
	DC_AOD_DOUBLE_TAP_X=200,
	DC_AOD_DOUBLE_TAP_Y=201,
	DC_AOD_DOUBLE_TAP_W=202,
	DC_AOD_DOUBLE_TAP_H=203,
};
...
struct syna_tcm_hcd {
	...
	struct syna_tcm_module_pool mod_pool;
	...
	int (*power_on)(struct syna_tcm_hcd *tcm_hcd);
	int (*power_off)(struct syna_tcm_hcd *tcm_hcd);
	int (*set_pinctrl)(struct syna_tcm_hcd *tcm_hcd, bool state);
};
...
/* to support vivo interface + */
int reflash_wrapper_run_fw_update(const unsigned char *fw_bin,
		unsigned int fw_bin_size);
int reflash_wrapper_read_oem_data(unsigned char *output_data,
		unsigned int buffer_len);
int reflash_wrapper_write_oem_data(const unsigned char *data,
