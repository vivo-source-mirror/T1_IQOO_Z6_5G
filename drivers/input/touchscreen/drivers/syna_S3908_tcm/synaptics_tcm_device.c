...
DECLARE_COMPLETION(device_remove_complete_v2);
...
if (0) {
	if (count && device_hcd->out.buf[0] == CMD_SET_TOUCH_REPORT_CONFIG) {
		int i = 0;
		for(i=0;i < count; i++) {
			LOGE(tcm_hcd->pdev->dev.parent, "device_hcd->out.buf[%d] = %d \n",i, device_hcd->out.buf[i]);
		}
...
exit:
	complete(&device_remove_complete_v2);
...
{
	return syna_tcm_add_module_v2(&device_module, true);
}
...
void device_module_exit(void)
{
	if (device_hcd == NULL)
		goto remove;
	if (device_hcd->report.buf) {
		kfree(device_hcd->report.buf);
		device_hcd->report.buf = NULL;
	}
	if (device_hcd->tcm_hcd->config.buf) {
		kfree(device_hcd->tcm_hcd->config.buf);
		device_hcd->tcm_hcd->config.buf = NULL;
	}

remove:
	syna_tcm_add_module_v2(&device_module, false);
	wait_for_completion(&device_remove_complete_v2);
