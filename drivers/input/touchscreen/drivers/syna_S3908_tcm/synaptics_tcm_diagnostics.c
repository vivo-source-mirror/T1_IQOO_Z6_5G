...
DECLARE_COMPLETION(diag_remove_complete_v2);
...
exit:
	complete(&diag_remove_complete_v2);
...
{
	return syna_tcm_add_module_v2(&diag_module, true);
}

void diag_module_exit(void)
{
	syna_tcm_add_module_v2(&diag_module, false);

	wait_for_completion(&diag_remove_complete_v2);
