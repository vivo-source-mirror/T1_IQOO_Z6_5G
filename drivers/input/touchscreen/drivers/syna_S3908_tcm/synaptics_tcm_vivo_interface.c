...
//static ssize_t bbk_xxx_readUdd(struct vts_device *vts_data, u8 *touch_data, size_t nbytes);
//static ssize_t bbk_xxx_writeUdd(struct vts_device *vts_data, u8 *touch_data, size_t nbytes);
...
static ssize_t bbk_xxx_readUdd(struct vts_device *vts_data, u8 *touch_data, size_t nbytes)
{
	...
	retval = reflash_wrapper_read_oem_data(touch_data, UDD_DATA_SIZE);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static ssize_t bbk_xxx_writeUdd(struct vts_device *vts_data, u8 *touch_data, size_t nbytes)
{
	...
	retval = reflash_wrapper_write_oem_data(touch_data, UDD_DATA_SIZE);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
