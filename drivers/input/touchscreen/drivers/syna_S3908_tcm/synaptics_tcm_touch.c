	...
	while (idx < config_size) {
		...
		case TOUCH_GESTURE_COORDINATE:
			...
			if (touch_data->gesture_report == 0x63) {
				gesture_x[2] = gesture_x[3];
				gesture_y[2] = gesture_y[3];
				for (i = 3; i <= 6; i++) {
					gesture_x[i] = 0xffff;
					gesture_y[i] = 0xffff;
				}
...
static void touch_report(void)
{
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		if (status == FINGER || status == GLOVED_FINGER) {
	...
	if (touch_data->gesture_report == 0x80) {
		...
		if (VTS_ST_NORMAL == tcm_hcd->switch_to_mode) {
			/*for finger gesture info start*/

			finger_gesture_info[0] = x;
			finger_gesture_info[1] = y;

			/*for finger gesture info end*/
		}
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		z = pre_data[idx].z;
		vts_report_point_up(vtsdev, idx, touch_count, x, y, z, z, touch_data->palm_detected, ktime_get());
#endif
			break;
		case FINGER:
		case GLOVED_FINGER:
...
#else
		touch_count++;
		vts_report_point_down(vtsdev, idx, touch_count, x, y, z, z, touch_data->palm_detected, NULL, 0, ktime_get());
#endif
			break;
		default:
			break;
