...
int bbk_goodix_gesture_point_get_V2(struct goodix_ts_core *core_data, u16 *data)
{
	...
	for (i = 0; i < ges_num; i++){
		...
		covert_point_pixel(vtsdev, temp_x, temp_y, &coordinate_x[i], &coordinate_y[i], 0);
...
int bbk_goodix_set_charger_bit_V2(struct vts_device *vtsdev, int state)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if ((1 == state) && (0x02 == (read_val & 0x02))) {
			VTI("success in charger mode");
			break;
		} else if ((0 == state) && (0x00 == (read_val & 0x02))) {
...
/*
	msleep(40);
	if (goodix_i2c_read_V2(ts_dev, BBK_CMD_ADDR, &read_val, 1))
		VTE("Read cmd reg fail.");
	else
		VTI("Read cmd reg value:%02x.", read_val);
*/
