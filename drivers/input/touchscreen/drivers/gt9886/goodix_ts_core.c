...
static int goodix_faceDetect_startEnd(struct vts_device *vtsdev, int cmd)
{
	...
	if (cmd == 1) {
		/*
		ret = bbk_goodix_set_auto_idle_V2(0);
		if (ret < 0) {
			VTI("disable idle failed!");
			goto END;
		}
		*/
		...
		mdelay(10);
		/*
		ret = bbk_goodix_set_auto_idle_V2(1);
		if (ret < 0) {
			VTI("enable idle failed!");
			goto END;
		}
		*/
		core_data->face_detect_cmd = cmd;
...
static int bbk_goodix_finger_cmd_check(struct goodix_ts_device *ts_dev,int enable)
{   
    ...
    for (ret = 0; ret < 3;ret++){
		 ...
		 VTI("enable = %d,read buf = %d",enable,buf);
		 
		 if(buf == 0x13 && 1 == enable)
			break;
		 if(buf == 0x12 && 0 == enable)
			break;
...
static int bbk_goodix_long_press_enable(struct vts_device *vtsdev, int enable) {
	...
	if(fp_feedback)
	     bbk_goodix_finger_cmd_check(ts_dev,enable);
	return ret;
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	vts_parse_dt_property(vtsdev, ts_device->dev->of_node);
