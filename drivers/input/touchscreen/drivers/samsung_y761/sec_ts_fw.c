						...
						size_t size, int bl_update, int restore_cal, int retry)
{
	...
	if (!bl_update) {
		...
		if (retry_time < 3) {
			...
			if ((ts->ic_info[6] != tBuff[2]) || (ts->ic_info[7] != tBuff[3])) {
...
extern int bbk_slsi_erase_vivo_specail_cal(u8 val);
extern int bbk_slsi_get_vivo_calibration_status(int *);
extern int bbk_slsi_get_vivo_special_calibration_status(int *);
extern int bbk_slsi_start_force_calibration(void);
extern int bbk_slsi_get_vsync_freq(int *);
...
int sec_ts_get_special_cal_status_on_probe(struct vts_device *vtsdev)
{
	...
	if (calibration_twice_set) {
		...
		ret = bbk_slsi_get_vivo_special_calibration_status(&cali_special);
		if (ret < 0) {
	...
	*result = VTS_SENSOR_CALIBERATE_FAILED;
	...
	if (code == 165 || code == 167) {
		...
		if (calibration_twice_set == 1) {
			ret = bbk_slsi_get_vsync_freq(&vsync_freq);
			if (ret < 0) {
	    ...
	    /*show tag*/
		ret = bbk_slsi_get_vivo_special_calibration_status(&cali_satus);
		if (ret < 0) {
			VTI("read tag fail");
			return -EPERM;
		}
		...
		/*erase tag*/
		VTI("erase cali tag");
		ret = bbk_slsi_erase_vivo_specail_cal(0);
		msleep(200);
		if (ret < 0) {
			VTI("erase tag fail");
			return -EPERM;
		...
		if (ret < 0) {
			VTE("cali fail");
			msleep(200);
			bbk_slsi_erase_vivo_specail_cal(0x00);
			msleep(200);
			return -EPERM;
		} else {
			msleep(200);
			bbk_slsi_erase_vivo_specail_cal(0xA1);
		}
