...
int bbk_slsi_erase_vivo_specail_cal(u8 val)
{
	int ret = 0;
	struct sec_ts_data *ts = g_ts_data;
	u8 tBuff[1] = { 0x00 };
...
int bbk_slsi_get_vsync_freq(int *vsync_freq)
{
	int ret = 0;
	struct sec_ts_data *ts = g_ts_data;
...
int bbk_slsi_get_vivo_special_calibration_status(int *cali_satus)
{
	int ret = 0;
	struct sec_ts_data *ts = g_ts_data;
	u8 vivo_cal_status[1] = {0};
...
static int sec_get_vivo_special_calibration_status(struct vts_device *vtsdev)
{
	...
	ret = bbk_slsi_get_vivo_special_calibration_status(&cali_status);
...
static void sec_ts_read_event(struct sec_ts_data *ts, ktime_t kt)
{
	...
	int pre_ttype = 0;
	u16 double_click_x, double_click_y = 0;
	...
	do {
				...
				if ((ts->coord[t_id].ttype == SEC_TS_TOUCHTYPE_NORMAL)
					...
					if (ts->coord[t_id].action == SEC_TS_COORDINATE_ACTION_RELEASE) {
						...
						vts_report_point_up(vtsdev, ts->coord[t_id].id, ts->touch_count,  ts->coord[t_id].x, ts->coord[t_id].y,
							ts->coord[t_id].major,ts->coord[t_id].major, 0, kt);
						if (ts->touch_count == 0) {
							need_release = 1;
							ts->check_multi = 0;
						}
						...
						getnstimeofday(&ts->time_pressed[t_id]);
#else
						...
						vts_report_point_down(vtsdev, ts->coord[t_id].id, ts->touch_count,  ts->coord[t_id].x, ts->coord[t_id].y,
							ts->coord[t_id].major,ts->coord[t_id].major, 0, NULL, 0, kt);
		...
		case SEC_TS_GESTURE_EVENT:
			...
			case TOUCH_GESTURE_DOUBLTAP:
				double_click_x = ((p_gesture_status->gesture_data_2 | ((u16)(p_gesture_status->gesture_data_4 & 0xf0) << 4)) *  display_max_x) / touch_max_x;
				double_click_y = ((p_gesture_status->gesture_data_3 | ((u16)(p_gesture_status->gesture_data_4 & 0x0f) << 8)) * display_max_y) / touch_max_y;
...
int bbk_slsi_set_charger_bit(struct vts_device *vtsdev, int state)
{
	...
	if(state) {
		ret = sec_ts_set_charger(ts, true);
	} else {
		ret = sec_ts_set_charger(ts, false);
	}
...
int bbk_slsi_gesture_point_handle(struct vts_device *vtsdev, u8 id)
{
	...
	for (i = 0; i < report_num; i++) {
		gesture_point_x[i] = (((buff[3 * i] << 4) | (buff[3 * i + 2] >> 4)) * display_max_x) / touch_max_x;
		gesture_point_y[i] = (((buff[3 * i + 1] << 4) | (buff[3 * i + 2] & 0x0f)) * display_max_y) / touch_max_y;
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
			getnstimeofday(&ts->time_released[i]);
#else
...
static void sec_ts_reset_work(struct work_struct *work)
{
	...
	if (last_state == VTS_ST_NORMAL) {
		VTI("stay in normal mode, do nothing");
	} else if (last_state == VTS_ST_GESTURE) {
		ret = sec_ts_set_lowpowermode(ts, TO_LOWPOWER_MODE);
		if (ret < 0) {
			VTE("%s: failed to reset, ret:%d\n", __func__, ret);
			ts->reset_is_on_going = false;
