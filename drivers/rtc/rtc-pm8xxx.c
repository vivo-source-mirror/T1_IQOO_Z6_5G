...
static int pm8xxx_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	dev_err(dev, "%s|%d|%d Alarm Set for h:m:s=%ptRt, y-m-d=%ptRdr, enable=%d\n",
		current->comm, current->pid, current->tgid,
		&alarm->time, &alarm->time, alarm->enabled);
	{
		struct rtc_time rtc_tm;

		pm8xxx_rtc_read_time(dev, &rtc_tm);
		dev_err(dev, "Current rtc h:m:s=%%ptRt, y-m-d=%ptRdr\n",
				&rtc_tm, &rtc_tm);
	}
rtc_rw_fail:
	spin_unlock_irqrestore(&rtc_dd->ctrl_reg_lock, irq_flags);
...
static int pm8xxx_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	dev_err(dev, "%s|%d|%d Alarm read for - h:m:s=%ptRt, y-m-d=%ptRdr (%d)\n",
		current->comm, current->pid, current->tgid,
		&alarm->time, &alarm->time, !!(ctrl_reg & regs->alarm_en));
