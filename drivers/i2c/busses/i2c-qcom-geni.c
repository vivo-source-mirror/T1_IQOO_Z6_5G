...
/* vivo_chg add tart */
extern int ex_fg_clk_gpio;
extern int ex_fg_sda_gpio;

int msm_get_i2c_gpio_value(struct i2c_adapter *adap)
{
	u8 clk_gpio_value = 0;
	u8 data_gpio_value = 0;
	u8 value = 0;
	struct geni_i2c_dev *gi2c = i2c_get_adapdata(adap);

	if (!gi2c) {
		pr_err("msm_i2c_gpio_pull_down !ctrl\n");
		return 0;
	}
	if (!gi2c->clk_gpio || !gi2c->data_gpio) {
		pr_err("i2c gpio not configed\n");
		return 0;
	}
	if (strcmp(dev_name(gi2c->dev), "4c90000.i2c")) {
		pr_err("i2c bus error\n");
		return 0;
	}
	pinctrl_select_state(gi2c->i2c_rsc.geni_pinctrl,
				gi2c->i2c_rsc.geni_gpio_sleep);
	msleep(100);
	gpio_direction_input(gi2c->clk_gpio);
	gpio_direction_input(gi2c->data_gpio);
	msleep(10);
	clk_gpio_value = gpio_get_value(gi2c->clk_gpio);
	data_gpio_value = gpio_get_value(gi2c->data_gpio);

	ex_fg_clk_gpio = clk_gpio_value;
	ex_fg_sda_gpio = data_gpio_value;

	value = clk_gpio_value ^ data_gpio_value;
	pr_err("clk_gpio_value=%d,data_gpio_value=%d,value=%d\n", clk_gpio_value, data_gpio_value, value);

	return value;
}
EXPORT_SYMBOL_GPL(msm_get_i2c_gpio_value);
int msm_i2c_gpio_pull_down(struct i2c_adapter *adap)
{
#if 0
	struct geni_i2c_dev *gi2c = i2c_get_adapdata(adap);
	...
	if (gi2c->pwr_state == I2C_MSM_PM_SYS_SUSPENDED) {
		dev_err(ctrl->dev,"msm_i2c_gpio_pull_down is calling when system is suspended\n");
		return 0;
	}

	if (!gi2c->clk_gpio || !ctrl->data_gpio) {
		pr_err("i2c gpio not configed\n");
		return 0;
	}

	disable_irq(gi2c->rsrcs.irq);

	gi2c->gpio_mode = true;
	msm_i2c_gpio_config(gi2c, 0);

	gpio_direction_output(gi2c->clk_gpio, 0);
	gpio_direction_output(gi2c->data_gpio, 0);
	msleep(2500);

	msm_i2c_gpio_config(gi2c, 1);
	gi2c->gpio_mode = false;

	pr_warn("Bus busy cleared after 2.5s pull down\n");
	enable_irq(gi2c->rsrcs.irq);
	return 0;
#else
	struct geni_i2c_dev *gi2c = i2c_get_adapdata(adap);
	...
	pr_err("Bus busy cleared after 2.5s pull down\n");
	return 0;
#endif
}
EXPORT_SYMBOL_GPL(msm_i2c_gpio_pull_down);

int msm_i2c_14_active(struct i2c_adapter *adap){
	int ret;
	struct geni_i2c_dev *gi2c = i2c_get_adapdata(adap);
	...
	if (ret) {
		pr_err("msm_i2c_14_active fail\n");
		return ret;
	}
	msleep(100);
	pr_err("msm_i2c_14_active ok\n");
	return 0;
}
EXPORT_SYMBOL_GPL(msm_i2c_14_active);
...
static int geni_i2c_probe(struct platform_device *pdev)
{
	...
	/* vivo_chg add start */
	if (!strcmp(dev_name(gi2c->dev), "4c90000.i2c")) {
		gi2c->clk_gpio = of_get_named_gpio(pdev->dev.of_node, "vivo,clk-gpio", 0);
		if (gi2c->clk_gpio < 0) {
			pr_err("failed to get clk-gpio.\n");
			gi2c->clk_gpio = 0;
		}
		gi2c->data_gpio = of_get_named_gpio(pdev->dev.of_node, "vivo,data-gpio", 0);
		if (gi2c->data_gpio < 0) {
			pr_err("failed to get data-gpio.\n");
			gi2c->data_gpio = 0;
		}
		if (gi2c->clk_gpio > 0 && gi2c->data_gpio > 0) {
			ret = gpio_request(gi2c->clk_gpio,"clk_gpio");
			if (ret) {
				pr_err("failed to request clk_gpio,rc=%d\n",ret);
			}
			ret = gpio_request(gi2c->data_gpio,"data_gpio");
			if (ret) {
				pr_err("failed to request data_gpio,rc=%d\n",ret);
			}
		}
		pr_err("clk_gpio=%d,data_gpio=%d\n", gi2c->clk_gpio, gi2c->data_gpio);
	}
	/* vivo add end */
