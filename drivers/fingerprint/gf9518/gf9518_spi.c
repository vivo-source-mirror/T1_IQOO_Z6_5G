...
static void spi_clock_set(struct gf_dev *gf_dev, int speed)
{
	...
	if (rate < 0) {
		pr_err("%s, no match found for requested clock frequency = %d", __func__, speed);
		return;
	}
...
#else
		pr_debug("%s, Doesn't support control clock", __func__);
#endif
		break;
...
static int gf_release(struct inode *inode, struct file *filp)
{
	...
	if (!gf_dev->users) {
		...
		pr_info("%s, disble_irq, irq = %d", __func__, gf_dev->irq);
		gf_disable_irq(gf_dev);
...
#endif
{
	...
	status = gf9518_power_on(gf_dev);
	if (status) {
		pr_err("%s, failed to enable power", __func__);
		goto error_hw;
	}
	...
	if (status) {
		pr_err("%s, failed to request IRQ = %d", __func__, gf_dev->irq);
		goto err_irq;
	}
...
#if IS_ENABLED(CONFIG_BBK_FINGERPRINT)
	if ((get_fp_id() != GOODIX_GF9518) && (get_fp_id() != GOODIX_GF9518N) && (get_fp_id() != GOODIX_GF9608) && (get_fp_id() != GOODIX_GF9578))  {
		pr_warn("%s, wrong fp id, not gf9518 driver, exit", __func__);
		return 0;
	}
#endif
