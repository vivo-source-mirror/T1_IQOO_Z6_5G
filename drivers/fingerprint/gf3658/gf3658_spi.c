...
//	wakeup_source_init(&vivo_wakeup_t.fp_wakelock, "fp_wakelock");
	wakeup_source_add(&vivo_wakeup_t.fp_wakelock);
...
#endif
{
	...
	wakeup_source_remove(&vivo_wakeup_t.fp_wakelock);
...
static int __init gf_init(void)
{
	...
	*/
	...
	if (os_boot_puresys == 1) {
		pr_err("%s:boot puresys,don't register touchscreen driver.", __func__);
		return 0;
	}
	//vivo wuzengshun add for boot pure system to upgrade end
#endif
