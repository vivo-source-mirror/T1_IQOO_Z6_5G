 ...
 * "none" - system;
 * "all" - related;
 * "subsys" - only the subsys related;
 * "subsys1,subsys2" - only subsys1 and subsys2 related;
 * default - related;
 */
static char ssr_related[128];
module_param_string(ssr_related, ssr_related, sizeof(ssr_related), 0400);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < ARRAY_SIZE(restart_levels); i++)
		if (!strncasecmp(buf, restart_levels[i], count)) {
			...
			if (ssr_related[0] && strnstr(ssr_related, "always", sizeof(ssr_related) - 1)) {
				if (strnstr(ssr_related, subsys->desc->name, sizeof(ssr_related) - 1))
					subsys->restart_level = RESET_SUBSYS_COUPLED;
				else if (strnstr(ssr_related, "all", sizeof(ssr_related) - 1))
					subsys->restart_level = RESET_SUBSYS_COUPLED;
				else if (strnstr(ssr_related, "none", sizeof(ssr_related) - 1))
					subsys->restart_level = RESET_SOC;
			}
			pr_info("%s: subsys[%s] %sset restart_level[%s]\n", __func__, subsys->desc->name,
					(subsys->restart_level == i) ? "" : "force ",
...
struct subsys_device *subsys_register(struct subsys_desc *desc)
{
	...
	if (ofnode) {
		...
		subsys->restart_level = RESET_SUBSYS_COUPLED;
		if (ssr_related[0]) {
			if (strnstr(ssr_related, desc->name, sizeof(ssr_related) - 1))
				subsys->restart_level = RESET_SUBSYS_COUPLED;
			else if (strnstr(ssr_related, "all", sizeof(ssr_related) - 1))
				subsys->restart_level = RESET_SUBSYS_COUPLED;
			else if (strnstr(ssr_related, "none", sizeof(ssr_related) - 1))
		...
		pr_info("HKC subsys[%s] restart_level[%s]\n", desc->name,
				restart_levels[subsys->restart_level]);
