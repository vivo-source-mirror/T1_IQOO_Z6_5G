...
static bool hung_task_current;
static struct task_struct *hung_task_culprit;
struct task_struct *hung_task_get_culprit(void);
static struct task_struct *hung_task_get_culprit_get(void)
{
	struct task_struct *task_culprit;
	struct task_struct *(*helper)(void);

	helper = symbol_get(hung_task_get_culprit);
	if (helper) {
		task_culprit = helper();
		symbol_put(hung_task_get_culprit);
		return task_culprit;
	}
...
void panic_backtrace_log_for_hung_task(void)
{
	...
	callstack.skip = 0;
	save_stack_trace_tsk(hung_task_culprit, &callstack);

	for (iterator = 0; iterator < callstack.nr_entries; iterator++) {
		if (callstack.entries[iterator] == ULONG_MAX)
			continue;

		curr_idx += snprintf(vaddr + curr_idx, size - curr_idx,
					 "[<%p>] %pS\n", (void *)(callstack.entries[iterator]), (void *)(callstack.entries[iterator]));
	}

	return;
}

extern int panic_cpu_id;
void panic_backtrace_stop(void)
{
	...
	printk("%s curr_idx = %d\n", __func__, curr_idx);
	if (panic_cpu_id == -1) {
		panic_cpu_id = raw_smp_processor_id();
		return;
	}
	if (vaddr == NULL || stop_logging)
		return;

	/* khungtask panic case.
	 * if khungtaskd trigger the panic, and the blocked task is not null,
	 * then storing the backtrace_log for the blocked task at the stop point.
	 */
	if (hung_task_current && hung_task_culprit)
		panic_backtrace_log_for_hung_task();
...
void panic_backtrace_log(int cpu, unsigned long where)
{
	...
	if (vaddr == NULL || cpu != panic_cpu_id || stop_logging)
		return;
#if 0
	printk("%s curr_idx = %d\n", __func__, curr_idx);
	printk("%s cpu = %d where = %p %pS\n", __func__, cpu, where, where);
	printk("%s vaddr = %p panic_cpu_id = %d stop_logging = %d\n", __func__, vaddr, panic_cpu_id, stop_logging);
#endif
	if (curr_idx == 0) {
		u64 tv_kernel;
		...
		/* khungtask panic case.
		 * do initial setup when first time call panic_backtrace_log.
		 */
		if (!strncmp(current->comm, "vkhungtaskd", TASK_COMM_LEN) ||
			!strncmp(current->comm, "khungtaskd", TASK_COMM_LEN)) {
			hung_task_current = true;
			hung_task_culprit = hung_task_get_culprit_get();
		}
	}
	...
	/* khungtask panic case.
	 * if khungtaskd trigger the panic, and the blocked task is not null,
	 * then skip storing the backtrace_log for khungtaskd itself.
	 */
	if (hung_task_current && hung_task_culprit)
		return;
