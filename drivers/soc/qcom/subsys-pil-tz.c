...
static void subsys_crash_collect(struct work_struct *work)
{
	...
	/* 1:modem, 2:adsp, 3:cdsp, 4:esoc0, 5:other subsystem*/
...
#if !defined(VIVO_PROJECT_MODEL)
/*not GDPR code start*/
	track_event(MODULE_ID, EVENT_ID, ex_data);
/*not GDPR code end*/
#endif
	return;
}
...
static void log_failure_reason(const struct pil_tz_data *d)
{
	...
	if (d->smem_id == -1) {
		strlcpy(crash_reason, "pil_tz_data smem_id equals '-1'", (size_t)(MAX_SSR_REASON_LEN - 1));
	...
	if (!smem_reason[0]) {
		...
		strlcpy(crash_reason, "SFR: (unknown, empty string found)", min(size, (size_t)(MAX_SSR_REASON_LEN - 1)));
		schedule_work(&subsys_crash_work);
		return;
	}
	...
	strlcpy(subtype, name, min(size, (size_t)MAX_SUBSYS_NAME_LEN));
	strlcpy(crash_reason, smem_reason, min(size, (size_t)(MAX_SSR_REASON_LEN - 1)));
	schedule_work(&subsys_crash_work);
	/* vivo marongqiang add for subsystem crash collect end */
}
