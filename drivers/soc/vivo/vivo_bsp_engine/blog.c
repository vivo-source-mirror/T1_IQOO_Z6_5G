...
static u32 get_blog_hdr_crc(struct blog_header *hdr)
{
	...
	offset = (unsigned long)((u32 *)(&hdr->crc) + 1) - (unsigned long)hdr;
...
static int read_log_partition(unsigned long pos,
		unsigned char *buf, size_t count)
{
	...
	ret = partition_rw(BLOG_PARTITION, 0, pos + BLOG_PARTITION_OFFSET,
			buf, count);
	if (ret < 0)
...
static int write_log_partition(unsigned long pos, unsigned char *buf,
		size_t count)
{
	int ret;
	...
	ret = partition_rw(BLOG_PARTITION, 1, pos + BLOG_PARTITION_OFFSET,
			buf, count);
	if (ret < 0)
...
{
	return read_log_partition(0, (unsigned char *)&blog_hdr,
			sizeof(struct blog_header));
}

static int set_blog_header_to_partition(void)
{
	blog_hdr.crc = get_blog_hdr_crc(&blog_hdr);
	return write_log_partition(0, (unsigned char *)&blog_hdr,
			sizeof(struct blog_header));
}

static int write_step_to_partition(unsigned int type, unsigned int pos,
...
static int find_stage2_id(const char *name, unsigned long step)
{
	...
	for (i = 0;
			i < ARD_MODULE_INFO_NUM && *(module_list + i * MODULE_NAME_LEN);
			i++) {
		if (!strncmp(name,
					module_list + i * MODULE_NAME_LEN,
					MODULE_NAME_LEN)) {
			ret = i;
			break;
		}
...
static int check_blog_header(void)
{
	...
	blog_dbg("[%s][%d] magic %x version %d\n", __func__, __LINE__,
			blog_hdr.magic, blog_hdr.version);
	if (blog_hdr.magic == BLOG_MAGIC_NUM &&
			blog_hdr.version == BLOG_VERSION)
		return 0;
...
static unsigned int get_real_ptr(struct step_buffer *step, unsigned int ptr)
{
	return (ptr < step->size) ? ptr :
		(step->ring_offset +
		 (ptr - step->size) % (step->size - step->ring_offset));
}
...
static void update_step_partition(unsigned int type, struct step_buffer *step)
{
	...
	if (step->wp >= step->size) {
		blog_dbg("[%s][%d] type %d buffer full, not support partial ring!\n",
...
static void copy_to_step(struct step_buffer *step, const char *buf,
		unsigned int len)
{
...
void kstep_log(const char *fmt, ...)
{
	...
	len = snprintf(tmp, sizeof(tmp) - 1, "[%6d.%06d] ",
			(int)(clk / 1000000000), (int)(clk / 1000 % 1000000));
	va_start(args, fmt);
		...
		size_t count, loff_t *ppos)
{
	...
	blog_dbg("[%s][%d] parse: %s, %d, %s\n", __func__, __LINE__,
			s2_name, mod_step, state_str);

	if (INVAILD_STAGE2(s2_id) || INVAILD_STEP(mod_step) ||
			INVAILD_STATE(state)) {
		...
		unsigned long code, void *args)
{
	...
	pr_info("reboot code(%d) args(%s)\n", code, args ? : "null");
	kstep_log("reboot code(%d) args(%s)\n", code, args ? : "null");
...
static ssize_t state_proc_write(struct file *file, const char *buffer,
...
int proc_steplog_init(void)
{
	...
	for (i = 0; i < BLOG_NUM_MAX; i++) {
		...
		proc_create_data(blog_name[i], 0400, blog_entry,
				&blog_proc_fops, (void *)i);
	}
