...
static void dwc3_ext_event_notify(struct dwc3_msm *mdwc)
{
	...
	dev_dbg(mdwc->dev, "enter: mdwc->inputs:%x hs_phy_flags:%x\n",
				mdwc->inputs, mdwc->hs_phy->flags);
	...
	dev_dbg(mdwc->dev, "exit: mdwc->inputs:%x\n", mdwc->inputs);
	queue_delayed_work(mdwc->sm_usb_wq, &mdwc->sm_work, 0);
}
...
static int dwc3_msm_usb_set_role(struct device *dev, enum usb_role role)
{
	...
	dbg_log_string("cur_role:%s new_role:%s\n", usb_role_string(cur_role),
						usb_role_string(role));
	dev_dbg(mdwc->dev, "cur_role:%s new_role:%s\n", usb_role_string(cur_role),
						usb_role_string(role));
	...
	if (mdwc->drd_state != DRD_STATE_UNDEFINED && cur_role == role) {
		...
		dev_dbg(mdwc->dev, "no USB role change");
		return 0;
	}
...
#if IS_ENABLED(CONFIG_USB_DWC3_VUSB)
ssize_t dwc3_msm_otg_mode(void *mdwc, char *buf)
{
	struct dwc3_msm *motg = mdwc;
	ssize_t size;
	...
	case DRD_STATE_PERIPHERAL:
		...
		break;
	default:
		if (motg->id_state == DWC3_ID_GROUND)
			size = snprintf(buf, PAGE_SIZE, "%s", "host\n");
		else
...
int dwc3_msm_otg_vbus(void *mdwc)
{
	struct dwc3_msm *motg = mdwc;
	...
	return motg->vbus_active;
}

extern void dwc3_vusb_register_callback(void *func, char *name);
#endif
...
#if IS_ENABLED(CONFIG_USB_DWC3_VUSB)
	dwc3_vusb_register_callback(dwc3_msm_otg_mode, "mode");
	dwc3_vusb_register_callback(dwc3_msm_otg_vbus, "vbus");
#endif
