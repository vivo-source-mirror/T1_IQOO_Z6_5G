...
#include <linux/vivo_power_supply.h>
...
#define VIVO_USB_TYPE_TYPEC 0
#define VIVO_USB_TYPE_MICRO 1
#define VIVO_USB_TYPE_TCPC  2

ssize_t (*dwc3_vusb_otg_mode)(void *mdwc, char *buf);
int (*dwc3_vusb_otg_vbus)(void *mdwc);

#define USB_TCPC_VOTER "USB_TCPC_VOTER"
#define ACCDET_TCPC_VOTER "ACCDET_TCPC_VOTER"
void (*vusb_vote_typec_drp)(const char *voter, bool drp);
int (*vusb_get_typec_drp_voter_effective)(const char *voter);
...
void dwc3_vusb_register_callback(void *func, char *name)
{
	if (!strncmp(name, "mode", 4)) {
		if (!dwc3_vusb_otg_mode)
			dwc3_vusb_otg_mode = func;
	} else if (!strncmp(name, "vbus", 4)) {
		if (!dwc3_vusb_otg_vbus)
			dwc3_vusb_otg_vbus = func;
	}
}
EXPORT_SYMBOL(dwc3_vusb_register_callback);

void vusb_vote_register_callback(void *func_vote, void *func_get)
{
	if (!vusb_vote_typec_drp)
		vusb_vote_typec_drp = func_vote;
	if (!vusb_get_typec_drp_voter_effective)
		vusb_get_typec_drp_voter_effective = func_get;
}
EXPORT_SYMBOL(vusb_vote_register_callback);

static unsigned int g_role = TYPEC_PORT_UFP;
void tcpc_usb_set_data_role(enum typec_port_data role)
{
	g_role = role;
}
EXPORT_SYMBOL(tcpc_usb_set_data_role);

int tcpc_usb_get_data_role(unsigned int *role)
{
	if (!role) {
		pr_err("%s %d  or role is null\n", __func__, __LINE__);
		return -ENODEV;
	}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if (dwc3_vusb->mode == VIVO_USB_TYPE_MICRO)
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	if (!dwc3_vusb_otg_mode) {
		pr_err("%s: %d: otg mode callback is null\n", __func__, __LINE__);
		return -ENODEV;
	}

	mutex_lock(&dwc3_vusb->switch_mutex);
	size = dwc3_vusb_otg_mode(dev_get_drvdata(&dwc3_vusb->usbc_pdev->dev), buf);

	/* Typec hw auto detect */
	if (dwc3_vusb->typec_hw_det) {
		if (!strncmp(buf, "peripheral", 10) || !strncmp(buf, "none", 4)) {
			if (!vusb_get_typec_drp_voter_effective &&
				    vusb_get_typec_drp_voter_effective(ACCDET_TCPC_VOTER)) {
				size = snprintf(buf, PAGE_SIZE, "%s", "ccopened\n");
				pr_info("%s %d in ccopend mode\n", __func__, __LINE__);
			}
		}
	}
					 ...
					 const char *buf, size_t size)
{
	...
	if (!strncmp(value, "enabled", 7))
		...
		pr_err("%s: %d: store value error\n", __func__, __LINE__);
		return -EINVAL;
	}

	if ((dwc3_vusb->mode == VIVO_USB_TYPE_MICRO) && (dwc3_vusb->id_pull < 0)) {
		pr_err("%s: %d: id-volt invalid\n", __func__, __LINE__);
		return -ENODEV;
	}

	if ((dwc3_vusb->mode != VIVO_USB_TYPE_MICRO) && !vusb_vote_typec_drp) {
		pr_err("%s: %d: vusb_vote_typec_drp is null\n", __func__, __LINE__);
		return -ENODEV;
	}
	...
	if (is_enable) {
		...
		if (dwc3_vusb->mode == VIVO_USB_TYPE_MICRO) {
			...
			cancel_delayed_work(&dwc3_vusb->host_disabled_work);
			vusb_vote_typec_drp(USB_TCPC_VOTER, false);
		} else {
			pr_err("%s %d typec mode %d is not support\n", __func__, __LINE__, dwc3_vusb->mode);
		}
...
//					  POWER_SUPPLY_PROP_OTG_VBUS_ENABLE, &pval);
		...
		if (dwc3_vusb->usbc_pdev && dwc3_vusb_otg_vbus) {
			if (dwc3_vusb_otg_vbus(dev_get_drvdata(&dwc3_vusb->usbc_pdev->dev))) {
...
static void dwc3_vusb_typec_detect_work(struct work_struct *work)
{
	...
	unsigned int role = TYPEC_PORT_UFP;
	bool cc;
	int ret;
	...
	/*
	 * if usb type is tcpc, then use tcpc_usb_get_data_role
	 * to check usb plugin or audio plugin
	 */
	if (dwc3_vusb->mode == VIVO_USB_TYPE_TCPC) {
		ret = tcpc_usb_get_data_role(&role);
		if (ret != 0) {
			pr_err("%s: %d: get date failed\n", __func__, __LINE__);
			mutex_unlock(&dwc3_vusb->switch_mutex);
			return;
		}
		if (dwc3_vusb->typec_hw_det && (role == TYPEC_PORT_DRD))
			cc = false;
		else if (role == TYPEC_PORT_DFP)
			cc = false;
		else if (TYPEC_PORT_UFP)
			cc = true;
	} else {
		cc = true;
		pr_err("%s %d typec mode %d is not support\n", __func__, __LINE__, dwc3_vusb->mode);
	...
	pr_info("%s: %d: cc = %d-%d, cc_state = %d\n", __func__, __LINE__, role, cc, dwc3_vusb->cc_state);
...
static void dwc3_vusb_host_disabled_work(struct work_struct *work)
{
	...
	if (dwc3_vusb->mode == VIVO_USB_TYPE_MICRO) {
		...
		pr_err("%s: %d: usb type not support\n", __func__, __LINE__);
	}
			       ...
			       struct dwc3_vusb *dwc3_vusb)
{
	...
	/* Typec USB */
	if (dwc3_vusb->mode == VIVO_USB_TYPE_TYPEC ||
		dwc3_vusb->mode == VIVO_USB_TYPE_TCPC) {
		/* hw auto detect */
...
err1:
	...
	wake_lock_destroy(&dwc3_vusb->host_wake_lock);
err:
	return ret;
}
