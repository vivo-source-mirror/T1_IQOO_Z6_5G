...
EXPORT_SYMBOL_GPL(tcpm_inquire_typec_role);

#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
uint8_t vivo_get_typec_power_role(void)
{
...
#else
	pr_err("%s CONFIG_TCPC_CLASS not defined\n", __func__);
...
EXPORT_SYMBOL_GPL(tcpm_typec_role_swap);
...
EXPORT_SYMBOL_GPL(tcpm_typec_change_role);

#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
void vivo_set_typec_power_role(int power_role)
{
#if IS_ENABLED(CONFIG_TCPC_CLASS)
	static struct tcpc_device *tcpc = NULL;
	...
	pr_info("%s set pwr_role:%d\n", __func__, power_role);
	tcpc_typec_change_role(tcpc, (uint8_t)power_role);
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
uint8_t vivo_get_pd_power_role(void)
{
#if IS_ENABLED(CONFIG_TCPC_CLASS)
	static struct tcpc_device *tcpc = NULL;
	...
	return tcpm_inquire_pd_power_role(tcpc);
#else
	pr_err("%s CONFIG_TCPC_CLASS not defined\n", __func__);
...
uint8_t vivo_get_pd_date_role(void)
{
#if IS_ENABLED(CONFIG_TCPC_CLASS)
	static struct tcpc_device *tcpc = NULL;
	...
	return tcpm_inquire_pd_data_role(tcpc);
#else
	pr_err("%s CONFIG_TCPC_CLASS not defined\n", __func__);
...
EXPORT_SYMBOL_GPL(tcpm_dpm_pd_power_swap);
...
EXPORT_SYMBOL_GPL(tcpm_dpm_pd_data_swap);
...
EXPORT_SYMBOL_GPL(tcpm_dpm_pd_vconn_swap);
