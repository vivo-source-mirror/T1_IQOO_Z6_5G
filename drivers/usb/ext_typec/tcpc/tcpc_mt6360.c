...
static int mt6360_pmu_tcpc_irq_register(struct tcpc_device *tcpc)
{
	...
	is_enable = gpio_get_value(chip->usbid_gpio);
	pr_err("is_enable = %d\n", is_enable);
#endif
	...
	for (i = 0; i < ARRAY_SIZE(mt6360_pmu_tcpc_irq_desc); i++) {
		...
		if (!(tcpc->tcpc_flags & TCPC_FLAGS_WATER_DETECTION) &&
			...
			continue;
#else
	/*usbid interuption will regist when is_enable is false, this manipulation is based on the design of board-id*/
		if (is_enable && strcmp(irq_desc->name, "usbid_evt") == 0)
			continue;
#endif
#endif
			   ...
			   struct mt6360_tcpc_platform_data *pdata)
{
	...
	dev->of_node = np;
#ifdef CONFIG_VIVO_USBID_ENABLE
	chip->usbid_gpio = of_get_named_gpio(np, "vivo,usbid-en-gpio", 0);
#endif
			    ...
			    const struct i2c_device_id *id)
{
	...
	chip->usbid_present = false;
	chip->usbid_present_first_detect = false;
	if (chip->usbid_gpio > 0) {
		ret = gpio_request(chip->usbid_gpio, "usbid_gpio");
		if (ret) {
			pr_err("failed to request usbid_gpio,rc=%d\n", ret);
		}
	}
