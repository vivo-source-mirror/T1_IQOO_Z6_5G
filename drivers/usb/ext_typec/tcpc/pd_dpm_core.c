...
EXPORT_SYMBOL_GPL(dpm_check_supported_modes);
...
EXPORT_SYMBOL_GPL(dpm_get_svdm_svid_data);
...
#ifdef CONFIG_USB_PD_REV30
	//wakeup_source_init(&pd_port->pps_request_wake_lock, "pd_pps_request_wakelock");
	wakeup_source_add(&pd_port->pps_request_wake_lock);
