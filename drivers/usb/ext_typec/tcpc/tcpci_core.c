...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
		memset(str, 0x0, 64);
		sprintf(str, "[%d] %s\n", tcpm_inquire_typec_role(tcpc), role_text[tcpm_inquire_typec_role(tcpc)]);
		strcat(buf, str);

		memset(str, 0x0, 64);
		sprintf(str, "\nnote:\n");
		strcat(buf, str);
		for (i = 0; i < TYPEC_ROLE_NR; i++) {
			memset(str, 0x0, 64);
			sprintf(str, "%d - %s\n", i, role_text[i]);
			strcat(buf, str);
		}
...
EXPORT_SYMBOL_GPL(tcpc_dev_get_by_name);
	...
	struct tcpc_desc *tcpc_desc, struct tcpc_ops *ops, void *drv_data)
{
	...
	wakeup_source_add(&tcpc->attach_wake_lock);
	//wakeup_source_init(&tcpc->dettach_temp_wake_lock, "tcpc_detach_wakelock");
	wakeup_source_add(&tcpc->dettach_temp_wake_lock);
...
int tcpc_device_irq_enable(struct tcpc_device *tcpc)
{
	...
	if (boot_mode != KERNEL_POWER_OFF_CHARGING_BOOT && boot_mode != LOW_POWER_OFF_CHARGING_BOOT) {
		...
		pr_info("%s : power_off_charging_mode skip event_init_work\n", __func__);
	}
#else
	schedule_delayed_work(
			&tcpc->event_init_work, msecs_to_jiffies(10*1000));
...
EXPORT_SYMBOL_GPL(tcpc_schedule_init_work);
...
void tcpc_device_unregister(struct device *dev, struct tcpc_device *tcpc)
{
	...
	wakeup_source_remove(&tcpc->pd_port.pps_request_wake_lock);
#endif /* CONFIG_USB_PD_REV30 */
	//wakeup_source_trash(&tcpc->dettach_temp_wake_lock);
	//wakeup_source_trash(&tcpc->attach_wake_lock);

	wakeup_source_remove(&tcpc->dettach_temp_wake_lock);
	wakeup_source_remove(&tcpc->attach_wake_lock);
#if IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF)
