...
static inline void typec_source_attached_entry(struct tcpc_device *tcpc_dev)
{
	...
	tcpci_vendor_defined_cmd(tcpc_dev, 1);
	vivo_sgm7220(tcpc_dev);
#endif
}
...
EXPORT_SYMBOL_GPL(tcpc_typec_handle_pe_pr_swap);
...
EXPORT_SYMBOL_GPL(tcpc_typec_error_recovery);
...
#ifdef CONFIG_TCPC_SGM7220
	if (typec_role == TYPEC_ROLE_SNK) {
		tcpci_vendor_defined_cmd(tcpc_dev, 0);
	} else if (typec_role == TYPEC_ROLE_DRP) {
		tcpci_vendor_defined_cmd(tcpc_dev, 2);
	}
#endif
