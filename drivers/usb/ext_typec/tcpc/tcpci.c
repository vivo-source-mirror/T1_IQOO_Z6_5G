...
EXPORT_SYMBOL_GPL(tcpci_check_vsafe0v);

int tcpci_alert_status_clear(
	struct tcpc_device *tcpc, uint32_t mask)
{
	if (tcpc->ops->alert_status_clear == NULL)
		return 0;

	return tcpc->ops->alert_status_clear(tcpc, mask);
}
EXPORT_SYMBOL_GPL(tcpci_alert_status_clear);

#ifdef CONFIG_TCPC_SGM7220
int tcpci_vendor_defined_cmd(
	struct tcpc_device *tcpc, uint32_t mask)
{
	if (tcpc->ops->vendor_defined_cmd == NULL)
		return 0;

	return tcpc->ops->vendor_defined_cmd(tcpc, mask);
}
#endif
...
{
	if (tcpc->ops->fault_status_clear == NULL)
		return 0;
...
int tcpci_get_alert_mask(
	struct tcpc_device *tcpc, uint32_t *mask)
{
	if (tcpc->ops->get_alert_mask == NULL)
		return 0;
...
{
	if (tcpc->ops->get_alert_status == NULL)
		return 0;

	return tcpc->ops->get_alert_status(tcpc, alert);
}
EXPORT_SYMBOL_GPL(tcpci_get_alert_status);
	...
	struct tcpc_device *tcpc, uint16_t *pw_status)
{
	...
	if (tcpc->ops->get_power_status == NULL)
		return 0;
...
{
	if (tcpc->ops->init_alert_mask == NULL)
		return 0;
...
EXPORT_SYMBOL_GPL(tcpci_set_cc);

int tcpci_set_polarity(struct tcpc_device *tcpc, int polarity)
{
	if (tcpc->ops->set_polarity == NULL)
		return 0;
...
EXPORT_SYMBOL_GPL(tcpci_set_vconn);
...
EXPORT_SYMBOL_GPL(tcpci_set_msg_header);

int tcpci_set_rx_enable(struct tcpc_device *tcpc, uint8_t enable)
{
	if (tcpc->ops->set_rx_enable == NULL)
		return 0;

	return tcpc->ops->set_rx_enable(tcpc, enable);
}
EXPORT_SYMBOL_GPL(tcpci_set_rx_enable);
...
{
	if (tcpc->ops->transmit == NULL)
		return 0;

	return tcpc->ops->transmit(tcpc, type, header, data);
}
EXPORT_SYMBOL_GPL(tcpci_transmit);
...
EXPORT_SYMBOL_GPL(tcpci_set_bist_test_mode);

int tcpci_set_bist_carrier_mode(struct tcpc_device *tcpc, uint8_t pattern)
{
	if (tcpc->ops->set_bist_carrier_mode == NULL)
		return 0;
...
EXPORT_SYMBOL_GPL(tcpci_set_bist_carrier_mode);

#ifdef CONFIG_USB_PD_RETRY_CRC_DISCARD
int tcpci_retransmit(struct tcpc_device *tcpc)
{
	if (tcpc->ops->retransmit == NULL)
		return 0;

	return tcpc->ops->retransmit(tcpc);
}
EXPORT_SYMBOL_GPL(tcpci_retransmit);
...
int tcpci_notify_typec_state(struct tcpc_device *tcpc)
{
	...
	//pr_info("%s---old_state = %d, new_state = %d.\n", __func__, tcp_noti.typec_state.old_state, tcp_noti.typec_state.new_state);
	if (tcp_noti.typec_state.old_state == TYPEC_UNATTACHED &&
		tcp_noti.typec_state.new_state == TYPEC_ATTACHED_AUDIO) {
		pr_info("%s-audio Plug in.\n", __func__);
		/* audio plugin logic here */
		max20328_extern_usbc_event_changed(TYPEC_ACCESSORY_AUDIO);
	} else if (tcp_noti.typec_state.old_state == TYPEC_ATTACHED_AUDIO &&
		tcp_noti.typec_state.new_state == TYPEC_UNATTACHED) {
		pr_info("%s-audio Plug out.\n", __func__);
		/* audio plugout logic here */
		max20328_extern_usbc_event_changed(TYPEC_ACCESSORY_NONE);
	}
...
EXPORT_SYMBOL_GPL(tcpci_notify_role_swap);
...
EXPORT_SYMBOL_GPL(tcpci_notify_pd_state);
...
EXPORT_SYMBOL_GPL(tcpci_source_vbus);
int tcpci_sink_vbus(
	struct tcpc_device *tcpc, uint8_t type, int mv, int ma)
{
	struct tcp_notify tcp_noti;
...
#ifdef VIVO_TYPC_FEATURE
	if(tcpc->vbus_level == TCPC_VBUS_VALID)
		tcp_noti.vbus_state.plug = 1;
	else
		tcp_noti.vbus_state.plug = 0;
#endif /* VIVO_TYPC_FEATURE */
...
EXPORT_SYMBOL_GPL(tcpci_sink_vbus);
...
EXPORT_SYMBOL_GPL(tcpci_enable_force_discharge);
...
EXPORT_SYMBOL_GPL(tcpci_notify_hard_reset_state);
...
EXPORT_SYMBOL_GPL(tcpci_enter_mode);
...
EXPORT_SYMBOL_GPL(tcpci_exit_mode);
...
EXPORT_SYMBOL_GPL(tcpci_dp_status_update);
...
EXPORT_SYMBOL_GPL(tcpci_dp_configure);
...
EXPORT_SYMBOL_GPL(tcpci_dp_attention);
...
EXPORT_SYMBOL_GPL(tcpci_dp_notify_status_update_done);
...
EXPORT_SYMBOL_GPL(tcpci_dp_notify_config_start);
...
EXPORT_SYMBOL_GPL(tcpci_dp_notify_config_done);
...
EXPORT_SYMBOL_GPL(tcpci_notify_uvdm);
...
EXPORT_SYMBOL_GPL(tcpci_dc_notify_en_unlock);
...
EXPORT_SYMBOL_GPL(tcpci_notify_alert);
...
EXPORT_SYMBOL_GPL(tcpci_notify_status);
...
EXPORT_SYMBOL_GPL(tcpci_notify_request_bat_info);
