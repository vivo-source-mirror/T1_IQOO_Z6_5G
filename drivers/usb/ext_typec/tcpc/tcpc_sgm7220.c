...
/* vendor defined commend */
bool power_status_change = false;
...
static int sgm7220_read_reg(struct i2c_client *i2c, u8 reg, u8 *dest)
{
	...
	if (ret < 0) {
		pr_err("%s: (0x%x) error, ret(0x%x)\n", __func__, reg, ret);
		return ret;
	}

	ret &= 0xff;
	*dest = ret;
	pr_err("%s: (0x%x) error, value(0x%x)\n", __func__, reg, ret);
	return 0;
}
...
static void sgm7220_irq_work_handler(struct kthread_work *work)
{
	...
	sgm7220_read_reg(chip->client, REG_MOD, &reg_val);
	pr_err("%s:: reg_0x08 = 0x%x \n", __func__, reg_val);
	sgm7220_read_reg(chip->client, REG_INT, &reg_val);
	pr_err("%s:: reg_0x09 = 0x%x \n", __func__, reg_val);
	sgm7220_read_reg(chip->client, REG_SET, &reg_val);
	pr_err("%s:: reg_0x0A = 0x%x \n", __func__, reg_val);
	sgm7220_read_reg(chip->client, REG_CTL, &reg_val);
	pr_err("%s:: reg_0x45 = 0x%x \n", __func__, reg_val);
...
static irqreturn_t sgm7220_intr_handler(int irq, void *data)
{
	...
	wake_lock_timeout(&chip->irq_wake_lock, sgm7220_IRQ_WAKE_TIME);
	...
	*pwr_status = 0;
	...
	if (val & CABLE_STATE_AS_UFP) {
		...
		*pwr_status |= TCPC_REG_POWER_STATUS_VBUS_PRES;
		power_status_change = false;
	}

#ifdef CONFIG_TCPC_VSAFE0V_DETECT_IC
	if ((power_status_change) && (val & CABLE_STATE_AS_DFP)) {
		/* vivo add: workaround for power status change */
		*pwr_status |= TCPC_REG_POWER_STATUS_VBUS_PRES;
	} else if (val & CABLE_STATE_AS_DFP) {
		/* sgm7220 does not supprt vbus detection,
		assume vbus is detected if act as DFP */
...
int sgm7220_vendor_defined_cmd(struct tcpc_device *tcpc, uint32_t mask)
{
	struct sgm7220_chip *chip = tcpc_get_dev_data(tcpc);
	if (mask == 1) {
		power_status_change = true;
	} else if (mask == 0) {
		power_status_change = false;
	} else if (mask == 2) {
		power_status_change = false;
		sgm7220_software_reset(tcpc);
		kthread_queue_work(&chip->irq_worker, &chip->irq_work);
	}
	return 0;
}
...
static struct tcpc_ops sgm7220_tcpc_ops = {
	...
	.vendor_defined_cmd = sgm7220_vendor_defined_cmd,
};
...
static int sgm7220_parse_dt(struct sgm7220_chip *chip, struct device *dev)
{
	...
	if (!np) {
		pr_err("%s find node sgm7220_type_c_port0 fail\n", __func__);
		return -ENODEV;
	}
...
static int sgm7220_tcpcdev_init(struct sgm7220_chip *chip, struct device *dev)
{
	...
	if (of_property_read_u32(
		...
		desc->notifier_supply_num = 0;
	pr_err("%s: notifier_supply_num = %d\n", __func__, desc->notifier_supply_num);
	...
	pr_err("%s: desc->rp_lvl = %d\n", __func__, desc->rp_lvl);
	...
	pr_err("%s: desc->vconn_supply = %d\n", __func__, desc->vconn_supply);
...
static const struct of_device_id sgm_match_table[] = {
	{.compatible = "sgm,usb_typec",},
	{},
};

static struct i2c_driver sgm7220_driver = {
	.driver = {
		.name = "sgm7220_usb_typec",
		.owner = THIS_MODULE,
...
static int __init sgm7220_init(void)
{
	...
	pr_err("sgm7220_init (%s): initializing...\n", SGM7220_DRV_VERSION);
	np = of_find_node_by_name(NULL, "sgm7220_usb_typec");
	if (np != NULL)
		pr_err("sgm7220_usb_typec node found...\n");
	else
		pr_err("sgm7220_usb_typec node not found...\n");
