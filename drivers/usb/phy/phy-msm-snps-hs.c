...
static int msm_hsphy_init(struct usb_phy *uphy)
{
	...
	dev_dbg(uphy->dev, "(mode: %d)x0:%08x x1:%08x x2:%08x x3:%08x\n", !!(phy->phy.flags & PHY_HOST_MODE),
...
static int msm_hsphy_probe(struct platform_device *pdev)
{
	...
	if (phy->param_override_seq_cnt_host > 0) {
		...
		if (phy->param_override_seq_cnt_host % 2) {
			dev_err(dev, "invalid param_override_seq_cnt_host\n");
			return -EINVAL;
		}
	...
	/*miaoqiang add for host tunnig end*/
