...
#endif
static enum wb_flush_ctrl ufshcd_is_wb_flush_need(struct ufs_hba *hba);
...
static int ufshcd_devfreq_scale(struct ufs_hba *hba, bool scale_up)
{
	...
	up_write(&hba->clk_scaling_lock);
	/*  Enable Write Booster always */
	ufshcd_wb_ctrl(hba, true);

	if (ufshcd_is_wb_flush_need(hba) == NEED_FLUSH) {
		dev_err(hba->dev, "%s: %d Enable wb flush\n",
				__func__, __LINE__);
		ufshcd_wb_toggle_flush_during_h8(hba, true);
	} else if (ufshcd_is_wb_flush_need(hba) == NO_NEED_FLUSH) {
		dev_err(hba->dev, "%s: %d Disable wb flush\n",
				__func__, __LINE__);
...
#ifdef CONFIG_BLK_ENHANCEMENT
	trigger_cmd_issue(cmd->request, lrbp->issue_time_stamp);
#endif
	spin_unlock_irqrestore(hba->host->host_lock, flags);
	goto out;
...
int ufshcd_query_attr_retry_ven(struct ufs_hba *hba,
	...
	u32 *attr_val)
{
	return ufshcd_query_attr_retry(hba, opcode, idn, index, selector, attr_val);
}
//EXPORT_SYMBOL(ufshcd_query_attr_retry_ven);
...
static inline void ufshcd_remove_non_printable_ven(char *val)
{
	if (!val || !*val)
		return;
			    ...
			    u8 *buf, u32 size, bool ascii)
{
	...
	if (ascii) {
		...
		for (i = 0; i < ascii_len; i++)
			ufshcd_remove_non_printable_ven(&buff_ascii[i]);
...
#ifdef CONFIG_BLK_ENHANCEMENT
			if (cmd->request)
				trigger_cmd_compl(cmd->request, lrbp->compl_time_stamp);
#endif
...
static enum wb_flush_ctrl ufshcd_is_wb_flush_need(struct ufs_hba *hba)
{
	...
	if (avail_buf == UFS_WB_BUF_REMAIN_PERCENT(100))
		return NO_NEED_FLUSH;
	else if (avail_buf < UFS_WB_BUF_REMAIN_PERCENT(60))
		return NEED_FLUSH;
...
#if defined(CONFIG_PM) && defined(CONFIG_QGKI)
...
static int ufshcd_scsi_add_wlus(struct ufs_hba *hba)
{
	...
	ufshcd_scsi_device_rpmb_get(hba, sdev_rpmb);
...
static int ufshcd_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	ufshcd_hba_vreg_set_hpm(hba);
#if IS_ENABLED(CONFIG_SCSI_UFSHCD_VIVO)
