...
#define		UFS_GEAR_MAX			        (UFS_HS_G4 + 1)
...
#define		UFS_CAPACITY_32GB				0x800000  /* unit: 4KB */
...
/* MICRON */
struct nand_info_micron {
	uint16_t   factory_bad_block_count;
	uint16_t   run_time_bad_block_count;
	uint16_t   spare_block_count;
	uint16_t   reserved_block_count_SLC;
	uint16_t   reserved_block_count_TLC;
	uint8_t    exhausted_life_for_SLC;
	uint8_t    exhausted_life_for_TLC;
	uint16_t   meta_data_corruption;
	uint16_t   write_amplification_factor;
	uint32_t   minimum_block_erase_for_TLC;
	uint32_t   maximum_block_erase_for_TLC;
	uint32_t   average_block_erase_for_TLC;
	uint32_t   reserved1;
	uint32_t   minimum_block_erase_for_SLC;
	uint32_t   maximum_block_erase_for_SLC;
	uint32_t   average_block_erase_for_SLC;
	uint32_t   reserved2;
	uint32_t   initialization_count_success;
	uint32_t   initialization_count_failure;
	uint32_t   read_reclaim_count_for_SLC;
	uint32_t   read_reclaim_count_for_TLC;
	uint32_t   read_data_size; //unit:100 MB
	uint32_t   written_data_size; //unit:100 MB
	uint32_t   SPOR_write_fail_count;
	uint32_t   SPOR_recovery_count;
	uint32_t   VDET_count;
	uint32_t   UECC_count;
...
//int ufshcd_read_string_desc(struct ufs_hba *hba, int desc_index, u8 *buf, bool ascii);
...
int ufshcd_query_attr_retry_ven(struct ufs_hba *hba,
