...
#define DETECT_GPIO "vivoRF_detect_gpio"
...
static struct pinctrl *RF_pinctrl;

static int set_pinctrl_state(struct device *dev)
{
	int ret = 0;
	struct pinctrl_state *nopull;

	if (IS_ERR_OR_NULL(dev)) {
		printk(KERN_ERR"the pinctrl dev is NULL\n");
		return -EINVAL;
	}
	RF_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(RF_pinctrl)) {
		printk(KERN_ERR" Failed to get pinctrl\n");
		return -EINVAL;
	}

	nopull = pinctrl_lookup_state(RF_pinctrl, DETECT_GPIO);

	if (IS_ERR_OR_NULL(nopull)) {
		printk(KERN_ERR": Failed to get pintrl state, ret is %ld\n", PTR_ERR(nopull));
		return -EINVAL;
	}

	ret = pinctrl_select_state(RF_pinctrl, nopull);
	if (ret < 0) {
		printk(KERN_ERR" Failed to set pinctrl state");
	}
						   ...
						   char *buf)
{
	...
	return snprintf(buf, 80, "%d %d %d %d %d %d %d %d %d %d\n\r", gpio_1, state1, gpio_2, state2, gpio_3, state3, gpio_4, state4, gpio_5, state5);
}
...
#ifdef CUSTOMIZE_PD2005F
	char *board_version = NULL;
	unsigned int board_flag = 0;
	board_version = get_bbk_board_version();
	printk(KERN_ERR "[%s]:[%s] board_version: %s, len = %d\n", TAG, __func__, board_version, strlen(board_version));
	if (strlen(board_version) != 0) {
		board_flag = board_version[0];
		if (board_flag == '0')
			return 0;
	}
#endif
