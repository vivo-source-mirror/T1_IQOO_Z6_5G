						...
						struct sdhci_msm_host *msm_host)
{
	...
	if (of_get_property(np, "fast_power_off", NULL)) {
		msm_host->mmc->caps2 |= MMC_FAST_POWER_OFF;
		pr_notice("mmc%d: in fast_power_off mode\n", msm_host->mmc->index);
	} else
		msm_host->mmc->caps2 &= ~MMC_FAST_POWER_OFF;

	if (of_get_property(np, "support_sdr104", NULL)) {
		msm_host->mmc->caps2 |= MMC_SUPPORT_SDR104;
		pr_notice("mmc%d: in support_sdr104 mode\n", msm_host->mmc->index);
	} else
		msm_host->mmc->caps2 &= ~MMC_SUPPORT_SDR104;
#endif
