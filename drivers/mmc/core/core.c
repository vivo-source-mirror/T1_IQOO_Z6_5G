...
void _mmc_detect_change(struct mmc_host *host, unsigned long delay, bool cd_irq)
{
	...
	pr_info("%s: mmc_schedule_delayed_work ret = %d\n", mmc_hostname(host), ret);
#else
...
#if IS_ENABLED(CONFIG_MMC_NON_GKI)
	pr_info("%s === %s === rescan_disable=%d bus_dead=%d fast_power_off=%d\n",
		mmc_hostname(host), __func__, host->rescan_disable, host->bus_dead, !!host->caps2 & MMC_FAST_POWER_OFF);
#endif
...
#if IS_ENABLED(CONFIG_MMC_NON_GKI)
	mmc_gpio_set_int_cnt_enable(host, 0);
#endif
