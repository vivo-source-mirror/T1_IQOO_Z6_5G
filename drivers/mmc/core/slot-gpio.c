...
#if IS_ENABLED(CONFIG_MMC_NON_GKI)
	bool status;
	int int_cnt_enable; /* Indicates enable record the int_cnt */
	u64 int_cnt;        /* For statistics interrupts count */
#endif
};
...
static irqreturn_t mmc_gpio_cd_irqt(int irq, void *dev_id)
{
	 ...
	 */
	if (mmc_gpio_get_int_cnt_enable(host))
		mmc_gpio_set_int_cnt(host, mmc_gpio_get_int_cnt(host) + 1);
	if (!host->ops)
		goto out;
	...
	if (status ^ ctx->status) {
		...
		if (!host->card)
			...
			mmc_detect_change(host, msecs_to_jiffies(host->caps2 & MMC_FAST_POWER_OFF ? 50 : 1000));
	}
...
int mmc_gpio_get_int_cnt_enable(struct mmc_host *host)
{
	...
	if (!ctx) {
		pr_err("%s: %d: ctx is null\n", __func__, __LINE__);
		return 0;
	}

	return ctx->int_cnt_enable;
}
EXPORT_SYMBOL(mmc_gpio_get_int_cnt_enable);
...
void mmc_gpio_set_int_cnt_enable(struct mmc_host *host, int int_cnt_enable)
{
	...
	ctx->int_cnt_enable = int_cnt_enable;
}
EXPORT_SYMBOL(mmc_gpio_set_int_cnt_enable);
...
EXPORT_SYMBOL(mmc_gpio_get_int_cnt);
...
EXPORT_SYMBOL(mmc_gpio_set_int_cnt);
...
EXPORT_SYMBOL(mmc_gpio_update_status);
