...
static int mmc_int_cnt_enable_get(void *data, u64 *val)
{
	...
	*val = mmc_gpio_get_int_cnt_enable(host);
...
void mmc_add_host_debugfs(struct mmc_host *host)
{
	...
	debugfs_create_file("clock", S_IRUSR | S_IWUSR, root, host,
			    &mmc_clock_fops);
#if IS_ENABLED(CONFIG_MMC_NON_GKI)
	debugfs_create_file("int_cnt_enable",
			    0600, root, host, &mmc_int_cnt_enable_fops);
	debugfs_create_file("int_cnt",
			    0600, root, host, &mmc_int_cnt_fops);
#endif
