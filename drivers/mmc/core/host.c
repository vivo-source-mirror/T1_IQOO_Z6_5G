...
#if IS_ENABLED(CONFIG_MMC_NON_GKI)
static ssize_t show_int_cnt_enable(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct mmc_host *host = cls_dev_to_mmc_host(dev);

	return sprintf(buf, "%d\n", mmc_gpio_get_int_cnt_enable(host));

}

static ssize_t store_int_cnt_enable(struct device *dev,
				    struct device_attribute *attr, const char *buf, size_t count)
{
	struct mmc_host *host = cls_dev_to_mmc_host(dev);
	int val, ret;
	...
	mmc_claim_host(host);
	mmc_gpio_set_int_cnt_enable(host, val);
	if (val == 1)
		mmc_gpio_set_int_cnt(host, 0);
	mmc_release_host(host);

	return count;
}
DEVICE_ATTR(int_cnt_enable, 0644, show_int_cnt_enable, store_int_cnt_enable);

static ssize_t show_int_cnt(struct device *dev,
			    struct device_attribute *attr, char *buf)
{
	struct mmc_host	*host = cls_dev_to_mmc_host(dev);

	return sprintf(buf, "%d\n", mmc_gpio_get_int_cnt(host));
}
DEVICE_ATTR(int_cnt, 0644, show_int_cnt, NULL);
...
#endif
#if IS_ENABLED(CONFIG_MMC_NON_GKI)
	sysfs_remove_group(&host->class_dev.kobj, &dev_attr_grp);
#endif
