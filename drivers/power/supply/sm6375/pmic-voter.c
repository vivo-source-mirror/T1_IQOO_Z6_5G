...
EXPORT_SYMBOL_GPL(get_votable_info);

void force_active_votable(struct votable *votable,const char *voter,bool enable,int val)
{
	int rc = 0;

	if (!votable || !voter){
		pr_err("votable or voter is null\n");
		return;
	}
	pr_err("===== votable name:%s, force voter:%s, (%d, %d) =====\n",votable->name, voter, enable, val);
	votable->force_val = val;
	rc = force_active_set(votable, enable);
}
EXPORT_SYMBOL_GPL(force_active_votable);


void get_votable_summary(struct votable *votable, char *buf, int size)
{
	int i = 0;
	int len = 0;
	uint8_t str[32];
	...
	if (votable->effective_result <= 0)
		...
		len += sprintf(buf, "%d,", votable->effective_result);

	for (i = 0; i < votable->num_clients; i++) {
		if (votable->client_strs[i] && votable->votes[i].enabled) {
			memset(str, 0x0, sizeof(str));
			if (votable->votes[i].value == 0)
				len += sprintf(str, "%s:100,", votable->client_strs[i]);
			else
				len += sprintf(str, "%s:%d,", votable->client_strs[i], votable->votes[i].value);
...
EXPORT_SYMBOL_GPL(get_votable_summary);
