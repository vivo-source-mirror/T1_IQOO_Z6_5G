...
#else
	struct proc_dir_entry *proc_dent_dir;
	struct proc_dir_entry *proc_dent;

	proc_dent_dir = proc_mkdir("tzdbg", NULL);
	if (proc_dent_dir == NULL) {
...
#else
		proc_dent = proc_create_data(tzdbg.stat[i].name, 0444, proc_dent_dir, &tzdbg_fops, &tzdbg.debug_tz[i]);
		if (proc_dent == NULL) {
...
#else
	platform_set_drvdata(pdev, proc_dent_dir);
#endif
...
#else
	struct proc_dir_entry   *proc_dent_dir;
	proc_dent_dir = platform_get_drvdata(pdev);
	proc_remove(proc_dent_dir);
#endif
}
