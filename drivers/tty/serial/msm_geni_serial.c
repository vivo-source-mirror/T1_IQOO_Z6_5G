...
// vivo guojunzheng add AT_MODE begin
#ifdef MODULE
extern unsigned int is_atboot;
#else
unsigned int is_atboot;

static int __init parse_atboot(char *arg)
{
	if (!arg)
		return 0;

	if (strncmp(arg, "sendAT", 6) == 0 || strncmp(arg, "sendNormal", 10) == 0)
		is_atboot = 1;
	else
		is_atboot = 0;

	//pr_info("%s: is_atboot:=%d\n", __func__, is_atboot);
	return 0;
}

early_param("console-at", parse_atboot);
#endif
// vivo guojunzheng add AT_MODE end
...
static int msm_geni_serial_port_setup(struct uart_port *uport)
{
	...
	if (!uart_console(uport) || is_atboot == 1) {
...
static int __init msm_geni_serial_init(void)
{
	...
	// vivo guojunzheng add AT_MODE begin
	if (is_atboot == 1) {
		msm_geni_console_driver.cons->flags |= CON_AT_MODE;
	}
	// vivo guojunzheng add AT_MODE end
