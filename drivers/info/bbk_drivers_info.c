...
static struct device_node *of_dtb_chosen = NULL;
#define FINAL_CMDLINE_SIZE 4096
static char final_cmdline[FINAL_CMDLINE_SIZE] = {0};
...
module_param_string(board_version, bbk_board_version, sizeof(bbk_board_version), 0);
module_param_string(model_version, bbk_model_version, sizeof(bbk_model_version), 0);

static char mpp_char_val[16] = "00";
module_param_string(vol_mpp_char, mpp_char_val, sizeof(mpp_char_val), 0);

static char mpp_dig_val[16] = "00";
module_param_string(vol_mpp_dig, mpp_dig_val, sizeof(mpp_dig_val), 0);
...
static int __early_param(char *keyword, char *out_buf, char out_len)
{
    char *p = NULL, *q = NULL;
    int write_len = 0;

    p = strstr(final_cmdline, keyword);
    if(!p)
    {   
        printk(KERN_ERR "Can not find string '%s' from cmdline.\n", keyword);
        return -1;
    }
        
    q = strstr(p, " "); // x1=y1 x2=y2.

    if(!q)
    {   
        printk(KERN_ERR "find string '%s' from cmdline, but can not find end of char ' '.\n", keyword);
        return -1;
    }
    write_len = q - (strlen(keyword) + p) + 1;
    if (write_len > out_len)
    {   
		
        printk(KERN_ERR "parse string '%s' out buffer len exceeds, max len=[%d], actual len=[%d].\n", keyword, out_len, write_len);
        return -1;
    }
    
    strlcpy(out_buf, p + strlen(keyword), write_len);
    
    printk(KERN_INFO "parse cmdline, %s%s,  value-len=%d.\n", keyword, out_buf, write_len);

    return 0;
}

static int final_cmdline_init(void)
{
    const char *cmdline = NULL,  *cmdline_ex = NULL;
    ...
    //WARN(strlen(cmdline) > 1800, "Cmdline exceeds 1800 bytes!");
    strlcpy(final_cmdline, cmdline, sizeof(final_cmdline));
...
static int cmdline_param_parse(void)
{
    if (final_cmdline_init() < 0)
    {
        pr_err("%s: final_cmdline_init failed\n", __func__);
        return -1;
    }

    __early_param("vol_mpp_dig=", mpp_dig_val, sizeof (mpp_dig_val));
    __early_param("vol_mpp_char=", mpp_char_val, sizeof (mpp_char_val));
    __early_param("bbk_board_version=", bbk_board_version, sizeof (bbk_board_version));
    __early_param("bbk_model_version=", bbk_model_version, sizeof (bbk_model_version));
...
module_init(bbk_drivers_log_switch_init);
