...
static int fan53870_regulator_enable(struct regulator_dev *rdev)
{
	...
	val = 0x00;
	rc = fan53870_write(fan_reg->regmap, 0x13, &val, 1);
	if (rc < 0) {
		fan53870_err(fan_reg, "failed to write LDO_COMP0 rc=%d\n", rc);
	}

	rc = fan53870_write(fan_reg->regmap, 0x14, &val, 1);
	if (rc < 0) {
...
static int fan53870_initializeGPIO(struct fan53870_chip *chip)
{
	...
	//enable pin
	if (of_parse_phandle(node, "fan53870,enable", 0) != NULL) {
		chip->gpio_enable = of_get_named_gpio(node, "fan53870,enable", 0);
		if (!gpio_is_valid(chip->gpio_enable)) {
			pr_err("FAN53870: failed to get named enable gpio\n");
		} else {
			rc = gpio_request(chip->gpio_enable, "fan53870 enable");
			if (rc < 0) {
				pr_err("FAN53870 could not request GPIO for enable\n");
				return rc;
			}
			gpio_direction_output(chip->gpio_enable, 1);
			mdelay(500);
		}
	} else {
		pr_info("FAN53870: failed to find  fan53870,enable configuration\n");
	}
	pr_info("enable pin");
	...
	if (of_parse_phandle(node, "fan53870,reset", 0) != NULL) {
		...
		if (!gpio_is_valid(chip->gpio_reset)) {
			...
			gpio_direction_output(chip->gpio_enable, 0);
			mdelay(500);
			gpio_direction_output(chip->gpio_enable, 1);
		}
	} else {
