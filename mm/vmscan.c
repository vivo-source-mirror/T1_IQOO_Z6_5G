...
{
#ifdef CONFIG_FCACHE_CTRL
	enum fcache_mode suffice = fctrl_en ? is_sufficient() : FCACHE_INVAL;
#endif
	LIST_HEAD(ret_pages);
		...
		if (fctrl_en) {
			...
			if ((mapping = page_mapping_file(page)) != NULL) {
				...
				if (is_vital_inode(inode)) {
					...
					if (suffice == FCACHE_OK) {
						goto activate_locked;
					} else if (suffice == FCACHE_VITAL) {
						if (!PageVital(page))
							goto activate_locked;
					}
				}
...
#ifdef CONFIG_FCACHE_CTRL
			lru = page_lru(page);
			BUG_ON(lru > LRU_UNEVICTABLE && lru < NR_LRU_LISTS);
#endif
			putback_lru_page(page);
			spin_lock_irq(&pgdat->lru_lock);
			continue;
...
#ifdef CONFIG_FCACHE_CTRL
		if (sc->special) {
			if (lru > LRU_UNEVICTABLE && lru < NR_LRU_LISTS) {
				update_lru_size(lruvec, LRU_ACTIVE_FILE,
						page_zonenum(page), nr_pages);
...
#ifdef CONFIG_FCACHE_CTRL
	move_pages_to_lru(lruvec, sc, &page_list);
#else
			       ...
			       enum lru_list lru)
{
	...
	while (!list_empty(&l_hold)) {
		...
		if (lru == LRU_ACTIVE_FILE && !sc->special && fctrl_en) {
			if (PageCandi(page) && page_is_file_cache(page)) {
...
#ifdef CONFIG_FCACHE_CTRL
	nr_activate = move_pages_to_lru(lruvec, sc, &l_active);

	if (lru == LRU_ACTIVE_FILE && !sc->special) {
		sc->special = LRU_VITAL;
		nr_activate += move_pages_to_lru(lruvec, sc, &l_vital);
#ifdef CONFIG_FCACHE_CTRL_DATA
		sc->special = LRU_STAPLE;
		nr_activate += move_pages_to_lru(lruvec, sc, &l_staple);
#endif
		sc->special = 0;
	}

	nr_deactivate = move_pages_to_lru(lruvec, sc, &l_inactive);

	list_splice(&l_vital, &l_active);
#ifdef CONFIG_FCACHE_CTRL_DATA
	list_splice(&l_staple, &l_active);
#endif
