...
static int blk_mq_wait_token(struct request_queue *q,
		struct bio *bio,
		struct blk_mq_alloc_data *data)
{
	...
	struct sbitmap_queue *sbq = &q->token;

	if (bio_op(bio) == REQ_OP_READ)
		tag = __sbitmap_queue_get(sbq);
	else
		tag = __sbitmap_queue_get_shallow(sbq, q->shallow);
	...
	for (;;) {
		...
		int prio = blkg ? blkg->blkcg->priority : 0;
		...
		if (atomic64_read(&current->dynamic_svp))
			break;
#endif
					  ...
					  struct blk_mq_alloc_data *data)
{
...
#ifdef CONFIG_BLK_FATE_QOS
			if (q->io_trace && !current->fate_lock && bio) {
				struct blkcg_gq *blkg = bio->bi_blkg;
				int prio = blkg ? blkg->blkcg->priority : 0;

				if (prio == 5 && !(data->flags & BLK_MQ_REQ_NOWAIT) &&
					!blk_op_is_passthrough(data->cmd_flags)) {
					int svp = 0;
#ifdef CONFIG_RSC_LOCK_BOOST
					svp = atomic64_read(&current->dynamic_svp);
#endif
					if (!svp)
						token = blk_mq_wait_token(q, bio, data);
				}
			}
#endif
...
#ifdef CONFIG_BLK_FATE_QOS
		if (token != -1)
			sbitmap_queue_clear(&q->token, token, data->ctx->cpu);
#endif
	...
	if (data->flags & BLK_MQ_REQ_INTERNAL)
		atomic_inc(&q->tag_set->sched_inflight[rq_is_sync(rq)]);
	else
		atomic_inc(&q->tag_set->inflight[rq_is_sync(rq)]);
#endif
	return rq;
}
...
#ifdef CONFIG_BLK_FATE_QOS
	if (rq->token != -1)
		sbitmap_queue_clear(&q->token, rq->token, ctx->cpu);
#endif
	if (rq->tag != -1) {
#ifdef BLK_ENHANCEMENT_MQ_ATOMIC
		atomic_dec(&q->tag_set->inflight[rq_is_sync(rq)]);
#endif
		blk_mq_put_tag(hctx, hctx->tags, ctx, rq->tag);
	}
	if (sched_tag != -1) {
#ifdef BLK_ENHANCEMENT_MQ_ATOMIC
		atomic_dec(&q->tag_set->sched_inflight[rq_is_sync(rq)]);
#endif
...
#ifdef BLK_ENHANCEMENT_MQ_ATOMIC
		atomic_inc(&rq->q->tag_set->inflight[rq_is_sync(rq)]);
#endif
#ifdef CONFIG_BLK_ENHANCEMENT
					...
					unsigned long msecs)
{
	...
	if (1) {
		int cpu = get_cpu();
		hctx->last_queue[cpu] = ktime_get_ns();
		put_cpu();
	}
#endif
...
static void blk_mq_run_work_fn(struct work_struct *work)
{
	...
	if (1) {
		int cpu = get_cpu();
		hctx->last_exec[cpu] = ktime_get_ns();
		put_cpu();
	}
#endif
...
#ifdef CONFIG_BLK_FATE_QOS
	if (sbitmap_queue_init_node(&q->token,
			10, -1, false,
			GFP_KERNEL, q->node)) {
		goto err_hctxs;
	} else {
		unsigned int shift = q->token.sb.shift;
		q->shallow = max((1U << shift) * 60 / 100U, 1U);
		sbitmap_queue_min_shallow_depth(&q->token, q->shallow);
	}
#endif
