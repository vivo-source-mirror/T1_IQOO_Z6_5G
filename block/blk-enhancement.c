...
int proc_fate_show(struct seq_file *m, struct pid_namespace *ns,
		     struct pid *pid, struct task_struct *tsk)
{
	char *buf;
	int retval;
	union fate_lock *info;
	...
	info = (union fate_lock *)&tsk->fate_lock;

	snprintf(buf, PATH_MAX, " %10s:  %d\n"
		" %10s:  %d\n"
		" %10s:  %d\n",
		"MUTEX", info->b.mutex,
		"RWSEM_R", info->b.rwsem_r,
		"RWSEM_W", info->b.rwsem_w);
