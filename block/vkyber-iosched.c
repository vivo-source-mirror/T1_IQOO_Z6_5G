...
enum {
	 ...
	 * operations.
	 */
	KYBER_WRITE_PERCENT = 65,
	KYBER_ASYNC_PERCENT = 50,
};
				...
				unsigned int percentile)
{
	...
	trace_vkyber_latency(kqd->q, kyber_domain_names[sched_domain],
				...
				unsigned int sched_domain, unsigned int depth)
{
	...
	if (depth != kqd->domain_tokens[sched_domain].sb.depth) {
		...
		trace_vkyber_adjust(kqd->q, kyber_domain_names[sched_domain],
				   depth);
	}
}
...
static struct kyber_queue_data *kyber_queue_data_alloc(struct request_queue *q)
{
	...
	kqd->write_depth = (1U << shift) * KYBER_WRITE_PERCENT / 100U;
...
static void kyber_limit_depth(unsigned int op, struct blk_mq_alloc_data *data)
{
	...
	if (!op_is_sync(op)) {
		...
		data->shallow_depth = kqd->write_depth;
	}
}
...
static struct elevator_type kyber_sched = {
	...
	.elevator_name = "vkyber",
	.elevator_owner = THIS_MODULE,
};

static int __init vkyber_init(void)
{
	return elv_register(&kyber_sched);
}

static void __exit vkyber_exit(void)
{
	elv_unregister(&kyber_sched);
}

module_init(vkyber_init);
module_exit(vkyber_exit);
