				     ...
				     struct dsi_regulator_info *regs)
{
	...
	dsi_for_each_child_node(root, node) {
		...
		DSI_INFO("[%s] minv=%d maxv=%d, en_load=%d, dis_load=%d\n",
...
/**
 * dsi_pwr_enable_vreg_by_name() - enable/disable single regulator by name
 */
int dsi_pwr_enable_vreg_by_name(struct dsi_regulator_info *regs, bool enable, char *vreg_name)
{
	int rc = 0, i = 0;
	...
	if (!vreg_name || ! regs || !regs->count || !regs->vregs) {
		pr_err("param error.");
		return -EINVAL;
	}
	if (enable) {
		for (i = 0; i < regs->count; i++) {
			...
			if (strcmp(vreg->vreg_name,vreg_name)) {
				pr_info("%s:vreg->vreg_name=%s,mismatch the special name(%s)", __func__, vreg->vreg_name, vreg_name);
				continue;
			}
			...
			if (!regulator_is_enabled(vreg->vreg)) {
				...
				DSI_INFO("vreg_name(%s) already enable.");
			}

			pr_info("%s:vreg_name(%s) set %d success", __func__, vreg_name, enable);
		...
		for (i = (regs->count - 1); i >= 0; i--) {
			...
			(void)regulator_set_load(regs->vregs[i].vreg,
						regs->vregs[i].disable_load);
			while (regulator_is_enabled(vreg->vreg)) {
...
EXPORT_SYMBOL(dsi_pwr_enable_vreg_by_name);
