...
#define PANEL_BOARD_POWER_CTRL


extern const char *cmd_set_state_map[];
extern int dsi_pwr_enable_vreg_by_name(struct dsi_regulator_info *regs, bool enable, char *vreg_name);
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->reset_gpio)) {
		...
		if (rc) {
			DSI_ERR("request for reset_gpio failed, rc=%d,gpio %d\n", rc,r_config->reset_gpio);
			goto error;
		}
	}
...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->reset_config.vddr_en_gpio)) {
		rc = gpio_direction_output(panel->reset_config.vddr_en_gpio, 1);
		if (rc) {
			DSI_ERR("unable to set dir for disp gpio rc=%d\n", rc);
			goto exit;
		}
		usleep_range(13*1000, (13*1000)+100);
	}
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	pr_info("%s:%s power on", __func__,panel->type);
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	pr_info("%s:%s power off", __func__,panel->type);
	if (gpio_is_valid(panel->reset_config.disp_en_gpio))
		gpio_set_value(panel->reset_config.disp_en_gpio, 0);
	...
	if (gpio_is_valid(panel->reset_config.vddr_en_gpio))
		gpio_set_value(panel->reset_config.vddr_en_gpio, 0);
...
void incell_tp_reset_output(struct dsi_panel *panel, bool enable)
{
	...
	u32 bl_lvl)
{
	...
	DSI_INFO("dsi_panel_update_backlight %d",bl_lvl);
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	if (!gpio_is_valid(panel->reset_config.reset_gpio) &&
		...
		DSI_ERR("[%s] succed get reset gpio, reset_gpio=%d\n", panel->name, panel->reset_config.reset_gpio);
	...
	panel->reset_config.vddr_en_gpio = utils->get_named_gpio(utils->data,
				"qcom,platform-vddr-gpio", 0);
	if (!gpio_is_valid(panel->reset_config.vddr_en_gpio))
		DSI_INFO("[%s] failed get vddr_en_gpio, rc=%d\n", panel->name, rc);
	DSI_INFO("[%s]:qcom,platform-vddr-gpio=%d", panel->name, panel->reset_config.vddr_en_gpio);
	...
	if (!gpio_is_valid(panel->reset_config.disp_enp_gpio))
		DSI_INFO("[%s] failed get disp_enp_gpio, rc=%d\n", panel->name, rc);
				...
				struct dsi_parser_utils *utils)
{
	...
	DSI_INFO("dsc slice_height %d\n",priv_info->dsc.config.slice_height);
