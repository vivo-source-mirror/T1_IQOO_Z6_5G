...
#include "vivo/tas2562/tas2562.h"
...
/*audio_v add*/
#define KEY_IMUSIC_COLLECT      0x2f8 /* Key(760) to IMUSIC_COLLECT  move from include/uapi/linux/input-event-codes.h  */
...
/*audio_v add*/
struct msm_soc_dynamic_dai_links {
	const char *id;
	struct snd_soc_dai_link dai_link;
};

static int msm_mi2s_enable(bool enable);

extern unsigned int is_atboot;
struct msm_asoc_mach_data *vivo_pdata = NULL;
static struct device *msm_i2s_dev = NULL;
static int vivo_clk_mi2s_id = -1;
static unsigned int vivo_clk_on = 0;
static int wcd_hifi_mode_switch = 0;
static int vivo_fm_mode_switch = 0;
static int fm_to_spk = 0;
static int is_mostest = 0;
/*audio_v add end*/
enum {
	TDM_0 = 0,
	TDM_1,
...
struct mi2s_conf {
	...
	u32 mi2s_sample_rate_rule;
	DECLARE_BITMAP(status_mask, MI2S_STATUS_MAX);
};
...
static u32 mi2s_ibit_clk[MI2S_MAX] = {
	Q6AFE_LPASS_CLK_ID_PRI_MI2S_IBIT,
	Q6AFE_LPASS_CLK_ID_SEC_MI2S_IBIT,
	Q6AFE_LPASS_CLK_ID_TER_MI2S_IBIT,
	Q6AFE_LPASS_CLK_ID_QUAD_MI2S_IBIT,
...
static unsigned int mi2s_sample_rate_validation(unsigned int sample_rate)
{
	switch (sample_rate) {
		...
		case SAMPLING_RATE_384KHZ:
			return sample_rate;
		default:
			pr_warn("%s: unsupported sample rate %u, setting to default 48kHz.\n",
...
static struct dev_config cdc_dma_rx_cfg[] = {
	[RX_CDC_DMA_RX_0] = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 2},
	[RX_CDC_DMA_RX_1] = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 2},
	[RX_CDC_DMA_RX_2] = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 2},
	[RX_CDC_DMA_RX_3] = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 2},
	[RX_CDC_DMA_RX_5] = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 2},
	[RX_CDC_DMA_RX_6] = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S24_LE, 2},
};
...
static char const *mi2s_mode_text[] = {"Master-Fixed", "Master", "Slave", "Undefine", "Slave_DSD"};
static const char *const hp_sel_text[] = {"Normal", "HiFi"};
static char const *switch_text[] = {"OFF", "ON"};
static char const *is_mostest_text[] = {"false", "true"};
static char const *switch_text_fm_mode[] = {"SPK", "FM"};
...
static SOC_ENUM_SINGLE_EXT_DECL(mi2s_mode_sel, mi2s_mode_text);
static SOC_ENUM_SINGLE_EXT_DECL(hp_sel_enum, hp_sel_text);
static SOC_ENUM_SINGLE_EXT_DECL(vivo_mi2s_clock_switch, switch_text);
static SOC_ENUM_SINGLE_EXT_DECL(vivo_is_mostest, is_mostest_text);
static SOC_ENUM_SINGLE_EXT_DECL(fm_mode_switch, switch_text_fm_mode);
...
#define BTN_NUM				8
static u16 mbhc_btn_defs[BTN_NUM] = {100, 263, 463, 463,
									 463, 463, 463, 463};
...
static struct wcd_mbhc_config wcd_mbhc_cfg = {
	...
	.key_code[1] = KEY_IMUSIC_COLLECT, /*audio_v modify*/
	...
	.key_code[7] = 0,
	.linein_th = 5000,
	.moisture_en = true, /*audio_v modify*/
	...
	/*audio_v add*/
	.gnd_det_en = false,
	.get_hp_switch_state = NULL,
	.set_hp_switch_state = NULL,
	.get_hifi_dac_state = NULL,
	.smartpa_debug_retry_probe = NULL,
	.imped_detect_en = false,
	.imped_adjust_ref = 10,
	.hp_impedance = NULL,
	.mbhc_detect_moisture = false,
	/*audio_v add end*/
};
...
/*audio_v: archer add for max20328 or fsa4480 mos test*/
static int dl_mos_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = is_mostest;
	pr_info("%s: is_mostest = %d\n", __func__,
			(int)(ucontrol->value.enumerated.item[0]));

	return 0;
}

static int dl_mos_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	is_mostest = ucontrol->value.integer.value[0];
	pr_info("%s: is_mostest = %d\n", __func__,
		is_mostest);
	max20328_notify_mos(is_mostest);

	return 0;
}
/*audio_v: archer add for max20328 or fsa4480 mos test end*/
...
static int mi2s_get_port_idx(struct snd_kcontrol *kcontrol)
{
	...
	if (strnstr(kcontrol->id.name, "PRIM_MI2S_RX",
		...
		idx = TERT_MI2S;
		 ...
		 sizeof("QUAT_MI2S_TX"))) {
		idx = QUAT_MI2S;
	} else if (strnstr(kcontrol->id.name, "PRIM_MI2S",
		 sizeof("PRIM_MI2S"))) {
		idx = PRIM_MI2S;
...
{
	int idx = mi2s_get_port_idx(kcontrol);
	return 0; /* unsupport to set for user */
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (ucontrol->value.integer.value[0]) {
	case 3:	// modify by jackie for wcd codec not support 32bit
	case 2:
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	mi2s_intf_conf[idx].mi2s_sample_rate_rule =
		ucontrol->value.enumerated.item[0];

	pr_info("%s: idx[%d]_mode = %d, item = %d\n", __func__,
		 idx, mi2s_intf_conf[idx].mi2s_sample_rate_rule,
		 ucontrol->value.enumerated.item[0]);
...
static int vivo_ph_sel_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!fun) {
		pr_err("%s: vivo_codec_function is NULL\n", __func__);
		return 0;
	}

	if (fun->get_hp_switch_state)
		ucontrol->value.integer.value[0] =
				fun->get_hp_switch_state();
	return 0;
}

static int vivo_ph_sel_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct vivo_codec_function *fun = get_vivo_codec_function();
	int hp_sel_val;
	...
	hp_sel_val = ucontrol->value.integer.value[0] ? HP_PATH_NORMAL : HP_PATH_HIFI;
	if (fun->set_hp_switch_state)
		fun->set_hp_switch_state(hp_sel_val);
	return 0;
}

static int vivo_mi2s_clk_get(struct snd_kcontrol *kcontrol,
                             struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s: vivo clock %d ", __func__,vivo_clk_on);
	ucontrol->value.integer.value[0] = vivo_clk_on;
	return 0;
}

static int vivo_mi2s_clk_put(struct snd_kcontrol *kcontrol,
                             struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	vivo_clk_on = !!ucontrol->value.integer.value[0];
	pr_info("%s: vivo clock = %d\n", __func__, vivo_clk_on);
	ret = msm_mi2s_enable(vivo_clk_on);
	pr_info("%s: end \n", __func__);
	return ret;
}

static int wcd_hifi_mode_get(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = wcd_hifi_mode_switch;
	pr_info("%s: %d\n", __func__,
		(int)(ucontrol->value.enumerated.item[0]));
	return 0;
}

static int wcd_hifi_mode_put(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
	wcd_hifi_mode_switch = ucontrol->value.integer.value[0];
	pr_info("%s: %d\n", __func__,
		(int)(ucontrol->value.integer.value[0]));
	return 0;
}

static int vivo_fm_mode_get(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = vivo_fm_mode_switch;
	pr_info("%s: vivo_fm_mode_switch = %d\n", __func__,
		(int)(ucontrol->value.enumerated.item[0]));
	return 0;
}

static int vivo_fm_mode_put(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
	struct vivo_codec_function *fun = get_vivo_codec_function();
	...
	vivo_fm_mode_switch = ucontrol->value.integer.value[0];
	pr_info("%s: vivo_fm_mode_switch = %d\n", __func__,
		vivo_fm_mode_switch);

	if (fun && fun->ext_pa_enable) {
		usleep_range(1000, 1050);

		if (vivo_fm_mode_switch)
			fun->ext_pa_enable(EXT_PA_SWICH_SPK_FM);
		else if (fm_to_spk) {
			fun->ext_pa_enable(EXT_PA_SWICH_SPK);
		}
	}
	return 0;
}

static int usb_audio_samplerate_cfg[20] = {0};

static int usb_audio_samplerate_validation(int sample_rate)
{
	int i = 0;

	if (!usb_audio_samplerate_cfg[0]) {
		pr_info("%s: usb not to set rate, choose default rate 48000\n", __func__);
		return SAMPLING_RATE_48KHZ;
	}
	for (i = 1; i < (usb_audio_samplerate_cfg[0] + 1); i++) {
		if (sample_rate == usb_audio_samplerate_cfg[i]) {
			pr_info("%s: rate %d usb supported\n", __func__, usb_audio_samplerate_cfg[i]);
			return sample_rate;
		}
	}
	pr_info("%s: rate %d usb unsupported, choose usb rate %d\n",
		__func__, sample_rate, usb_audio_samplerate_cfg[1]);
	return usb_audio_samplerate_cfg[1];
}

static int usb_audio_samplerate_cfg_get(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int usb_audio_samplerate_cfg_put(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
	int i = 0;
	/* usb_sample_rates_cfg[0]: count of the samplerate.
	 * usb_sample_rates_cfg[i]: i > 0, samplerate value of usb support.
	 * usb_sample_rates_cfg[1]: is the max samplerate.
	 */
	usb_audio_samplerate_cfg[0] = ucontrol->value.integer.value[0];
	pr_info("%s: rate size = %d\n", __func__, usb_audio_samplerate_cfg[0]);
	for (i = 1; i < (usb_audio_samplerate_cfg[0] + 1); i++) {
		usb_audio_samplerate_cfg[i] = ucontrol->value.integer.value[i];
		pr_info("%s: rate %d\n", __func__, usb_audio_samplerate_cfg[i]);
	}
	return 0;
}
...
static const struct snd_kcontrol_new msm_int_snd_controls[] = {
	...
	SOC_ENUM_EXT("VA_CDC_DMA_TX_2 SampleRate",
			...
			cdc_dma_tx_sample_rate_get,
			cdc_dma_tx_sample_rate_put),
	SOC_SINGLE_MULTI_EXT("USB_AUDIO SampleRate Config", SND_SOC_NOPM, 0, 384000, 0, 20,
			usb_audio_samplerate_cfg_get, usb_audio_samplerate_cfg_put),
};
...
static const struct snd_kcontrol_new msm_common_snd_controls[] = {
	...
	SOC_ENUM_EXT("PRIM_MI2S Mode", mi2s_mode_sel,
			mi2s_mode_get, mi2s_mode_put),
	SOC_ENUM_EXT("SEC_MI2S Mode", mi2s_mode_sel,
			mi2s_mode_get, mi2s_mode_put),
	SOC_ENUM_EXT("TERT_MI2S Mode", mi2s_mode_sel,
			mi2s_mode_get, mi2s_mode_put),
	SOC_ENUM_EXT("QUAT_MI2S Mode", mi2s_mode_sel,
			mi2s_mode_get, mi2s_mode_put),
	SOC_ENUM_EXT("QUIN_MI2S Mode", mi2s_mode_sel,
			mi2s_mode_get, mi2s_mode_put),
	SOC_ENUM_EXT("HP Sel", hp_sel_enum,
			vivo_ph_sel_get, vivo_ph_sel_put),
	SOC_ENUM_EXT("WCD HiFi Mode Switch", hp_sel_enum,
			wcd_hifi_mode_get, wcd_hifi_mode_put),
	SOC_ENUM_EXT("Vivo MI2S Clock", vivo_mi2s_clock_switch,
			vivo_mi2s_clk_get, vivo_mi2s_clk_put),
	SOC_ENUM_EXT("SPK_FM_mode", fm_mode_switch,
			vivo_fm_mode_get, vivo_fm_mode_put),
};
...
/*audio_v add for max20328 or fsa4480 mos test*/
static const struct snd_kcontrol_new DL_MOS_TEST[] = {
	SOC_ENUM_EXT("DL_MOS_TEST",vivo_is_mostest,
			dl_mos_get,dl_mos_put),
};
/*audio_v add for max20328 or fsa4480 mos test end*/
				...
				struct snd_pcm_hw_params *params)
{
	...
	case MSM_BACKEND_DAI_USB_RX:
		...
		rate->min = rate->max = usb_audio_samplerate_validation(params_rate(params));
	...
	case MSM_BACKEND_DAI_QUATERNARY_MI2S_TX:
		...
		mi2s_rate = mi2s_sample_rate_validation(params_rate(params));
		switch(mi2s_intf_conf[index].mi2s_sample_rate_rule) {
		case MI2S_MODE_MASTER:
			fmt = SND_SOC_DAIFMT_CBS_CFS;
			break;
		case MI2S_MODE_SLAVE:
			fmt = SND_SOC_DAIFMT_CBM_CFM;
			break;
		case MI2S_MODE_UNDEFINE:
			if (!(mi2s_rate % SAMPLING_RATE_48KHZ)) {
				fmt = SND_SOC_DAIFMT_CBS_CFS;
			} else {
				fmt = SND_SOC_DAIFMT_CBM_CFM;
			}
			break;
		case MI2S_MODE_SLAVE_DSD:
			fmt = SND_SOC_DAIFMT_CBM_CFM;
			mi2s_rate = SAMPLING_RATE_176P4KHZ;
			break;
		case MI2S_MODE_MASTER_FIXED:
		default:
			fmt = SND_SOC_DAIFMT_CBS_CFS;
			mi2s_rate = SAMPLING_RATE_48KHZ;
			break;
		}
		/**
		 * for each mi2s interface, for tx and rx, bit_format
		 * and sample_rate neee to be keep the same value, only
		 * allow to set different channels.
		 */
		rate->min = rate->max = mi2s_rate;
		param_set_mask(params, SNDRV_PCM_HW_PARAM_FORMAT,
				   mi2s_rx_cfg[index].bit_format);

		if (dai_link->dpcm_playback) {
			channels->min = channels->max =
				mi2s_rx_cfg[index].channels;
		} else if (dai_link->dpcm_capture) {
			channels->min = channels->max =
				mi2s_tx_cfg[index].channels;
		}

		pr_info("%s mi2s_mode %d fmt 0x%x\n", __func__,
				mi2s_intf_conf[index].mi2s_sample_rate_rule,
				fmt);
		if (!(test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
			test_bit(MI2S_TX_STARTED, mi2s_intf_conf[index].status_mask) ||
			test_bit(MI2S_CAL_STARTED, mi2s_intf_conf[index].status_mask))) {
			if (fmt == SND_SOC_DAIFMT_CBM_CFM) {
				mi2s_clk[index].clk_id = mi2s_ebit_clk[index];
			} else {
				mi2s_clk[index].clk_id = mi2s_ibit_clk[index];
			}

			rc = snd_soc_runtime_set_dai_fmt(rtd, fmt);
			if (rc < 0) {
				dev_err(rtd->card->dev,
					"%s: set fmt for codec dai(s) failed, err:%d\n",
					__func__, rc);
			}
		} else {
			pr_info("%s: BE DAI MI2S ID(%d) is already running\n",
				__func__, index);
			/* if i2s clk enabled for calibration, set to 48kHz/24bit/master */
			if (!(test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
			test_bit(MI2S_TX_STARTED, mi2s_intf_conf[index].status_mask)) &&
			test_bit(MI2S_CAL_STARTED, mi2s_intf_conf[index].status_mask)) {
				rate->min = rate->max = SAMPLING_RATE_48KHZ;
				rc = snd_soc_runtime_set_dai_fmt(rtd, SND_SOC_DAIFMT_CBS_CFS);
				if (rc < 0) {
					dev_err(rtd->card->dev,
						"%s: set fmt for codec dai(s) failed, err:%d\n",
						__func__, rc);
				  ...
				  bool active)
{
	...
	return max20328_switch_event(pdata->fsa_handle, FSA_MIC_GND_SWAP); /*audio_v add*/
}
...
static int msm_mi2s_snd_prepare(struct snd_pcm_substream *substream)
{
	int ret = 0;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
	...
	if (!(test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
			test_bit(MI2S_TX_STARTED, mi2s_intf_conf[index].status_mask) ||
			test_bit(MI2S_CAL_STARTED, mi2s_intf_conf[index].status_mask))) {
		mi2s_rx_cfg[index].sample_rate = cpu_dai->rate;
		...
		set_bit(substream->stream, mi2s_intf_conf[index].status_mask);
	} else {
		set_bit(substream->stream, mi2s_intf_conf[index].status_mask);
		pr_info("%s: BE DAI MI2S ID(%d) is already running.\n",
			__func__, index);
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	/**
	 * MI2S_RX_STARTED = SNDRV_PCM_STREAM_PLAYBACK,
	 * MI2S_TX_STARTED = SNDRV_PCM_STREAM_CAPTURE,
	 * so use substream->stream to simplify the code.
	 */
	clear_bit(substream->stream, mi2s_intf_conf[index].status_mask);
	if (!(test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
		test_bit(MI2S_TX_STARTED, mi2s_intf_conf[index].status_mask) ||
		test_bit(MI2S_CAL_STARTED, mi2s_intf_conf[index].status_mask))) {
		if (pdata->mi2s_gpio_p[index]) {
		...
		pr_info("%s: MI2S DAI(%d) can not be shutdown.\n",
				__func__, index);
	}
...
static int vivo_ext_pa_enable_event(struct snd_soc_dapm_widget *w,
			       struct snd_kcontrol *k, int event)
{
	struct vivo_codec_function *function = get_vivo_codec_function();

	pr_info("%s: event = %d\n", __func__, event);
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		if (function && function->ext_pa_enable) {
			function->ext_pa_enable(EXT_PA_SWICH_SPK);
			usleep_range(20 * 1000, 20 * 1050);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		if (function && function->ext_pa_enable) {
			usleep_range(10, 11);
			function->ext_pa_enable(EXT_PA_SWICH_NONE);
		}
		break;
	default:
		return -EINVAL;
...
static const char * const spke_text[] = {
	"ZERO", "Switch",
};
static const struct soc_enum spk_ext_enum =
	SOC_ENUM_SINGLE(SND_SOC_NOPM, 0, ARRAY_SIZE(spke_text),
	spke_text);
static const struct snd_kcontrol_new spkextpa_mux[] = {
	SOC_DAPM_ENUM("SPK Ext Switch", spk_ext_enum)
};
...
static const struct snd_soc_dapm_widget msm_int_dapm_widgets[] = {
	...
	SND_SOC_DAPM_DAC_E("SPK Ext PA", NULL, SND_SOC_NOPM, 0, 0,
				vivo_ext_pa_enable_event,
				SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
				SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX("SPK Ext Switch",
		SND_SOC_NOPM, 0, 0, spkextpa_mux),
	SND_SOC_DAPM_OUTPUT("SPK Ext OUT"),
};

static const struct snd_soc_dapm_route audio_ana_route[] = {
	{"SPK Ext OUT", NULL, "SPK Ext PA"},
	{"SPK Ext PA", NULL, "SPK Ext Switch"},
	{"SPK Ext Switch", "Switch", "AUX PGA"},
};
...
static void *def_wcd_mbhc_cal(void)
{
	...
	/* set btn threshold */
	for (i = 0; i < BTN_NUM; i++) {
		btn_high[i] = mbhc_btn_defs[i];
		pr_debug("%s btn_high[%d] %d.\n", __func__, i, btn_high[i]);
	}
#endif
...
static int vivo_mi2s_clk_enable(bool enable, int port_id)
{
	int ret = 0;
	u32 index = (port_id & 0xf) >> 1;
	...
	if (enable) {
		mi2s_clk[index].clk_freq_in_hz = 48000 * 2 * 32;
	}
	mi2s_clk[index].enable = enable;
	mi2s_clk[index].clk_id = mi2s_ibit_clk[index];
	ret = afe_set_lpass_clock_v2(port_id,
	                             &mi2s_clk[index]);
	if (ret < 0) {
		pr_err("%s: afe lpass clock failed for port 0x%x , err:%d\n",
		       __func__, port_id, ret);
	}
...
static int msm_mi2s_enable(bool enable)
{
	...
	int ret = 0, port_id = 0, index = 0;
	port_id = msm_get_port_id(vivo_clk_mi2s_id);
	index = (port_id & 0xf) >> 1;
	...
	if (enable) {
		if (!(test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
			...
			ret = vivo_mi2s_clk_enable(true, port_id);
			if (ret < 0) {
				pr_err("%s failed to enable sclk\n", __func__);
				goto end;
			}
			if (vivo_pdata && vivo_pdata->mi2s_gpio_p[index]) {
				msm_cdc_pinctrl_select_active_state(
						vivo_pdata->mi2s_gpio_p[index]);
			} else if (msm_i2s_dev) {
				pinctrl_pm_select_default_state(msm_i2s_dev);
				pr_info("%s pinctrl has been set\n", __func__);
			}
			...
			if (afe_port_start(port_id,
			                   &port_config, 48000) < 0) {
				pr_err("%s:Fail to open AFE port\n", __func__);
				ret = -EINVAL;
				goto err_clk;
			}
		} else if (test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
		           test_bit(MI2S_TX_STARTED, mi2s_intf_conf[index].status_mask)) {
			if (mi2s_clk[index].clk_freq_in_hz != 3072000 ||
			    mi2s_clk[index].clk_id != mi2s_ibit_clk[index]) {
				pr_err("%s: Playback/Capture is active and sample rate is not 48kHz.\n",
				       __func__);
				ret = -EINVAL;
				goto end;
			}
		}
		set_bit(MI2S_CAL_STARTED, mi2s_intf_conf[index].status_mask);
	} else {
		if (test_bit(MI2S_CAL_STARTED, mi2s_intf_conf[index].status_mask)) {
			if (test_bit(MI2S_RX_STARTED, mi2s_intf_conf[index].status_mask) ||
			    test_bit(MI2S_TX_STARTED, mi2s_intf_conf[index].status_mask)) {
				clear_bit(MI2S_CAL_STARTED, mi2s_intf_conf[index].status_mask);
				pr_info("%s: BE DAI(%d) can not be shutdown.\n", __func__, index);
				goto end;
			}
			if (afe_close(port_id) < 0) {
			...
			if (vivo_pdata && vivo_pdata->mi2s_gpio_p[index]) {
				...
				pinctrl_pm_select_sleep_state(msm_i2s_dev);
			ret = vivo_mi2s_clk_enable(false, port_id);
			if (ret < 0) {
				pr_err("%s failed to enable sclk\n", __func__);
			}
...
int vivo_tas2562_rx_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_component *component =
				snd_soc_rtdcom_lookup(rtd, TAS2562_CODEC);
	struct tas2562_priv *pTAS2562 = NULL;
	...
	pTAS2562 = snd_soc_component_get_drvdata(component);
	if (!pTAS2562) {
		pr_err("%s: priv data is NULL\n", __func__);
		return 0;
	}

	pTAS2562->mi2s_rx_port_id = msm_get_port_id(rtd->dai_link->id);
	pTAS2562->mi2s_tx_port_id = pTAS2562->mi2s_rx_port_id + 1;
	pr_info("%s: mi2s_rx_port_id: %d, mi2s_tx_port_id: %d.\n",
			__func__, pTAS2562->mi2s_rx_port_id, pTAS2562->mi2s_tx_port_id);
	smartpa_set_mi2s_port(pTAS2562->mi2s_rx_port_id, pTAS2562->mi2s_tx_port_id);
...
/* Add for vivo hostless @zgbing */
static struct snd_soc_dai_link vivo_hostless_dai[] = {
	{/* hw:x,42 */
...
static struct msm_soc_dynamic_dai_links vivo_dynamic_dai_links[] = {
	{
		.id = "qcom,mi2s-sec-tas-spk-rx",
		.dai_link = {
			.name = LPASS_BE_SEC_MI2S_RX,
			.stream_name = "Secondary MI2S Playback",
			.no_pcm = 1,
			...
			.id = MSM_BACKEND_DAI_SECONDARY_MI2S_RX,
			.be_hw_params_fixup = msm_be_hw_params_fixup,
			.init = vivo_tas2562_rx_init,
			.ops = &msm_mi2s_be_ops,
			.ignore_suspend = 1,
			.ignore_pmdown_time = 1,
			SND_SOC_DAILINK_REG(sec_tas_rx),
		}
	},
	{
		.id = "qcom,mi2s-sec-tas-spk-tx",
		.dai_link = {
			.name = LPASS_BE_SEC_MI2S_TX,
			.stream_name = "Secondary MI2S Capture",
			.no_pcm = 1,
			...
			.ops = &msm_mi2s_be_ops,
			.ignore_suspend = 1,
			SND_SOC_DAILINK_REG(sec_tas_tx),
		}
	},
};
...
static struct snd_soc_dai_link msm_holi_dai_links[
			...
			ARRAY_SIZE(vivo_hostless_dai) +
			ARRAY_SIZE(vivo_dynamic_dai_links)];
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	const char *propname = "qcom,msm-dynamic-dai-links";
	int num_dy_dailinks, num_dailinks;
	int i, j, found;
	const char *name;
	...
	if (!strcmp(match->data, "codec")) {
		...
		memcpy(msm_holi_dai_links + total_links,
		       vivo_hostless_dai,
		       sizeof(vivo_hostless_dai));
		total_links += ARRAY_SIZE(vivo_hostless_dai);
		memcpy(msm_holi_dai_links + total_links,
	...
	/* add for dynamic dai links @zgbing */
	num_dy_dailinks = ARRAY_SIZE(vivo_dynamic_dai_links);
	num_dailinks = of_property_count_strings(dev->of_node, propname);
	if (num_dailinks < 0) {
		dev_info(dev, "%s(): Property '%s' does not exist\n",
			__func__, propname);
		goto exit;
	}
	for (i = 0; i < num_dailinks; i++) {
		rc = of_property_read_string_index(dev->of_node,
			propname, i, &name);
		if (rc < 0) {
			dev_err(dev, "%s(): Index %d could not be read\n",
				__func__, i);
			break;
		}
		for (found = 0,j = 0; j < num_dy_dailinks; j++) {
			if (!strcmp(vivo_dynamic_dai_links[j].id, name)) {
				memcpy(dailink + total_links,
					&vivo_dynamic_dai_links[j].dai_link,
...
static int msm_int_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_add_routes(dapm, audio_ana_route,
					ARRAY_SIZE(audio_ana_route));
	...
	snd_soc_dapm_ignore_suspend(dapm, "SPK Ext PA");
	snd_soc_dapm_ignore_suspend(dapm, "SPK Ext OUT");
...
#ifdef CONFIG_BBK_DRIVER_INFO
static void vivo_parse_snd_info(const struct device_node *np)
{
	unsigned int vivo_snd_info = 0, vivo_fm_pcm_info = 0;
	unsigned int mic_num, spa_driver_type, spa_num = 0, spa_manufacturer,
		ktv_solution, fm_pcm_spk, fm_pcm_hp;

	/* HiFi infomation */
	if (of_find_property(np, "vivo,hifi-support", NULL))
		vivo_snd_info |= 1 << BBK_SND_HIFI_SUPPORT_SHIFT;
	if (of_find_property(np, "vivo,hifi-always-on", NULL)) {
		vivo_snd_info |= 1 << BBK_SND_HIFI_ALWAYS_ON_SHIFT;
		wcd_mbhc_cfg.imped_detect_en = false;
	}
	if (of_find_property(np, "vivo,hifi-always-with-switch", NULL)) {
		vivo_snd_info |= 1 << BBK_SND_HIFI_ALWAYS_WITH_SWITCH_SHIFT;
		wcd_mbhc_cfg.imped_detect_en = false;
	}
	if (of_find_property(np, "vivo,hifi-double-dac", NULL))
		vivo_snd_info |= 1 << BBK_SND_HIFI_DOUBLE_DAC_SHIFT;
	/* ANC mic */
	if (of_find_property(np, "vivo,anc-mic-support", NULL))
		vivo_snd_info |= 1 << BBK_SND_ANC_MIC_SUPPORT_SHIFT;
	/* KTV */
	if (of_find_property(np, "vivo,ktv-support", NULL))
		vivo_snd_info |= 1 << BBK_SND_KTV_SUPOORT_SHIFT;
	/* FM */
	if (of_find_property(np, "vivo,fm-support", NULL))
		vivo_snd_info |= 1 << BBK_SND_FM_SUPPORT_SHIFT;
	/* Builtin Mic */
	if (of_find_property(np, "vivo,builtin-mic-num", NULL)) {
		of_property_read_u32(np,
			"vivo,builtin-mic-num", &mic_num);
		vivo_snd_info |= mic_num << BBK_SND_BUILTIN_MIC_NUM_SHIFT;
	}
	/* smart PA */
	if (of_find_property(np, "vivo,spa-num", NULL)) {
		of_property_read_u32(np,
			"vivo,spa-num", &spa_num);
		vivo_snd_info |= spa_num << BBK_SND_SMARTPA_NUM_SHIFT;
	}
	if (spa_num) {
		if (of_find_property(np, "vivo,spa-driver-type", NULL)) {
			of_property_read_u32(np,
				"vivo,spa-driver-type", &spa_driver_type);
			vivo_snd_info |= spa_driver_type << BBK_SND_PA_DRIVER_TYPE_SHIFT;
		}
		if (of_find_property(np, "vivo,spa-manufacturer", NULL)) {
			of_property_read_u32(np,
				"vivo,spa-manufacturer", &spa_manufacturer);
			vivo_snd_info |= spa_manufacturer << BBK_SND_PA_MANUFACTURER_SHIFT;
		}
	}
	/* KTV */
	if (vivo_snd_info & BBK_SND_KTV_SUPPORT_MASK) {
		if (of_find_property(np, "vivo,ktv-solution", NULL)) {
			of_property_read_u32(np,
				"vivo,ktv-solution", &ktv_solution);
			vivo_snd_info |= ktv_solution << BBK_SND_KTV_SOLUTION_SHIFT;
		}
	}
	/* FM pcm */
	if (vivo_snd_info & BBK_SND_FM_SUPPORT_MASK) {
		if (of_find_property(np, "vivo,fm-hostless-pcm-spk", NULL)) {
			of_property_read_u32(np,
				"vivo,fm-hostless-pcm-spk", &fm_pcm_spk);
			vivo_fm_pcm_info |= fm_pcm_spk << BBK_SND_FM_PCM_SPK_SHIFT;
		}
		if (of_find_property(np, "vivo,fm-hostless-pcm-hp", NULL)) {
			of_property_read_u32(np,
				"vivo,fm-hostless-pcm-hp", &fm_pcm_hp);
			vivo_fm_pcm_info |= fm_pcm_hp << BBK_SND_FM_PCM_HP_SHIFT;
		}
	}

	/* detect moisture */
	if (of_find_property(np, "vivo,mbhc-detect-moisture", NULL))
		wcd_mbhc_cfg.mbhc_detect_moisture = true;

	/* set value */
	set_vivo_snd_card_info(BBK_SND_CARD_MASK, vivo_snd_info);
	set_vivo_fm_info(BBK_FM_INFO_MASK, vivo_fm_pcm_info);
	pr_info("%s() snd info 0x%x, fm_pcm_info 0x%x.\n", __func__,
			vivo_snd_info, vivo_fm_pcm_info);
}
#endif

static void vivo_parse_btn_defs(const struct device_node *np)
{
	u32 btn_defs[BTN_NUM];
	int i = 0;

	if (of_find_property(np, "vivo,mbhc-btn-defs", NULL)) {
		of_property_read_u32_array(np, "vivo,mbhc-btn-defs", btn_defs, BTN_NUM);
		for (i = 0; i < BTN_NUM; i++) {
			mbhc_btn_defs[i] = 0xffff & btn_defs[i];
		}
	}
	return;
}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	/* parse btn defs by vivo audio team@fanyongxiang. */
	vivo_parse_btn_defs(pdev->dev.of_node);
...
/* not GDPR code end */
...
#ifdef CONFIG_BBK_DRIVER_INFO
	vivo_parse_snd_info(pdev->dev.of_node);
	...
	dev_err(&pdev->dev, "%s: Sound card %s registered successfully.\n",
		 __func__, card->name);
