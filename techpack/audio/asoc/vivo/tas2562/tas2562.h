...
#define TAS2562_DRIVER_ID           "2.0.2"
...
#define TAS2562_BoostPeakCurrentLimit TAS2562_REG(0x0, 0x0, 0x40)
...
struct tas2562_priv {
	...
	int Qt[MAX_CHANNELS];
	uint32_t pa_id[MAX_CHANNELS];
	uint32_t v_max_q15[MAX_CHANNELS];
	uint32_t i_max_q15[MAX_CHANNELS];
	uint32_t v_out_max_q15[MAX_CHANNELS];

	bool mbSmart_pa_switch_enable;
	bool vivo_smartpa_protection;
	int smartpa_i2c_check;
#endif
	/* Add this for setting special regs according to different producton
	 * by vivo audio team@fanyongxiang.
	 */
	int reg_size;
	int *reg_table;
};
