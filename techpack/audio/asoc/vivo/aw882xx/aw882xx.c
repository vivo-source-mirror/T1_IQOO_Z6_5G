...
#define AW882XX_DRIVER_VERSION "v0.2.5"
...
static struct aw882xx *aw882xx_priv;
...
static int aw882xx_start(struct aw882xx *aw882xx)
{
	...
	if (ret < 0) {
		...
		if (aw882xx->vivo_smartpa_protection)
			smartpa_set_re_to_dsp();
		else
...
static int aw882xx_probe(aw_snd_soc_codec_t *codec)
{
	...
	if (codec->dev->of_node) {
		if (chan_info->name_suffix)
			dev_set_name(codec->dev, "%s_%s", "smartpa-codec",
				chan_info->name_suffix);
		else
			dev_set_name(codec->dev, "%s", "smartpa-codec");
	}
...
{
    struct aw882xx *aw882xx = aw882xx_priv;
	const int size = 20;
	int n = 0;
...
static struct attribute_group aw882xx_node_attribute_group = {
	.name = NULL,		/* put in device directory */
	.attrs = sys_node_attributes
};

static int class_attr_create(struct aw882xx *aw882xx)
{
	int ret = -1;
	...
	aw882xx->k_obj = kobject_create_and_add("audio-tas2562",kernel_kobj);
	if (!aw882xx->k_obj) {
		pr_info("kobject_create_and_add audio-tas2562 file faild\n");
		return 0;
	}

	ret = sysfs_create_group(aw882xx->k_obj, &aw882xx_node_attribute_group);
	if (ret) {
		pr_info("sysfs_create_group audio-aw882xx file faild\n");
	}

	return ret;
}

static int class_attr_remove(struct aw882xx *aw882xx)
{
	if (aw882xx != NULL && aw882xx->k_obj) {
		sysfs_remove_group(aw882xx->k_obj, &aw882xx_node_attribute_group);
		kobject_del(aw882xx->k_obj);
		aw882xx->k_obj = NULL;
	}
	return 0;
}
	...
	const struct i2c_device_id *id)
{
	...
	aw882xx->dev = &i2c->dev;
	aw882xx->i2c = i2c;
	aw882xx->dev_state = 0;/* check out bad aw882xx IC @zousheng*/
...
*/
	...
	if (np) {
		if (chan_info->name_suffix)
			dev_set_name(&i2c->dev, "%s_%s", "smartpa-codec",
				chan_info->name_suffix);
		else
