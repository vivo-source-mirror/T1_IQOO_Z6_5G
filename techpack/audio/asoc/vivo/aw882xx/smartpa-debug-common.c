...
extern void smartpa_init_re_value(uint32_t *re_value);


static struct i2c_client * smartpa_debug_client = NULL;
...
static long smartpa_debug_shared_ioctl(struct file *file,
	unsigned int cmd, void __user *arg)
{
	...
	case TFA_IOCTL_SPK_SET:
		...
		if (ret){
			...
			smartpa_init_re_value(re_calib);
...
{
	pr_info("%s...\n","%s");
	return smartpa_debug_shared_ioctl(file, cmd, (void __user *)arg);
}

#ifdef CONFIG_COMPAT
static long smartpa_compat_debug_ioctl(struct file *file,
	unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	pr_info("%s...\n","%s");
	ret = smartpa_debug_shared_ioctl(file,cmd,compat_ptr(arg));
	return ret;
}
#endif
...
#ifdef CONFIG_COMPAT
	.compat_ioctl =   smartpa_compat_debug_ioctl,
#endif
