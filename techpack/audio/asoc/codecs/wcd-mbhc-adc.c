...
#define WCD_MBHC_ADC_HS_THRESHOLD_MV    1751
#define WCD_MBHC_ADC_HPSW_THRESHOLD_MV  426
#define WCD_MBHC_ADC_HPH_THRESHOLD_MV   89
#define WCD_MBHC_ADC_HS_POLLING_THD_MV  2644 /* 45ko */
...
static int wcd_mbhc_adc_get_hpsw_thres(struct wcd_mbhc *mbhc)
{
	int hpsw_threshold, micbias_mv;

	micbias_mv = wcd_mbhc_get_micbias(mbhc);
	if (mbhc->hpsw_thr) {
		if (mbhc->micb_mv == micbias_mv)
			hpsw_threshold = mbhc->hpsw_thr;
		else
			hpsw_threshold = (mbhc->hpsw_thr *
				micbias_mv) / mbhc->micb_mv;
	} else {
		hpsw_threshold = ((WCD_MBHC_ADC_HPSW_THRESHOLD_MV *
			micbias_mv) / WCD_MBHC_ADC_MICBIAS_MV);
	}
	return hpsw_threshold;
}
...

{
	...
	u32 hph_thr = 0, hs_thr = 0, hpsw_thr = 0;

	hs_thr = wcd_mbhc_adc_get_hs_thres(mbhc);
	hph_thr = wcd_mbhc_adc_get_hph_thres(mbhc);
	hpsw_thr = wcd_mbhc_adc_get_hpsw_thres(mbhc);

	if (adc_result < 0)
		plug_type = MBHC_PLUG_TYPE_NONE;
	else if (adc_result < hph_thr)
		plug_type = MBHC_PLUG_TYPE_HEADPHONE;
	else if (adc_result < hpsw_thr) {
		...
		plug_type = MBHC_PLUG_TYPE_HEADSET;
	else if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->mbhc_detect_moisture)
		plug_type = MBHC_PLUG_TYPE_NONE;
	else
		plug_type = MBHC_PLUG_TYPE_HEADSET;
	pr_debug("%s:hs_thr %d, hph_thr %d, hpsw_thr %d, adc_result %d, plug type is %d found\n",
			__func__, hs_thr, hph_thr, hpsw_thr, adc_result, plug_type);
	return plug_type;
}

static int wcd_mbhc_get_mic_adc_voltage_fn(struct wcd_mbhc *mbhc)
{
	int output_mv = 0;
	...
	if (mbhc->mbhc_cb->mbhc_micb_ctrl_thr_mic &&
		!mbhc->micbias_enable)
		mbhc->mbhc_cb->mbhc_micb_ctrl_thr_mic(mbhc->component, MIC_BIAS_2,
							  false);
...
static void wcd_mbhc_adc_detect_moisture_fn(struct work_struct *work)
{
	struct wcd_mbhc *mbhc;
	struct snd_soc_component *component;
	struct wcd937x_priv *wcd937x;
	int adc_threshold = 0;
	bool is_moisture = false;
	int output_mv = 0, timer = 0;

	mbhc = container_of(work, struct wcd_mbhc, detect_moisture_work);
	component = mbhc->component;
	wcd937x = snd_soc_component_get_drvdata(component);
	if (!wcd937x)
		return ;

	/* if missing to config in dtsi, returin */
	...
	adc_threshold = WCD_MBHC_ADC_HS_POLLING_THD_MV;

	while (1) {
		/* Wait for 500ms for FSM to update result */
		msleep(500);

		if (!mbhc->is_detect_moisture) {
			pr_debug("%s: is_detect_moisture %d. goto exit\n",
					__func__, mbhc->is_detect_moisture);
			if (mbhc->mbhc_cb->mbhc_micbias_control)
				mbhc->mbhc_cb->mbhc_micbias_control(mbhc->component,
									MIC_BIAS_2, MICB_DISABLE);
			if (mbhc->mbhc_cb->mbhc_micb_ctrl_thr_mic &&
		...
		if (wcd937x->pm_state == WCD937X_PM_ASLEEP) {
			continue;
		}
		...
		if (output_mv < 0)
			continue;
		else if (output_mv > adc_threshold)
			is_moisture = true;
		else
			is_moisture = false;
		...
		if (timer >= 3) {
			timer = 0;
			pr_debug("%s: output_mv: %d, mbhc->current_plug: %d, "
				"is_moisture: %d.\n", __func__, output_mv,
				mbhc->current_plug, is_moisture);
		}

		if (is_moisture && (mbhc->current_plug != MBHC_PLUG_TYPE_NONE)) {
			WCD_MBHC_RSC_LOCK(mbhc);
			mbhc->in_swch_irq_handler = false;
			mbhc->swch_detection_type = false;
			pr_info("%s: output_mv %d current_plug %d, remove\n",
				__func__, output_mv, mbhc->current_plug);
			break;
		} else if (!is_moisture && mbhc->current_plug == MBHC_PLUG_TYPE_NONE) {
			WCD_MBHC_RSC_LOCK(mbhc);
			mbhc->in_swch_irq_handler = true;
			mbhc->swch_detection_type = true;
			pr_info("%s: output_mv %d current_plug %d, insert\n",
					__func__, output_mv, mbhc->current_plug);
			break;
		}
	}
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	int output_mv = 0;
	int output_mv_old = -1;
	int output_mv_diff = -1;
	int is_hp_connect_on = 0;
	/* int cross_conn;
	int try = 0;
	int hs_threshold, micbias_mv; */
...
correct_plug_type:
	...
	while (!time_after(jiffies, timeout)) {
		/* A detection error will occur if the msleep time before first-detect is too small, move msleep200 here by guanzhaozhi*/
		if (!(mbhc->mbhc_cfg->enable_usbc_analog))
			msleep(200);
		if (mbhc->hs_detect_work_stop) {
			pr_debug("%s: stop requested: %d\n", __func__,
		 ...
		 */
		if (mbhc->is_atmode && mbhc->mbhc_cfg && mbhc->mbhc_cfg->enable_usbc_analog)
			max20328_switch_event(mbhc->fsa_np, FSA_USBC_AUIDO_HP_ON);

		output_mv = wcd_measure_adc_once(mbhc, MUX_CTL_IN2P);
		pr_debug("%s: mbhc measure adc output_mv %d.\n", __func__, output_mv);

		if (mbhc->mbhc_cfg->enable_usbc_analog) {
			if (output_mv_old == -1) {
				output_mv_old = output_mv;
				if (mbhc->mbhc_cfg->swap_gnd_mic)
					mbhc->mbhc_cfg->swap_gnd_mic(mbhc->component, true);
				msleep(20);
				continue;
			}
			if (output_mv_old > output_mv) {
				output_mv_diff = output_mv_old - output_mv;
				output_mv = output_mv_old;
				if (mbhc->mbhc_cfg->swap_gnd_mic)
					mbhc->mbhc_cfg->swap_gnd_mic(mbhc->component, true);
			} else {
				output_mv_diff = output_mv - output_mv_old;
			}
			output_mv_old = output_mv;
			max20328_switch_event(mbhc->fsa_np, FSA_USBC_AUIDO_HP_ON);
		}
		...
		if (plug_type == MBHC_PLUG_TYPE_HEADSET) {
			pr_debug("%s: cable might be headset: %d\n",
				__func__, plug_type);
			if (mbhc->mbhc_cfg->enable_usbc_analog && !is_hp_connect_on
				&& (output_mv_diff < 300)) {
				pr_debug("%s: cable might be special plug, set HP_ON to check.\n",
					__func__);
				output_mv_old = -1;
				is_hp_connect_on = 1;
				msleep(20);
			...
			(plug_type == MBHC_PLUG_TYPE_GND_MIC_SWAP)) {
			pr_debug("%s: cable might be headphone: %d\n",
				__func__, plug_type);
			plug_type = MBHC_PLUG_TYPE_HEADPHONE;
		...
		} else if (plug_type == MBHC_PLUG_TYPE_NONE) {
			pr_debug("%s: cable might be none: %d\n",
				__func__, plug_type);
			if (mbhc->mbhc_cfg->enable_usbc_analog)
				output_mv_old = -1;
		}

		/* To speeed up the continue above, set msleep200 here by guanzhaozhi*/
		if (mbhc->mbhc_cfg->enable_usbc_analog)
			msleep(200);
	...
	if (plug_type == MBHC_PLUG_TYPE_NONE) {
		...
		WCD_MBHC_RSC_LOCK(mbhc);
		mbhc->is_detect_moisture = true;
		pr_debug("%s: Entry moisture mode.\n", __func__);
		WCD_MBHC_RSC_UNLOCK(mbhc);
...
/* not GDPR code start */
#ifdef CONFIG_RSC_VAUDIT_DEBUG
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_MBHC, 0,
			"MBHC: moisture mode.");
#endif
/* not GDPR code end */
#endif
...
report:
	...
	pr_debug("%s: Valid plug found, plug type %d wrk_cmpt %d btn_intr %d is_detect_moisture %d is_atmode %d\n",
			__func__, plug_type, wrk_complete,
			mbhc->btn_press_intr, mbhc->is_detect_moisture, mbhc->is_atmode);
...
exit:
	/* Enable micb_2 pullup to detect botton @zgbing */
	if ((plug_type == MBHC_PLUG_TYPE_HEADSET) &&
...
static struct wcd_mbhc_fn mbhc_fn = {
	...
	.wcd_mbhc_detect_moisture_fn = wcd_mbhc_adc_detect_moisture_fn,
	.wcd_mbhc_get_mic_adc_voltage = wcd_mbhc_get_mic_adc_voltage_fn,
};
