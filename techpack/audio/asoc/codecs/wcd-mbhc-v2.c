...
#define MBHC_AT_IOCTL_MAGIC  'M'
#define MBHC_AT_IOCTL_GET_TYPE  _IOR(MBHC_AT_IOCTL_MAGIC, 0x01, int)
#define MBHC_AT_IOCTL_MOS_TEST  _IOR(MBHC_AT_IOCTL_MAGIC, 0x02, int)
#define MBHC_AT_IOCTL_HPH_REMOVE  _IOR(MBHC_AT_IOCTL_MAGIC, 0x03, int)
#define MBHC_AT_IOCTL_ATMODE      _IOR(MBHC_AT_IOCTL_MAGIC, 0x04, int)
#define MBHC_AT_IOCTL_NOTIFY_SMARTPA  _IOR(MBHC_AT_IOCTL_MAGIC, 0x05, int)
#define MBHC_AT_IOCTL_GET_USBC_STATUS _IOR(MBHC_AT_IOCTL_MAGIC, 0x06, int)
#define MBHC_AT_IOCTL_GET_USBC_MG_STATUS _IOR(MBHC_AT_IOCTL_MAGIC, 0x07, int)
#define MBHC_AT_IOCTL_GET_MIC_ADC_VOLTAGE _IOR(MBHC_AT_IOCTL_MAGIC, 0x08, int)

#define MBHC_DEV_NAME	"mbhcftm"
#define MBHC_AT_TIMEOUT_MS	100
#define MBHC_AT_PULLDWN_US	50000
#define MBHC_AT_BTN_DN	1
#define MBHC_AT_BTN_UP	2
#define MBHC_AT_MOST_EN	4
#define MBHC_AT_TYPE_TIMEOUT_MS	1000
#define MBHC_AT_TYPE_REPORT	1
#define MBHC_AT_TYPET_EN	2

static struct wcd_mbhc *at_mbhc = NULL;
static bool at_node_register_done = false;
static unsigned int at_mos_status = 0;
static unsigned int at_type_status = 0;
static wait_queue_head_t at_mbhc_wait;

#define WCD_MBHC_STATUS "mbhc_status"
//struct wake_lock mbhc_key_lock;
struct wakeup_source *mbhc_key_lock;
...
/* add this function to fix hp impedance by vivo audio team */
void vivo_imped_fix(struct wcd_mbhc *mbhc, uint32_t *impl, uint32_t *impr)
{
	if (*impl < 5) {
		pr_info("%s: zl %d, zr %d, imp detect failed, fixed them 32 ohm.\n", __func__, *impl, *impr);
		*impl = 32;
		*impr = 32;
	} else {
		pr_info("%s: zl %d, zr %d, fixed them impl value.\n", __func__, *impl, *impr);
		*impr = *impl;
		if (mbhc->mbhc_cfg && (*impl > 16))
			*impl = *impr = *impl - mbhc->mbhc_cfg->imped_adjust_ref;
	}

	if (mbhc->is_atmode)
		*impr = *impl = 51;

	pr_info("%s wcd_mbhc_fixed zl %d, zr %d is_atmode %d.\n",
			__func__, *impl, *impr, mbhc->is_atmode);
}
...
{
	/*audio_v add */
	__pm_wakeup_event(mbhc_key_lock, 3 * HZ);
	/*audio_v add end*/
				...
				enum snd_jack_types jack_type)
{
	...
	pr_debug("%s: enter insertion %d hph_status %x is_detect_moisture %d is_atmode %d\n",
		 __func__, insertion, mbhc->hph_status, mbhc->is_detect_moisture, mbhc->is_atmode);
	...
	if (!insertion) {
		...
		/*audio_v add */
		/* Disable micb_2 pullup if headset has removed @zgbing */
		if (mbhc->mbhc_cb->mbhc_micbias_control)
		...
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->enable_usbc_analog)
			max20328_switch_event(mbhc->fsa_np, FSA_USBC_AUIDO_HP_OFF);
		/*audio_v add end*/
		...
		/*audio_v add */
		/* hp swich to hifi for default when remove plug type. @zgbing */
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->set_hp_switch_state) {
			mbhc->mbhc_cfg->set_hp_switch_state(HP_PATH_HIFI);
		}
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->swap_gnd_mic &&
			!mbhc->mbhc_cfg->enable_usbc_analog)
			mbhc->mbhc_cfg->swap_gnd_mic(component, 0);
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->mbhc_detect_moisture
			&& mbhc->is_detect_moisture)
			schedule_work(&mbhc->detect_moisture_work);
		/*audio_v add end*/
	} else {
		/*
		...
		/*audio_v add */
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->enable_usbc_analog)
			max20328_switch_event(mbhc->fsa_np, FSA_USBC_AUIDO_HP_ON);
		/*audio_v add end*/
		...
		if (mbhc->impedance_detect &&
			...
			mbhc->mbhc_cfg->imped_detect_en &&
			!is_pa_on &&
			!(mbhc->mbhc_cfg->get_hifi_dac_state && mbhc->mbhc_cfg->get_hifi_dac_state())) {
			if (mbhc->mbhc_cfg->set_hp_switch_state) {
				mbhc->mbhc_cfg->set_hp_switch_state(HP_PATH_HIFI_BYPASS_ON);
			}  //audio_v modify
			...
			mbhc->mbhc_cb->compute_impedance(mbhc,
					&mbhc->zl, &mbhc->zr);
			/*audio_v add */
			/* do imped fix @leichenji */
			vivo_imped_fix(mbhc, &mbhc->zl, &mbhc->zr);
			/*audio_v add end*/
			WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_FSM_EN,
						 fsm_en);
			/*audio_v add */
			if (mbhc->mbhc_cfg->set_hp_switch_state) {
				mbhc->mbhc_cfg->set_hp_switch_state(HP_PATH_HIFI_BYPASS_OFF);
			}
			/*audio_v add end*/
...
/*audio_v del */
			...
			vivo_imped_fix(mbhc, &mbhc->zl, &mbhc->zr); //	/*audio_v add*/
		...
		/*audio_v add */
		/* hp swich to hifi when reporting plug type if hifi has opened. @zgbing */
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->set_hp_switch_state &&
			mbhc->mbhc_cfg->get_hifi_dac_state &&
			mbhc->mbhc_cfg->get_hifi_dac_state()) {
			mbhc->mbhc_cfg->set_hp_switch_state(HP_PATH_HIFI);
		}
		/*audio_v add end*/
	...
	pr_debug("%s: leave hph_status %x, clamp_state: %d\n",
		__func__, mbhc->hph_status, clamp_state);
	/*audio_v add end*/
}
...
exit:
	/*audio_v add */
	if (at_type_status & MBHC_AT_TYPET_EN) {
		pr_info("%s() plug type detected during AT Type Test.\n", __func__);
		mbhc->hs_detect_work_stop = true;
		at_type_status |= MBHC_AT_TYPE_REPORT;
		wake_up(&at_mbhc_wait);
	}
	/*audio_v add end*/
...
/*audio_v modify */
void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	bool micbias1 = false;
	struct snd_soc_component *component = mbhc->component;
	enum snd_jack_types jack_type;
	...
	pr_debug("%s: mbhc->current_plug: %d swch_detection_type: %d\n", __func__,
			mbhc->current_plug, mbhc->swch_detection_type);
	if (mbhc->mbhc_fn->wcd_cancel_hs_detect_plug)
		mbhc->mbhc_fn->wcd_cancel_hs_detect_plug(mbhc,
						&mbhc->correct_plug_swch);
	else
	...
	/* Disconnect MIC to GND */
	if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->swap_gnd_mic &&
		!mbhc->mbhc_cfg->enable_usbc_analog &&
		mbhc->mbhc_cfg->swap_gnd_mic(component,
		false)) {
		pr_debug("%s: Disconnect MIC to GND\n"
			, __func__);
	}

	/* We need to set 'HP Path' to Normal for Headset Button detection.
	    Added by ChenJinQuan. */
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		mbhc->swch_detection_type) {
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->set_hp_switch_state &&
			mbhc->mbhc_cfg->get_hp_switch_state)
			mbhc->mbhc_cfg->set_hp_switch_state(HP_PATH_NORMAL);
	}
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		 ...
		 */
		if (wcd_mbhc_moisture_detect(mbhc, mbhc->swch_detection_type))
			goto done;
		...
		if (mbhc->mbhc_cfg && !mbhc->mbhc_cfg->hs_ext_micbias &&
	...
	} else if (!mbhc->swch_detection_type) {
		/* Disable external voltage source to micbias if present */
		if (mbhc->mbhc_cb->enable_mb_source)
			mbhc->mbhc_cb->enable_mb_source(mbhc, false);
		/* Disable HW FSM */
...
void wcd_mbhc_swch_gpio_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	pr_debug("%s: current_plug: %d swch_detection_type: %d gpio_detection_type %d\n",
			__func__, mbhc->current_plug, mbhc->swch_detection_type,
			mbhc->gpio_detection_type);

	if ((mbhc->swch_detection_type && mbhc->gpio_detection_type) ||
		(!mbhc->swch_detection_type && mbhc->in_swch_irq_handler) ||
		(!mbhc->gpio_detection_type && mbhc->in_gpio_irq_handler))
		pr_debug("%s: continue to detect plug\n", __func__);
	 else
		goto jump_to_end;

	if (((mbhc->swch_detection_type && mbhc->gpio_detection_type) &&
		((mbhc->current_plug != MBHC_PLUG_TYPE_NONE) ||
		!mbhc->hs_detect_work_stop)) || !mbhc->mbhc_cfg)
		goto jump_to_end;
	...
	/* We need to set 'HP Path' to Normal for Headset Button detection.
	    Added by ChenJinQuan. */
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		(mbhc->swch_detection_type && mbhc->gpio_detection_type)) {
		if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->set_hp_switch_state &&
			mbhc->mbhc_cfg->get_hp_switch_state)
			mbhc->mbhc_cfg->set_hp_switch_state(HP_PATH_NORMAL);
	}
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
		 ...
		 * moisture detection and wait for interrupt
		 */
		if (wcd_mbhc_moisture_detect(mbhc,
			(mbhc->swch_detection_type && mbhc->gpio_detection_type)))
			goto done;
	...
	} else if (!(mbhc->swch_detection_type && mbhc->gpio_detection_type)) {
		/* Disable external voltage source to micbias if present */
		if (mbhc->mbhc_cb->enable_mb_source)
			mbhc->mbhc_cb->enable_mb_source(mbhc, false);
		/* Disable HW FSM */
...
jump_to_end:
	...
	mbhc->in_swch_irq_handler = false;
	mbhc->in_gpio_irq_handler = false;
...
static irqreturn_t wcd_mbhc_gpio_plug_detect_irq(int irq, void *data)
{
	int r = IRQ_HANDLED;
	...
	/* disable gpio irq */
	disable_irq_nosync(mbhc->headset_gpio_irq);
	if (unlikely((mbhc->mbhc_cb->lock_sleep(mbhc, true)) == false)) {
		...
		pr_debug("%s: use gpio irq detect\n",
		__func__);
		...
		mbhc->gpio_detection_type = !gpio_get_value_cansleep(mbhc->headset_gpio);

		for (; try_times > 0; try_times--) {
			for (i = read_times; i > 0; i--) {
				new_type = !gpio_get_value_cansleep(mbhc->headset_gpio);
				if (mbhc->gpio_detection_type != new_type) {
					mbhc->gpio_detection_type = new_type;
					break;
				}
			if (i == (read_times / 4))
		...
		if (try_times <= 0) {
			...
			pr_debug("%s: detect hp success, detection_type %d\n",
				__func__, mbhc->gpio_detection_type);
		}

		mbhc->in_gpio_irq_handler = true;
		wcd_mbhc_swch_gpio_irq_handler(mbhc);
		WCD_MBHC_RSC_UNLOCK(mbhc);

		mbhc->mbhc_cb->lock_sleep(mbhc, false);
	}
leave:
	if (mbhc->gpio_detection_type) {
		irq_set_irq_type(mbhc->headset_gpio_irq, IRQF_TRIGGER_HIGH);
	} else {
		irq_set_irq_type(mbhc->headset_gpio_irq, IRQF_TRIGGER_LOW);
	}
	enable_irq(mbhc->headset_gpio_irq);
	pr_debug("%s: leave %d\n", __func__, r);
	return r;
}

static void wcd_mbhc_mech_plug_detect_usbc(struct wcd_mbhc *mbhc, int mode)
{
	pr_debug("%s: enter\n", __func__);
	if (unlikely((mbhc->mbhc_cb->lock_sleep(mbhc, true)) == false)) {
		pr_warn("%s: failed to hold suspend\n", __func__);
		...
		/* Call handler */
		WCD_MBHC_RSC_LOCK(mbhc);
		mbhc->is_detect_moisture = false;
		mbhc->in_swch_irq_handler = true;
		mbhc->swch_detection_type = mode ? true : false;
		if (mbhc->headset_gpio > 0) {
			wcd_mbhc_swch_gpio_irq_handler(mbhc);
		} else {
...
static irqreturn_t wcd_mbhc_mech_plug_detect_irq(int irq, void *data)
{
	...
	/*audio_v add*/
	if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->enable_usbc_analog) {
		pr_debug("%s: enter. now nothing to do\n", __func__);
		return r;
	}
	...
	if (unlikely((mbhc->mbhc_cb->lock_sleep(mbhc, true)) == false)) {
		...
		mbhc->is_detect_moisture = false;
		mbhc->in_swch_irq_handler = true;
		WCD_MBHC_REG_READ(WCD_MBHC_MECH_DETECTION_TYPE, mbhc->swch_detection_type);
		/* Set the detection type appropriately */
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_MECH_DETECTION_TYPE,
					 !mbhc->swch_detection_type);
		if (mbhc->headset_gpio > 0) {
...
static irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	if (mbhc->in_swch_irq_handler || mbhc->in_gpio_irq_handler) {  	/*audio_v modify*/
	...
	/*audio_v add*/
	if (at_mos_status & MBHC_AT_MOST_EN) {
		pr_info("%s() Btn detected during AT Mos Test.\n", __func__);
		if (mask & SND_JACK_BTN_0) {
			at_mos_status |= MBHC_AT_BTN_DN;
		}
		goto done;
	}
	/*audio_v add end*/
...
/*audio_v modify*/
static void wcd_mbhc_release_handler_dwork(struct work_struct *work)
{
	/*audio_v add*/
	struct delayed_work *dwork;
	struct wcd_mbhc *mbhc;
	...
	/*audio_v add*/
	dwork = to_delayed_work(work);
	mbhc = container_of(dwork, struct wcd_mbhc, mbhc_btn_release_dwork);
	/*audio_v add end*/
	WCD_MBHC_RSC_LOCK(mbhc);
	if (wcd_swch_level_remove(mbhc)) {
		pr_debug("%s: Switch level is low ", __func__);
	...
	/*audio_v add*/
	if (at_mos_status & MBHC_AT_MOST_EN) {
		pr_info("%s() Btn release detected during AT Mos Test.\n", __func__);
		if (at_mos_status & MBHC_AT_BTN_DN)
			at_mos_status |= MBHC_AT_BTN_UP;
		wake_up(&at_mbhc_wait);
		goto exit;
	}
	/*audio_v add end*/
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	pr_debug("%s: enter\n", __func__);
	/*audio_v add*/
	/* schedule a work_q to deleyed 70ms to check if plug is stlill in */
	schedule_delayed_work(&mbhc->mbhc_btn_release_dwork,
				msecs_to_jiffies(70));
	/*audio_v add end*/
	...
	if (mbhc->buttons_pressed & WCD_MBHC_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			...
			if (mbhc->in_swch_irq_handler || mbhc->in_gpio_irq_handler) {
				pr_debug("%s: Switch irq kicked in, ignore\n",
					__func__);
			} else {
				pr_debug("%s: Reporting btn press\n",
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	int ret = 0;
	struct snd_soc_component *component = mbhc->component;
	int gpio_val = 1;     /*audio_v add*/

	pr_debug("%s: enter\n", __func__);
	WCD_MBHC_RSC_LOCK(mbhc);
	/*audio_v add*/
	/* change value from 2 to 0 to turn MIC_CLAMP off by audio team */
	 ...
	 */
	if (mbhc->mbhc_cfg->enable_usbc_analog) {
		mbhc->hphl_swh = 0;     /*audio_v add*/
		mbhc->gnd_swh = 0;      /*audio_v add*/
	...
	/* Button Debounce set to 32ms @zgbing */
	WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_BTN_DBNC, 3);     /*audio_v modify*/
	...
	if (mbhc->headset_gpio > 0) {
		...
		gpio_val = gpio_get_value_cansleep(mbhc->headset_gpio);
		...
		if (gpio_val == 0) {
			if (unlikely((mbhc->mbhc_cb->lock_sleep(mbhc, true)) == false)) {
				...
				bool is_lock = false;
				/* Check whether the component->mutex is locked. */
				is_lock = mutex_trylock(&component->mutex);
				mutex_unlock(&component->mutex);
				mbhc->in_gpio_irq_handler = true;
				mbhc->gpio_detection_type = true;
				wcd_mbhc_swch_gpio_irq_handler(mbhc);
				WCD_MBHC_RSC_UNLOCK(mbhc);
				enable_irq(mbhc->headset_gpio_irq);
				WCD_MBHC_RSC_LOCK(mbhc);
		...
		ret = enable_irq_wake(mbhc->headset_gpio_irq);
		if (ret)
			pr_err("%s: Failed to enable wake up irq %d\n",
				  __func__, mbhc->headset_gpio_irq);
	}
    /*audio_v add end*/
	WCD_MBHC_RSC_UNLOCK(mbhc);
	pr_debug("%s: leave\n", __func__);
...
#if 1 //IS_ENABLED(CONFIG_QCOM_FSA4480_I2C)      /*audio_v modify*/
					   ...
					   unsigned long mode, void *ptr)
{
	...
	if ((mode == TYPEC_ACCESSORY_AUDIO) ||
		(mode == TYPEC_ACCESSORY_NONE)) {     /*audio_v add*/
		if (mbhc->mbhc_cb->clk_setup) {
		...
		/*audio_v modify*/
		/* WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_L_DET_EN, 1); */
		wcd_mbhc_mech_plug_detect_usbc(mbhc, mode);
		/*audio_v modify end*/
	}
	return 0;
}
...
int wcd_mbhc_start(struct wcd_mbhc *mbhc, struct wcd_mbhc_config *mbhc_cfg)
{
	...
	/* Set impedance pointer for HiFi, changed by ChenJinQuan. */
	mbhc_cfg->hp_impedance = &mbhc->zl;     /*audio_v add*/
	...
	if (mbhc_cfg->enable_usbc_analog) {
		...
		mbhc->fsa_np = of_parse_phandle(card->dev->of_node,
				"max20328-i2c-handle", 0);       /*audio_v modify*/
		if (!mbhc->fsa_np) {
	...
	if (mbhc_cfg->enable_usbc_analog) {
		...
		rc = max20328_reg_notifier(&mbhc->fsa_nb, mbhc->fsa_np);      /*audio_v modify*/
		if (rc) {
			dev_err(card->dev, "%s: max20328_reg_notifier failed\n",
				__func__);
			rc = 0;
		}
	}
...
void wcd_mbhc_stop(struct wcd_mbhc *mbhc)
{
	...
	/*audio_v modify*/
	if (mbhc->mbhc_cfg && mbhc->mbhc_cfg->enable_usbc_analog)
		max20328_unreg_notifier(&mbhc->fsa_nb, mbhc->fsa_np);
...
static long  mbhc_at_ioctl (struct file *file,
	unsigned int cmd, unsigned long arg)
{
	...
	if (!at_mbhc) {
		pr_err("%s() no valid at_mbhc\n", __func__);
		return -EINVAL;
	}

	switch(cmd) {
	/* get headphone type */
	case MBHC_AT_IOCTL_GET_TYPE:
		pr_info("%s: AT start to detect plug type.\n", __func__);

		WCD_MBHC_RSC_LOCK(at_mbhc);
		at_type_status = MBHC_AT_TYPET_EN;
		if (at_mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) {
			wcd_mbhc_report_plug(at_mbhc, 0, SND_JACK_HEADPHONE);
		}
		else if (at_mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
			wcd_mbhc_report_plug(at_mbhc, 0, SND_JACK_HEADSET);
		}

		at_mbhc->in_swch_irq_handler = true;
		at_mbhc->hs_detect_work_stop = true;

		if (at_mbhc->headset_gpio > 0) {
			at_mbhc->in_gpio_irq_handler = true;
			wcd_mbhc_swch_gpio_irq_handler(at_mbhc);
		} else {
			wcd_mbhc_swch_irq_handler(at_mbhc);
		}
		WCD_MBHC_RSC_UNLOCK(at_mbhc);

		ret = wait_event_timeout(at_mbhc_wait,
			(at_type_status & MBHC_AT_TYPE_REPORT),
			msecs_to_jiffies(MBHC_AT_TYPE_TIMEOUT_MS));
		if (!ret) {
			pr_err("%s() mbhc type test timed out.\n",
				__func__);
		}
		...
		pr_info("%s() current plug type is %d, result %u\n", __func__, at_mbhc->current_plug, res);
		break;
	/* mos test */
	case MBHC_AT_IOCTL_MOS_TEST:
		if (at_mbhc->current_plug != MBHC_PLUG_TYPE_HEADSET) {
			pr_err("%s() plug type %d not correct\n", __func__, at_mbhc->current_plug);
			res = 0;
			break;
		}
		if (!at_mbhc->mbhc_cfg) {
			pr_err("%s() no valid mbhc_cfg\n", __func__);
			res = 0;
			break;
		}
		if(!at_mbhc->mbhc_cfg->swap_gnd_mic ||
			at_mbhc->mbhc_cfg->enable_usbc_analog) {
			pr_err("%s() hardware not support swap_gnd_mic\n", __func__);
			res = -1;
			break;
		}

		WCD_MBHC_RSC_LOCK(at_mbhc);
		at_mos_status = MBHC_AT_MOST_EN;
		WCD_MBHC_RSC_UNLOCK(at_mbhc);

		at_mbhc->mbhc_cfg->swap_gnd_mic(at_mbhc->component, 1);
		usleep_range(MBHC_AT_PULLDWN_US, MBHC_AT_PULLDWN_US);
		at_mbhc->mbhc_cfg->swap_gnd_mic(at_mbhc->component, 0);

		pr_info("%s MOS has been pulled down %d us\n", __func__, MBHC_AT_PULLDWN_US);
		ret = wait_event_timeout(at_mbhc_wait,
			((at_mos_status & MBHC_AT_BTN_DN)
			&& (at_mos_status & MBHC_AT_BTN_UP)),
			msecs_to_jiffies(MBHC_AT_TIMEOUT_MS));

		if (!ret) {
			pr_err("%s() mbhc Mos test timed out.\n",
				__func__);
			res = 0;
		} else {
			res = 1;
		...
		break;
	case MBHC_AT_IOCTL_HPH_REMOVE:
		pr_info("%s AT cmd to report plug out!", __func__);
		WCD_MBHC_RSC_LOCK(at_mbhc);
		if (at_mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) {
			wcd_mbhc_report_plug(at_mbhc, 0, SND_JACK_HEADPHONE);
		}
		...
		WCD_MBHC_RSC_UNLOCK(at_mbhc);
		break;
	case MBHC_AT_IOCTL_ATMODE:
		pr_info("%s AT cmd to set atmode, ignore!", __func__);
		WCD_MBHC_RSC_LOCK(at_mbhc);
		at_mbhc->is_atmode = true;
		WCD_MBHC_RSC_UNLOCK(at_mbhc);
		break;
	case MBHC_AT_IOCTL_NOTIFY_SMARTPA:
		pr_info("%s AT cmd to notify smartpa!", __func__);
		if (at_mbhc->mbhc_cfg && at_mbhc->mbhc_cfg->smartpa_debug_retry_probe)
			at_mbhc->mbhc_cfg->smartpa_debug_retry_probe();
		break;
	case MBHC_AT_IOCTL_GET_USBC_STATUS:
		res = get_usbc_peripheral_status();
		pr_info("%s AT cmd to get usbc status: %d.\n",
			__func__, res);
		break;
	case MBHC_AT_IOCTL_GET_USBC_MG_STATUS:
		res = get_usbc_mg_status();
		pr_info("%s AT cmd to get usbc mg status: %d.\n",
			__func__, res);
		break;
	case MBHC_AT_IOCTL_GET_MIC_ADC_VOLTAGE:
		if (at_mbhc->mbhc_fn && at_mbhc->mbhc_fn->wcd_mbhc_get_mic_adc_voltage) {
			WCD_MBHC_RSC_LOCK(at_mbhc);
			res = at_mbhc->mbhc_fn->wcd_mbhc_get_mic_adc_voltage(at_mbhc);
			WCD_MBHC_RSC_UNLOCK(at_mbhc);
		}
		pr_info("%s AT cmd to get mic adc voltage: %d.\n",
			__func__, res);
		break;
	default:
		printk("%s() mbhc do bot support ioctl cmd = %x\n", __func__, cmd);
		ret = -EINVAL;
		res = -1;
		break;
	}
...
static const struct file_operations mbhc_at_fileops = {
	.owner = THIS_MODULE,
	.unlocked_ioctl = mbhc_at_ioctl,
};

static struct miscdevice mbhc_at_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = MBHC_DEV_NAME,
	.fops = &mbhc_at_fileops,
};

int mbhc_at_init(struct wcd_mbhc *mbhc)
{
	int err = 0;
	...
	at_type_status = 0;
	WCD_MBHC_RSC_UNLOCK(mbhc);
	if (at_node_register_done)
		return 0;
	err = misc_register(&mbhc_at_device);
	if (err) {
		pr_err("%s: mbhc_at_device register failed\n", __func__);
		return err;
	}
	init_waitqueue_head(&at_mbhc_wait);
	at_node_register_done = true;

	return 0;
}

int mbhc_at_deinit(void)
{
	pr_info("%s\n",__func__);
	at_mbhc = NULL;
	return 0;
		      ...
		      bool impedance_det_en)
{
	...
	const char *hpsw_thre = "qcom,msm-mbhc-hpsw-mic-max-threshold-mv"; /*audio_v add*/
	...
	if (ret)
		dev_dbg(card->dev,
			"%s: missing %s in dt node\n", __func__, hph_thre);
	/*audio_v add*/
	ret = of_property_read_u32(card->dev->of_node, hpsw_thre,
				&(mbhc->hpsw_thr));
	if (ret)
		dev_dbg(card->dev,
			"%s: missing %s in dt node\n", __func__, hpsw_thre);
	...
	mbhc->in_gpio_irq_handler = false;
	mbhc->headset_gpio = 0;
	mbhc->headset_gpio_irq = 0;
	mbhc->swch_detection_type = false;
	mbhc->gpio_detection_type = false;
	mbhc->is_atmode = false;
	/*audio_v add end*/
	...
	if (mbhc->headset_jack.jack == NULL) {
		...
		INIT_DELAYED_WORK(&mbhc->mbhc_btn_release_dwork, wcd_mbhc_release_handler_dwork); /*audio_v add*/
	}
	...
	/*audio_v add*/
	mbhc->headset_gpio = of_get_named_gpio(card->dev->of_node,
			"qcom,headset-irq-gpio", 0);

	if (mbhc->headset_gpio > 0) {
		ret = gpio_request(mbhc->headset_gpio, "headset detect irq");
		if (ret < 0)
			pr_err("%s: gpio %d request error %d\n",
					__func__, mbhc->headset_gpio, ret);

		ret = gpio_direction_input(mbhc->headset_gpio);
		if (ret < 0)
			pr_err("%s: set gpio %d input error %d\n",
					__func__, mbhc->headset_gpio, ret);

		mbhc->headset_gpio_irq = gpio_to_irq(mbhc->headset_gpio);
		pr_info("%s: gpio_irq = %d\n",
				__func__, mbhc->headset_gpio_irq);
	} else {
		pr_info("%s: gpio irq may not to be supported, return value %d\n",
				__func__, mbhc->headset_gpio);
		goto gpio_irq_end;
	}

	headset_gpio_get_pinctrl(card->dev);

	ret = request_threaded_irq(mbhc->headset_gpio_irq, NULL,
				  wcd_mbhc_gpio_plug_detect_irq,
				  (IRQF_TRIGGER_LOW |
				  IRQF_ONESHOT),
				  "headset detect", mbhc);
	if (ret) {
		pr_err("%s: Failed to request gpio irq %d\n",
				__func__, mbhc->headset_gpio_irq);
		goto err_mbhc_hs_gpio_irq;
	} else
		disable_irq_nosync(mbhc->headset_gpio_irq);

gpio_irq_end:

//	wake_lock_init(&mbhc_key_lock, WAKE_LOCK_SUSPEND, "headset key wakelock");
	mbhc_key_lock = wakeup_source_register(mbhc->component->dev,
					dev_name(mbhc->component->dev));
	if (!mbhc_key_lock) {
		pr_err("Failed to register wakeup_source\n");
	}


	INIT_WORK(&mbhc->detect_moisture_work, mbhc->mbhc_fn->wcd_mbhc_detect_moisture_fn);
	/* read mbhc_status for user @zgbing */
	mbhc->at_file_node = debugfs_create_file(WCD_MBHC_STATUS, S_IFREG | S_IRUGO,
												NULL, mbhc, &mbhc_status_ops);
	mbhc_at_init(mbhc);
	/*audio_v add end*/
	pr_debug("%s: leave ret %d\n", __func__, ret);
	return ret;
err_mbhc_hs_gpio_irq: /*audio_v add*/
err_hphr_ocp_irq:
...
{
	struct snd_soc_component *component = mbhc->component;
	//wake_lock_destroy(&mbhc_key_lock);
	/*audio_v add*/
	wakeup_source_unregister(mbhc_key_lock);
	if (mbhc->headset_gpio > 0) {
		disable_irq(mbhc->headset_gpio_irq);
		free_irq(mbhc->headset_gpio_irq, mbhc);
	}
	/*audio_v add end*/
	mbhc->mbhc_cb->free_irq(component, mbhc->intr_ids->mbhc_sw_intr, mbhc);
	mbhc->mbhc_cb->free_irq(component, mbhc->intr_ids->mbhc_btn_press_intr,
