...
#define SYS_CLASS_DEBUG_AUDIO
#ifdef SYS_CLASS_DEBUG_AUDIO
struct wcd937x_class_dev {
	struct class wcd937x_class;
...
#define MAX_DATA_SIZE 4096 //sys class max size
static char data_temp[MAX_DATA_SIZE];
#define MAX_DET_SIZE 256 //sys class max size
static char det_temp[MAX_DET_SIZE];
static ssize_t micbiasdet_store(struct class *c, struct class_attribute *attr,
							const char *buf, size_t count)
{
	return 0;
}

static ssize_t micbiasdet_show(struct class *c, struct class_attribute *attr,
							char *buf)
{
	struct wcd937x_class_dev *wcdev = container_of(c, struct wcd937x_class_dev,
									wcd937x_class);
	int rc, pos = 0;
	unsigned int val = 0;

	pr_debug("[wcd937x]%s Enter\n", __func__);

	/* Write 0x80 to 0x12C*/
	regmap_write(wcdev->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_EN, 0x80);

	regmap_write(wcdev->regmap, WCD937X_DIE_CRACK_INT_DIE_CRK_DET_INT1, 0xC2);/* Write 0xC2 to 0x1D3*/
	usleep_range(1000, 1010);
	regmap_read(wcdev->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_OUT, &val);/* Read 0x12D*/
	pr_debug("[wcd937x]%s when write reg 0xC2, reg 0x12D = 0x%x\n", __func__, val);
	rc = snprintf(det_temp + pos, MAX_DET_SIZE, "C2:%02x;", val);
	pos += rc;

	regmap_write(wcdev->regmap, WCD937X_DIE_CRACK_INT_DIE_CRK_DET_INT1, 0x82);/* Write 0x82 to 0x1D3*/
	usleep_range(1000, 1010);
	regmap_read(wcdev->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_OUT, &val);/* Read 0x12D*/
	pr_debug("[wcd937x]%s when write reg 0x82, reg 0x12D = 0x%x\n", __func__, val);
	rc = snprintf(det_temp + pos, MAX_DET_SIZE, "82:%02x;", val);
	pos += rc;


	regmap_write(wcdev->regmap, WCD937X_DIE_CRACK_INT_DIE_CRK_DET_INT1, 0x42);/* Write 0x42 to 0x1D3*/
	usleep_range(1000, 1010);
	regmap_read(wcdev->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_OUT, &val);/* Read 0x12D*/
	pr_debug("[wcd937x]%s when write reg 0x42, reg 0x12D = 0x%x\n", __func__, val);
	rc = snprintf(det_temp + pos, MAX_DET_SIZE, "42:%02x;", val);
	pos += rc;

	regmap_write(wcdev->regmap, WCD937X_DIE_CRACK_INT_DIE_CRK_DET_INT1, 0x02);/* Write 0x02 to 0x1D3*/
	usleep_range(1000, 1010);
	regmap_read(wcdev->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_OUT, &val);/* Read 0x12D*/
	pr_debug("[wcd937x]%s when write reg 0x02, reg 0x12D = 0x%x\n", __func__, val);
	rc = snprintf(det_temp + pos, MAX_DET_SIZE, "02:%02x;", val);
	pos += rc;

	/* Write 0xC0 to 0x12C*/
	regmap_write(wcdev->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_EN, 0xC0);
	...
	rc = scnprintf(buf, MAX_DET_SIZE, "%s", det_temp);
	return rc;
}
static CLASS_ATTR_RW(micbiasdet);

static ssize_t registers_store(struct class *c, struct class_attribute *attr,
 							const char *buf, size_t count)
{
	struct wcd937x_class_dev *wcdev = container_of(c, struct wcd937x_class_dev,
													wcd937x_class);
	...
	pr_debug("[wcd937x]%s write 0x%x to 0x%x\n", __func__, val, addr);
	regmap_write(wcdev->regmap, addr, val);
	pr_debug("[wcd937x]%s Exit\n", __func__);
	return count;
}

static ssize_t registers_show(struct class *c, struct class_attribute *attr,
 							char *buf)
{
	struct wcd937x_class_dev *wcdev = container_of(c, struct wcd937x_class_dev,
												wcd937x_class);
	unsigned int val = 0;
	int i, rc, pos = 0;
	u16 reg_count = wcd937x_regmap_config.num_reg_defaults;
	wcdev->address = WCD937X_BASE_ADDRESS;

	pr_debug("[wcd937x]%s Enter, reg_count %d\n", __func__, reg_count);
	for (i = 0; i < reg_count; i++) {
		regmap_read(wcdev->regmap, wcd937x_regmap_config.reg_defaults[i].reg, &val);
		pr_debug("[wcd937x]%s reg 0x%x = 0x%x\n", __func__, wcd937x_regmap_config.reg_defaults[i].reg, val);
		rc = snprintf(data_temp + pos, MAX_DATA_SIZE, "%04x:%02x\n", wcd937x_regmap_config.reg_defaults[i].reg, val);
		pos += rc;
		pr_debug("[wcd937x]%s rc = %d, pos[%d] = %d\n", __func__, rc, i, pos);
	}
	pr_debug("[wcd937x]%s successfully read %d regs, pos %d\n", __func__, i, pos);

	rc = scnprintf(buf, MAX_DATA_SIZE, "%s", data_temp);
	pr_debug("[wcd937x]%s Exit %d\n", __func__, rc);
	return rc;
}
static CLASS_ATTR_RW(registers);
 							 ...
 							 const char *buf, size_t count)
{
	 ...
	 if (kstrtou16(buf, 0, &wcdev->count))
	 	return -EINVAL;
 								...
 								const char *buf, size_t count)
{
	 struct wcd937x_class_dev *wcdev = container_of(c, struct wcd937x_class_dev,
	 												wcd937x_class);
	 if (kstrtou16(buf, 0, &wcdev->address))
		 return -EINVAL;
...
static struct attribute *wcd937x_class_attrs[] = {
	 &class_attr_address.attr,
	 &class_attr_count.attr,
	 &class_attr_registers.attr,
	 &class_attr_micbiasdet.attr,
	 NULL,
};
ATTRIBUTE_GROUPS(wcd937x_class);
#endif
...
static int wcd937x_suspend(struct device *dev)
{
	...
	wcd937x->pm_state = WCD937X_PM_ASLEEP;
...
static int wcd937x_resume(struct device *dev)
{
	...
	wcd937x->pm_state = WCD937X_PM_AWAKE;
...
#ifdef SYS_CLASS_DEBUG_AUDIO
	struct wcd937x_class_dev *wcdev;
    int rc = 0;
#endif
	pr_err("%s: Enter\n", __func__);
...
#ifdef SYS_CLASS_DEBUG_AUDIO
	wcdev = devm_kzalloc(wcd937x->dev, sizeof(*wcdev), GFP_KERNEL);

	if (!wcdev)
		pr_err("[wcd937x]%s:Failed to alloc wcd937x class\n", __func__);
	else {
		wcdev->regmap = wcd937x->regmap;
		wcdev->wcd937x_class.name = dev_name(wcd937x->dev);
		wcdev->wcd937x_class.class_groups = wcd937x_class_groups;
		rc = class_register(&wcdev->wcd937x_class);
		if (rc < 0){
			pr_err("[wcd937x]%s: Failed to create wcd937x class rc=%d\n", __func__, rc);
			devm_kfree(wcd937x->dev, wcdev);
		} else {
			pr_info("[wcd937x]%s: Successed to create %s\n", __func__, wcdev->wcd937x_class.name);
		}
	}
#endif

	pr_err("%s: Successfully exit ret=%d\n", __func__, ret);
