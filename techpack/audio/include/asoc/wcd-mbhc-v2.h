...
struct wcd_mbhc_config {
	...
	/*audio_v add*/
	/* HP switch control for mbhc, changed by ChenJinQuan. */
	int (*get_hp_switch_state)(void);
	int (*set_hp_switch_state)(int);
	int (*get_hifi_dac_state)(void);
	void (*smartpa_debug_retry_probe)(void);
	bool imped_detect_en;
	uint32_t imped_adjust_ref;
	/* Impedance pointer for HiFi, changed by ChenJinQuan. */
	unsigned int *hp_impedance;
	bool mbhc_detect_moisture;
	/*audio_v add end*/
};
...
struct wcd_mbhc_fn {
	...
	/*audio_v add*/
	void (*wcd_mbhc_detect_moisture_fn)(struct work_struct *work);
	int (*wcd_mbhc_get_mic_adc_voltage)(struct wcd_mbhc *mbhc);
	/*audio_v add end*/
};
...
struct wcd_mbhc {
	...
	/* Delayed work to btn release @zgbing */
	struct delayed_work mbhc_btn_release_dwork; /*audio_v add*/
	int buttons_pressed;
	...
	bool in_gpio_irq_handler;/*audio_v add*/
	bool hphl_swh; /*track HPHL switch NC / NO */
	bool gnd_swh; /*track GND switch NC / NO */
	u32 hs_thr;
	u32 hph_thr;
	u32 micb_mv;
	u32 hpsw_thr;/*audio_v add*/
	u32 swap_thr;
	u32 moist_vref;
	u32 moist_iref;
	u32 moist_rref;
	...
	bool swch_detection_type;
	bool gpio_detection_type;
	/*audio_v add end*/
	...
	/*audio_v add*/
	/*add to read mbhc_status for user @zgbing */
	struct dentry *at_file_node;
	struct work_struct detect_moisture_work;
	bool is_detect_moisture;
	bool is_atmode;
	/*audio_v add end*/
};
