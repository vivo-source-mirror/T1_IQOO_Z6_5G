...
static struct apr_test audio_test = {
	.enable = 0,
	.cnt = 0,
};
...
EXPORT_SYMBOL(audio_apr_test_set);
...
int apr_send_pkt(void *handle, uint32_t *buf)
{
	...
	if (audio_test.enable && audio_test.cnt > 0) {
		int need_test = 0;
		switch (audio_test.cmd) {
		case APR_ERROR_TEST:
			need_test = 1;
			rc = -EINVAL;
			break;
		case APR_BLOCKED_TEST:
			need_test = 1;
			rc = 0;
			break;
		case APR_PCM_BUFFER_TEST:
			if (hdr->opcode == ASM_DATA_CMD_WRITE_V2 ||
				hdr->opcode == ASM_DATA_CMD_READ_V2)
				need_test = 1;
			rc = -EINVAL;
			break;
		default:
		...
		if (need_test) {
			audio_test.cnt--;
			pr_info("APR: audio test cnt %d ret %d\n",
						audio_test.cnt, rc);
			spin_unlock_irqrestore(&svc->w_lock, flags);
			return rc;
		}
	...
	if (svc->dest_id == APR_DEST_QDSP6 && rc == -EAGAIN) {
		pr_err("%s() apr_tal_write -0x%x\n", __func__, rc);
#if defined(VIVO_PROJECT_MODEL)
/*GDPR code*/
#else
/* not GDPR code start */
#ifdef CONFIG_RSC_VAUDIT_DEBUG
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_ADSP_SUBSYSTEM, 0,
			"%s: apr_tal_write fail, rc: 0x%x.", __func__, rc);
#endif
/* not GDPR code end */
#endif
