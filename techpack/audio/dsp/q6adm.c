...
struct adm_ctl {
	...
	atomic_t ld_stat;
	wait_queue_head_t ld_stat_wait;
};
...
/* def VIVO_AUDIO_KTV */
#if 1
int vivo_send_adm_apr_pkt(uint32_t *pkg)
{
	struct adm_cmd_set_pp_params_v5	*adm_params = NULL;
	adm_params = (struct adm_cmd_set_pp_params_v5 *) pkg;
	pr_debug("%s param length %d.\n", __func__, adm_params->payload_size);
	return apr_send_pkt(this_adm.apr, (uint32_t *)adm_params);
}
#endif
	...
	if (data->payload_size >= sizeof(uint32_t)) {
		...
		if (data->opcode == APR_BASIC_RSP_RESULT) {
			...
			case ADM_CMD_SET_PP_PARAMS_V6:
				...
				&this_adm.copp.wait[port_idx][copp_idx]);
				break;
				ktv_basic_callback(payload, data->payload_size);
				/*
			...
			case ADM_CMD_GET_PP_TOPO_MODULE_LIST_V2:
				...
				if (payload[1] != 0)
					pr_err("%s: ADM get topo list error = %d\n",
					       __func__, payload[1]);
				break;
			case ADM_CMD_KTV_OPEN:
			case ADM_CMD_KTV_SET_PARAMS:
			case ADM_CMD_KTV_GET_PARAMS:
				pr_debug("%s: KTV CMD 0x%x\n",
					__func__, payload[0]);
				ktv_basic_callback(payload, data->payload_size);
				break;
			case ADM_CMD_ADSP_DYNAMIC_MOUDLE:
				pr_debug("%s: ADM_CMD_ADSP_DYNAMIC_MOUDLE ret %d\n",
						__func__, payload[1]);
				atomic_set(&this_adm.ld_stat, payload[1]);
				wake_up(&this_adm.ld_stat_wait);
				break;
			default:
		...
		case ADM_CMDRSP_SHARED_MEM_MAP_REGIONS:
			...
			wake_up(&this_adm.adm_wait);
			break;
		case ADM_CMDRSP_KTV_GET_PARAMS:
			pr_debug("%s: ADM_CMDRSP_KTV_GET_PARAMS\n", __func__);
			pr_info("%s: port_id %x\n", __func__, data->token);
			ktv_params_callback(payload, data->payload_size);
			break;
		default:
			pr_err("%s: Unknown cmd:0x%x\n", __func__,
				data->opcode);
	    ...
	    struct msm_pcm_channel_mixer *ec_ref_chmix_cfg)
{
	...
	if ((tmp_port == AFE_PORT_ID_RT_PROXY_PORT_001_RX) &&
	    ((topology == AW_COPP_TOPOLOGY) ||
		(topology == AW_COPP_TOPOLOGY_V2) ||
		(topology == AW_COPP_TOPOLOGY_V3))){
		topology = NULL_COPP_TOPOLOGY;
		pr_debug("%s: the port is 0x%x, use NULL_COPP_TOPOLOGY\n", __func__, tmp_port);
	}
	...
	set_vivo_ktv_copp_idx(topology, copp_idx, port_id);
...
/* ADSP load capi_v2 dynamic modules */
int adm_load_dynamic_modules(void)
{
	int sz, rc = 0;
	struct apr_hdr *apr_hdr_ptr = NULL;
	...
	apr_hdr_ptr = kzalloc(sz, GFP_KERNEL);
	if (!apr_hdr_ptr) {
		pr_err("%s, adm params memory alloc failed", __func__);
		return -ENOMEM;
	}

	apr_hdr_ptr->hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
				APR_HDR_LEN(APR_HDR_SIZE), APR_PKT_VER);
	apr_hdr_ptr->pkt_size = sz;
	apr_hdr_ptr->src_svc = APR_SVC_ADM;
	apr_hdr_ptr->src_domain = APR_DOMAIN_APPS;
	apr_hdr_ptr->src_port = 0;
	apr_hdr_ptr->dest_svc = APR_SVC_ADM;
	apr_hdr_ptr->dest_domain = APR_DOMAIN_ADSP;
	apr_hdr_ptr->dest_port = 0;
	apr_hdr_ptr->token = 0;
	apr_hdr_ptr->opcode = ADM_CMD_ADSP_DYNAMIC_MOUDLE;

	atomic_set(&this_adm.ld_stat, -1);
	rc = apr_send_pkt(this_adm.apr, (uint32_t *)apr_hdr_ptr);
	if (rc < 0) {
		pr_err("%s: rc %d\n", __func__, rc);
		rc = -EINVAL;
		goto exit;
	...
	/* Wait for the callback */
	rc = wait_event_timeout(this_adm.ld_stat_wait,
		atomic_read(&this_adm.ld_stat) >= 0,
		msecs_to_jiffies(TIMEOUT_MS));
	if (!rc) {
		pr_err("%s: ADM_CMD_ADSP_DYNAMIC_MOUDLE timed out\n", __func__);
		rc = -EINVAL;
		goto exit;
	} else if (atomic_read(&this_adm.ld_stat) > 0) {
		pr_err("%s: DSP returned error[%s]\n",
				__func__, adsp_err_get_err_str(
				atomic_read(&this_adm.ld_stat)));
		rc = atomic_read(&this_adm.ld_stat);
		goto exit;
	}
	rc = 0;
...
EXPORT_SYMBOL(adm_load_dynamic_modules);
...
int __init adm_init(void)
{
	...
	/* ADSP load capi_v2 dynamic modules */
	atomic_set(&this_adm.ld_stat, 0);
	init_waitqueue_head(&this_adm.ld_stat_wait);
