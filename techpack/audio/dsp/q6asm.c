...
/* not GDPR code start */
		...
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_ADSP_SUBSYSTEM, 0,
			"%s(): closing asm, rc: %d.", __func__, rc);
#endif
/* not GDPR code end */
#endif
		pr_err("%s: Asm already opened, closing asm, rc: -0x%x.\n",
			__func__, rc);
...
#ifdef CONFIG_RSC_VAUDIT_DEBUG
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_ADSP_SUBSYSTEM, 0,
			"%s(): asm map timeout, rc: %d.", __func__, rc);
#endif
/* not GDPR code end */
#endif
		pr_err("%s: timeout. waited for memory_map\n", __func__);
...
/* not GDPR code end */
#endif
		pr_err("%s: Asm already opened, closing asm, rc: %d.\n",
			__func__, rc);
		q6asm_cmd(ac, CMD_CLOSE);
	}
