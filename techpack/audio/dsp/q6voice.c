...
static int voice_send_cvp_create_cmd(struct voice_data *v)
{
	...
	if (v->async_err > 0) {
		...
		ret = adsp_err_get_lnx_err_code(
				v->async_err);
		if (v->async_err == ADSP_ENORESOURCE) {
			pr_err("%s: error cnt %d \n", __func__, q6_voice_err_cnt++);
			if (q6_voice_err_cnt > 1 &&
...
/* not GDPR code start */
				...
				rsc_vaudit(VAUDIT_AUDIO, AUDIO_ADSP_SUBSYSTEM, 0,
					"Restart adsp, %s() DSP error[%s].",
					__func__, adsp_err_get_err_str(v->async_err));
#endif
/* not GDPR code end */
#endif
				pr_err("%s: restart adsp\n",__func__);
