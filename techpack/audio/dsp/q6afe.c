...
/* port AW882XX Driver by vivo audio team @guanzhaozhi */
...
#define AFE_PORT_ID_AWDSP_RX			(AFE_PORT_ID_SECONDARY_MI2S_RX)
#define AFE_PORT_ID_AWDSP_TX			(AFE_PORT_ID_SECONDARY_MI2S_RX)
#endif
/* #endif CONFIG_SND_SOC_AWINIC_AW882XX */

#ifndef CONFIG_SND_SOC_TFA9874_OR_HAPTIC
#define CONFIG_SND_SOC_TFA9874_OR_HAPTIC
#endif
...
#ifdef SMART_AMP
	struct afe_smartamp_calib_get_resp smartamp_calib_resp;
#endif
				    ...
				    uint32_t payload_size)
{
	...
	default:
#ifdef SMART_AMP
		if((param_hdr.module_id == AFE_SMARTAMP_MODULE) ||
			(param_hdr.module_id == AFE_SMARTAMP_MODULE_TX) ||
			(param_hdr.module_id == CAPI_V2_VIVO_SP_RX) ||
			(param_hdr.module_id == CAPI_V2_VIVO_SP_TX)) {
			expected_size += sizeof(struct afe_smartamp_get_params_t);
			data_dest = (u32 *) &this_afe.smartamp_calib_resp;
			pr_debug("%s: smartAmp module_id: 0x%x, instance_id: %u, reserved: %u, "
				"param_id: %u, param_size: %u.\n", __func__, param_hdr.module_id,
				param_hdr.instance_id, param_hdr.reserved, param_hdr.param_id,
				param_hdr.param_size);
			if (param_hdr.param_size > sizeof(struct afe_smartamp_get_params_t)) {
				param_hdr.param_size = sizeof(struct afe_smartamp_get_params_t);
				pr_err("%s: param_id %u resp param_size is exceed apss.\n",
...
/* #endif CONFIG_SND_SOC_AWINIC_AW882XX */
		...
		if (afe_token_is_valid(data->token))
			...
			return -EINVAL;
	} else if (data->opcode == AFE_SVC_CMDRSP_GET_PARAM) {
		struct afe_lpass_clk_cfg_callback *pdata = NULL;
		uint32_t *payload = data->payload;
		if (!payload || (data->token >= AFE_MAX_PORTS)) {
			pr_err("%s: Error: size %d payload %pK token %d\n",
				__func__, data->payload_size,
		...
		pr_info("%s: AFE_SVC_CMDRSC_GET_PARAM done\n", __func__);
		pdata = (struct afe_lpass_clk_cfg_callback *)payload;
		this_afe.clk_cfg = pdata->cfg;
		atomic_set(&this_afe.state, 0);
		atomic_set(&this_afe.status, pdata->status);
		if (afe_token_is_valid(data->token))
			wake_up(&this_afe.wait[data->token]);
		else
			return -EINVAL;
		...
		if (data->opcode == APR_BASIC_RSP_RESULT) {
			...
			if (payload[0] == AFE_CMD_ADSP_RESTART_STRESS_TEST){
				/* this will never happen for adsp crashed */
				vivo_apr_callback(payload[0], payload[1]);
				return 0;
			}
			...
			switch (payload[0]) {
			case AFE_SVC_CMD_GET_PARAM:
				memset(&this_afe.clk_cfg, 0, sizeof(struct afe_clk_get));
				atomic_set(&this_afe.state, 0);
				if (afe_token_is_valid(data->token))
					wake_up(&this_afe.wait[data->token]);
				else
...
#ifdef SMART_AMP
static int smartpa_mi2s_rx_port;
static int smartpa_mi2s_tx_port;

void smartpa_set_mi2s_port(int port_rx, int port_tx)
{
	smartpa_mi2s_rx_port = port_rx;
	smartpa_mi2s_tx_port = port_tx;
}
EXPORT_SYMBOL(smartpa_set_mi2s_port);
	        ...
	        uint8_t length, uint32_t module_id)
{
	...
	if ((module_id == AFE_SMARTAMP_MODULE) || (module_id == CAPI_V2_VIVO_SP_RX)) {
		port = smartpa_mi2s_rx_port;
		param_hdr.instance_id = INSTANCE_ID_0;
	} else if ((module_id == AFE_SMARTAMP_MODULE_TX) || (module_id == CAPI_V2_VIVO_SP_TX)) {
		port = smartpa_mi2s_tx_port;
	...
	param_hdr.module_id = module_id;
	param_hdr.param_id = param_id;
	param_hdr.param_size = sizeof(struct afe_smartamp_set_params_t);

	ret = q6afe_pack_and_set_param_in_band(port,
						   q6audio_get_port_index(port),
						   param_hdr, (u8 *) prot_config);
	if (ret < 0) {
		pr_err("%s: Failed to set calib data, port %d ret %d\n",
			   __func__, port, ret);
		goto params_fail;
	}
		...
		uint32_t param_id, uint32_t module_id)
{
	...
	calib_resp->pdata = param_hdr;
	memcpy(&calib_resp->param, &this_afe.smartamp_calib_resp.param,
		sizeof(this_afe.smartamp_calib_resp.param));
	pr_debug("%s: payload[0] %d\n", __func__, calib_resp->param.payload[0]);
	ret = 0;
params_fail:
done:
	mutex_unlock(&this_afe.afe_cmd_lock);
...
int afe_get_lpass_clk_cfg(int port_id, struct afe_clk_get *cfg)
{
	struct afe_lpass_clk_config_acquire clk_cfg;
	...
	clk_cfg.hdr.opcode = AFE_SVC_CMD_GET_PARAM;
	...
	clk_cfg.port_id = q6audio_get_port_id(port_id);
...
EXPORT_SYMBOL(afe_get_lpass_clk_cfg);
...
EXPORT_SYMBOL(vivo_afe_apr_send_pkt);
