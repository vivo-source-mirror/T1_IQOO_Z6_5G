...
/* not GDPR code start */
#ifdef CONFIG_RSC_VAUDIT_DEBUG
				rsc_vaudit(VAUDIT_AUDIO, AUDIO_SWR_BUS, 0,
					"SWR: clock enable req failed: %d", ret);
#endif
/* not GDPR code end */
#endif
				dev_err_ratelimited(swrm->dev,
...
static void swrm_enable_slave_irq(struct swr_mstr_ctrl *swrm)
{
	...
	pr_info("%s: vatsal master num %d swrm dev %d\n", __func__, swrm->master.num_dev, swrm->num_dev);
	for (i = 0; i < (swrm->master.num_dev + 1); i++) {
...
/* not GDPR code start */
#ifdef CONFIG_RSC_VAUDIT_DEBUG
	if (!swrm->master.num_dev || !swrm->num_dev)
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SWR_BUS, 0,
			"SWR: swrm_enable_slave_irq %d %d",
			swrm->num_dev, swrm->master.num_dev);
#endif
/* not GDPR code end */
#endif
}
...
#ifdef CONFIG_RSC_VAUDIT_DEBUG
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SWR_BUS, 0,
			"SWR: mstr_interrupt: Failed to hold suspend.");
#endif
/* not GDPR code end */
#endif
	...
	dev_err(swrm->dev, "%s acquired pm lock\n", __func__);
...
#ifdef CONFIG_RSC_VAUDIT_DEBUG
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SWR_BUS, 0,
			"SWR: wakeup_interrupt: Failed to hold suspend.");
#endif
/* not GDPR code end */
#endif
		dev_err(swrm->dev, "%s Failed to hold suspend\n", __func__);
...
#ifdef CONFIG_RSC_VAUDIT_DEBUG
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SWR_BUS, 0,
			"SWR: wakeup_work: Failed to hold suspend.");
#endif
/* not GDPR code end */
#endif
		dev_err(swrm->dev, "%s Failed to hold suspend\n", __func__);
...
#ifdef CONFIG_RSC_VAUDIT_DEBUG
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SWR_BUS, 0,
			"SWR: device 0x%llx is not ready.", dev_id);
#endif
/* not GDPR code end */
#endif
...
#ifdef CONFIG_RSC_VAUDIT_DEBUG
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SWR_BUS, 0,
			"SWR: wakeup_vote: Failed to hold suspend.");
#endif
/* not GDPR code end */
#endif
		dev_err(swrm->dev, "%s Failed to hold suspend\n", __func__);
...
static bool swrm_lock_sleep(struct swr_mstr_ctrl *swrm)
{
	...
	if (swrm->wlock_holders++ == 0) {
		dev_dbg(swrm->dev, "%s: holding wake lock, holders=%d\n", __func__, swrm->wlock_holders);
...
static void swrm_unlock_sleep(struct swr_mstr_ctrl *swrm)
{
	...
	dev_dbg(swrm->dev, "%s: wake lock, holders=%d\n", __func__, swrm->wlock_holders);
	mutex_unlock(&swrm->pm_lock);
	wake_up_all(&swrm->pm_wq);
}
